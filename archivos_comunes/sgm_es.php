<?php

#index.php
$Panel_de_Usuario = "Panel de Usuario";
$Salir = "Salir";
$Es = "Es";
$Cat = "Cat";
$Trabajado = "Trabajado";
$Laborables = "Laborables";
$Bienvenido = "Bienvenido";
$Identificarse = "Identificarse";
$meses = array("Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre");
$Si = "Si";
$No = "No";

#gestion_comercial.php
$Comercial = "Comercial";
$Contenidos = "Contenidos";
$Contenido = "Contenido";
$Contenido_Estendido = "Contenido Estendido";
$Text_Contenidos = "Texto de los Contenidos";
$Oferta_Comercial = "Oferta Comercial";
$Valores = "Valores";
$Datos_Generales = "Datos Generales";
$Datos_Adicionales = "Datos Adicionales"; 

#area_clientes
$Auditoria = "Auditoria";
$Alerta = "Alerta";
$Area = "Area";
$Atencion = "Atenci&oacute;n";
$Anotacion = "Anotaci&oacute;n";
$Anterior = "Anterior";
$Caracteres = "Car&aacute;cteres";
$Cuenta_Bancaria = "Cuenta Bancaria";
$Correcto = "Correcto";
$Datos_Bancarios = "Datos Bancarios";
$Datos_Comerciales = "Datos Comerciales";
$DOMICILIADA = "DOMICILIADA";
$Espacio = "Espacio";
$Error = "Error";
$FACTURADO = "FACTURADO";
$Fuera = "Fuera";
$Guardias = "Guardias";
$Lunes = "Lunes";
$Memoria = "Memoria";
$PAGADA = "PAGADA";
$PENDIENTE = "PENDIENTE";
$Plataforma = "Plataforma";
$Periodo = "Periodo";
$Parametros = "Parametros";
$Reports = "Reports";
$Respuesta = "Respuesta";
$Solucionado = "Solucionado";
$Seguridad = "Seguridad";
$Tecnico = "T&eacute;cnico";
$Uso = "Uso";
$Ultimo = "&Uacute;ltimo";
$Ultimos = "&Uacute;ltimos";
$Viernes = "Viernes";

#gestion_facturas.php
$Facturacion = "Facturaci&oacute;n";
$Resumenes = "Resumenes";
$Resumen = "Resumen";
$Totales = "Totales";
$Anual = "Anual";
$Enero = "Enero";
$Febrero = "Febrero";
$Marzo = "Marzo";
$Abril = "Abril";
$Mayo = "Mayo";
$Junio = "Junio";
$Julio = "Julio";
$Agosto = "Agosto";
$Septiembre = "Septiembre";
$Octubre = "Octubre";
$Noviembre = "Noviembre";
$Diciembre = "Diciembre";
$Periodos = "Periodos";
$Desplegar = "Desplegar";
$Todo = "Todo";
$Vencimiento = "Vencimiento";
$Linea = "Linea";
$Documentos = "Documentos";
$Modificadores = "Modificadores";
$Entrega = "Entrega";
$Divisa = "Divisa";
$Divisas = "Divisas";
$Convertir = "Convertir";
$Subtipos = "Subtipos";
$Cajas = "Cajas";
$Fuentes = "Fuentes";
$Cambiada = "Cambiada";
$Barras = "Barras";
$Base_imponible = "Base Imponible";
$Trimestre = "Trimestre";
$Recibos = "Recibos";
$Entidad = "Entidad";
$Bancaria = "Bancaria";
$Caja = "Caja";
$Idiomas = "Idiomas";
$Parcial = "Parcial";
$Traspaso = "Traspaso";
$Forma = "Forma";
$Pago = "Pago";
$Billetes = "Billetes";
$Monedas = "Monedas";
$Efectivo = "Efectivo";
$Situacion = "Situaci&oacute;n";
$Ingresos = "Ingresos";
$Gastos = "Gastos";
$Cerrar = "Cerrar";
$Simbolo = "Simbolo";
$Cobrado = "Cobrado";

#gestion_contrasenyas.php
$Aplicacion = "Aplicaci&oacute;n";
$Aplicaciones = "Aplicaciones";
$Contrasenas = "Contrase&ntilde;as";
$Campos = "Campos";
$Obligatorios = "Obligatorios";
$Ver = "Ver";

#gestion_monitorizacion.php
$Tiempo = "Tiempo";
$Cobertura = "Cobertura";
$Filtrar = "Filtrar";
$Final = "Final";
$Finalizar = "Finalizar";
$Minutos = "Minutos";
$SLA = "SLA";
$NBD = "NBD";
$Horas = "Horas";
$Desactivar = "Desactivar";
$Activar = "Activar";
$Alertas = "Alertas";
$Base = "Base";
$Bases = "Bases";
$Bases_Datos = "Bases de Datos";
$Servidores = "Servidores";
$Concepto = "Concepto";
$Proyecto = "Proyecto";
$Proyectos = "Proyectos";
$Obligatorio = "Obligatorio";
$Extranet = "Extranet";
$Destino = "Destino";
$Visible = "Visible";
$Pausar = "Pausar";
$Ano = "A&ntilde;o";

#gestion_clientes.php
$Administracion = "Administraci&oacute;n";
$Administrar = "Administrar";
$Agrupacion = "Agrupaci&oacute;n";
$Anadir = "A&ntilde;adir";
$Anadir_Contacto = "A&ntilde;adir Contacto";
$AnadiryEliminarIncompatibles = "A&ntilde;adir y Eliminar Incompatibles";
$Apellido = "Apellido";
$Archivos = "Archivos";
$Alias = "Alias";
$Ajuda_alias = "El <em>Alias</em> es el nombre que tendra la carpeta en el cloud donde el cliente podra consultar toda su documentaci&oacute;n desde el Area de Clientes";

$Boton = "Bot&oacute;n";
$Buscar = "Buscar";
$Buscar_Contacto = "Buscar Contacto";
$Busquedas = "Busquedas";

$Calle = "Calle";
$Calles = "Calles";
$Caracteristicas = "Caracter&iacute;sticas";
$Cargo = "Cargo";
$Central = "Central";
$Certificacion = "Certificaci&oacute;n";
$Certificaciones = "Certificaciones";
$Cerrada = "Cerrada";
$Cliente = "Cliente";
$Clientes =  "Clientes";
$Cliente_fidelizado = "Cliente fidelizado";
$Cliente_vip = "Cliente vip";
$Cobrada = "Cobrada";
$Color = "Color";
$Confirmada = "Confirmada";
$Contacto = "Contacto";
$CONTACTOS = "CONTACTOS";
$Contactos = "Contactos";
$Controles_de_calidad = "Controles de calidad";
$Con_delegaciones_asociadas = "Con delegaciones asociadas.";
$Completa = "Completa";
$Completo = "Completo";
$CP = "CP";
$Cuentas = "Cuentas";
$Campos_obligatorios = "Campos obligatorios";
$Cuenta_Contable = "Cuenta Contable";
$Costes_Adicionales = "Costes Adicionales";
$Cancelar = "Cancelar";

$Datos_fiscales_incompletos = "NIF/CIF/NIE incorrectos.";
$Datos_Adicionales = "Datos Adicionales";
$Datos_Administrativos = "Datos Administrativos";
$Datos_Contacto = "Datos Contacto";
$Datos_Fiscales = "Datos Fiscales";
$Datos_Tecnicos = "Datos T&eacute;cnicos";
$Datos_Facturacion = "Datos Facturaci&oacuten";
$Definiciones = "Definiciones";
$Definicion = "Definici&oacute;n";
$Delegacion = "Delegaci&oacute;n";
$Departamento = "Departamento";
$Dependencias = "Dependencias";
$Desde= "Desde";
$Despre = "Despre.";
$Direcciones = "Direcciones";
$Direccion = "Direcci&oacute;n";
$Direcciones_Envio = "Direcciones de Envio";

$Editar = "Editar";
$Eliminar = "Eliminar";
$Email = "E-mail";
$Empresa = "Empresa";
$en_orden_alfabetico = " en orden alfab&eacute;tico :";
$envio = "envio";
$Estado = "Estado";
$Estado_de_la_Certificacion = "Estado de la Certificaci&oacute;n";
$Estado_del_Servicio = "Estado del Servicio";
$Estado_de_Cuentas = "Estado de Cuentas";
$Extranjero = "Extranjero";

$Fax = "Fax";
$Fecha = "Fecha";
$Ficha_de_Contacto = "Ficha de Contacto";
$Filtrado_por = "Filtrado por";
$Fiscal = "Fiscal";
$Formulario = "Formulario";
$Fisica = "F&iacute;sica";
$Formulario_Subir_Archivo = "Formulario Subir Archivo";

$Gestion_Certificaciones = "Gestión de Certificaciones";
$Gestion_Comercial = "Gesti&oacute;n Comercial";
$Grupo = "Grupo";
$Grupos = "Grupos";
$Guardar = "Guardar";
$Guardar_Busqueda = "Guardar Busqueda";

$Hasta= "Hasta";
$Historico = "Hist&oacute;rico";

$Idioma = "Idioma";
$Inactivo = "Inactivo";
$Incidencias = "Incidencias";
$Incompatibles = "Incompatibles";
$Indicador = "Indicador";
$Indicadores = "Indicadores";
$Informacion = "Informaci&oacute;n";
$Informes = "Informes";
$Inicial = "Inicial";
$Importe = "Importe";
$Imprimir = "Imprimir";
$Imprimir_Lista = "Imprimir Lista";

$Juridica = "Jur&iacute;dica";

$Letra = "Letra";
$Listar = "Listar : ";
$Listado = "Listado";

$Modificar = "Modificar";
$Movil = "M&oacute;vil";

$NIF = "NIF";
$Nombre = "Nombre";
$Nombre_cliente = "Nombre cliente";
$Notas = "Notas";
$Nueva = "Nueva";
$Numero = "N&uacute;mero";

$Observacions = "Observaciones";
$Opciones = "Opciones";
$Otras = "Otras";
$Oficina_contable = "Oficina Contable";
$Organo_gestor = "Organo Gestor";

$Pais = "Pa&iacute;s";
$PERSONALES = "PERSONALES";
$Personales = "Personales";
$Personalizada = "Personalizada";
$Poblacion = "Poblaci&oacute;n";
$por_empresa = " por empresa";
$por_nombre = " por nombre";
$Predet = "Predet.";
$Predeterminar = "Predeterminar";
$Principal = "Principal";
$Proveedor = "Proveedor";
$Proveedores = "Proveedores";
$Provincia = "Provincia";

$Resultado_de_la_busqueda = "Resultado de la busqueda: ";
$Residente = "Residente";

$Secretario = "Secretario/a";
$Sector = "Sector";
$Sectores = "Sectores";
$Servicio = "Servicio";
$Solo_predeterminado = "Solo predeterminado";
$Simple = "Simple";

$Telefono = "Tel&eacute;fono";
$Tipo = "Tipo";
$Todos = "Todos";
$Trato = "Trato";
$Tipos = "Tipos";
$Tratos = "Tratos";
$Trabajadores = "Trabajadores";
$Total = "Total";

$Ubicacion = "Ubicaci&oacute;n";
$Ubicaciones = "Ubicaciones";
$Usuario = "Usuario";
$Unidad_tramitadora = "Unidad Tramitadora";

$Valor = "Valor";
$Valoracion = "Valoraci&oacute;n";
$Ver = "Ver";
$Ver_Todos = "Ver Todos";
$Ver_Todos_Personales = "Ver Todos Personales";
$Volver = "Volver";

$Web = "Web";

$ygriega = "y";

$Zona = "Zona";

#gestion_incidencias.php
$Acceso = "Acceso";
$Actual = "Actual";
$AGENDA = "AGENDA";
$Agenda = "Agenda";
$Agendas = "Agendas";
$agendas = "agendas";
$Anterior = "Anterior";
$Asociadas = "Asociadas";
$Asunto = "Asunto";
$Aviso = "Aviso";
$Abrir = "Abrir";

$Caducadas = "Caducadas";
$Calendario = "Calendario";
$Cambio = "Cambio";
$Codigo = "C&oacute;digo";
$Conclusion = "Conclusi&oacute;n";

$Departamentos = "Departamentos";
$Dependencia = "Dependencia";
$Desarrollo = "Desarrollo";
$Descripcion = "Descripci&oacute;n";
$diaa = "dia";
$Duracion = "Duraci&oacute;n";

$Edicion = "Edici&oacute;n";
$Estados = "Estados";
$Escritura = "Escritura";
$Exportar = "Exportar";
$Editable = "Editable";

$Ficha = "Ficha";
$Filtrado = "Filtrado";
$Finalizacion = "Finalizaci&oacute;n";
$finalizable = "finalizable : Contiene tareas no finalizadas";
$Finalizada = "finalizada";

$General = "General";
$Grafico = "Gr&aacute;fico";

$Historial = "Historial";
$HISTORICO = "HIST&Oacute;RICO";

$Incidencia = "Incidencia";
$Incidencias = "Incidencias";
$Invitadas = "Invitadas";
$Importar = "Importar";

$Jornada = "Jornada";

$Lectura = "Lectura";
$Limite = "Limite";
$Lista = "Lista";

$Mes = "Mes";
$Motivo = "Motivo";

$Nota = "Nota";
$NoVer = "No Ver";

$Origen = "Origen";

$por = "por";
$Permiso = "Permiso";
$Plantilla = "Plantilla";
$Plantillas = "Plantillas";
$Prevision = "Previsi&oacute;n";
$Privada = "Privada";
$Propietario = "Propietario";
$Proximas = "Pr&oacute;ximas";
$Pausadas = "Pausadas";

$Registro = "Registro";
$Registrada = "Registrada";

$Seleccionado = "Seleccionado";
$Seleccionar = "Seleccionar";
$Semana = "Semana";
$Servicios = "Servicios";
$Sincronizaciones = "Sincronizaciones";
$Sincronizar = "Sincronizar";
$SIN = "SIN";
$Sin = "Sin";
$SinAsunto = "Sin Asunto";
$SinCliente = "Sin Cliente";

$Tarea = "Tarea";
$Tareas = "Tareas";
$Telefonica = "Telef&oacute;nica";
$Titulo = "T&iacute;tulo";

#gestion_auxiliares
$Actualizar = "Actualizar";
$Ancho = "Ancho";
$Alto = "Alto";
$Auxiliares = "Auxiliares";
$Archivo = "Archivo";
$Archivos_subidos = "Archivos subidos";

$CLIENTE = "CLIENTE";
$Clase = "Clase";

$Descuento = "Descuento";
$Datos = "Datos";
$Dia = "D&iacute;a";

$Explotar = "Explotar";
$Ejecutar = "Ejecutar";

$FICHA = "FICHA";
$Factura = "Factura";
$Formato = "Formato";

$Hoja = "Hoja";
$Hojas = "Hojas";
$Hora_Inicio = "Hora Inicio";
$Hora_Fin = "Hora Fin";

$Imagen = "Imagen";
$Introduccion = "Introducci&oacute;n";
$IMPRIMIR = "IMPRIMIR";
$Incremento = "Incremento";
$Insertar = "Insertar";
$Impresos = "Impresos";

$Logos = "Logos";
$Logo = "Logo";

$Mantenimientos = "Mantenimientos";
$Mantenimiento = "Mantenimiento";
$manual = "manual";

$Nacion = "Naci&oacute;n";

$OT = "OT";

$Papel = "Papel";
$Primera = "Primera";
$Posicion = "Posici&oacute;n";
$Porcentage = "Porcentage";

$Resultado = "Resultado";
$Registrar_Documento = "Registrar Documento";

$Siglas = "Siglas";
$Sobre = "Sobre";

$Tamano = "Tama&ntilde;o";
$Tarifas = "Tarifas";
$Tabla = "Tabla";
$Ticket = "Ticket";
$Trabajo = "Trabajo";
$Traspasar = "Traspasar";

$Ultima = "&Uacute;ltima";

$Ver_Horas = "Ver Horas";

#gestion_usuarios.php
$Usuarios = "Usuarios";
$Crear = "Crear";
$Solo = "Solo";
$Activos = "Activos";
$Gestion = "Gesti&oacute;n";
$Remoto = "Remoto";
$Validado = "Validado";
$Activo = "Activo";
$Conexion = "Conexi&oacute;n";
$Permisos = "Permisos";
$Admin = "Admin.";
$Modulo = "M&oacute;dulo";
$Cambiar = "Cambiar";
$Mostrar = "Mostrar";
$Nacimiento = "Nacimiento";
$Contrasena = "Contrase&ntilde;a";
$Repetir = "Repetir";
$Creacion = "Creaci&oacute;n";
$Cuenta = "Cuenta";
$Red = "Red";
$Vigente = "Vigente";
$Almacen = "Almac&eacute;n";
$Almacenes = "Almacenes";
$Administrador = "Administrador";

#gestion_mailing.php
$Envio = "Envio";
$Ayuda = "Ayuda";
$Carta = "Carta";
$Configuracion = "Configuraci&oacute;n";
$Firma = "Firma";
$Texto = "Texto";
$Adjuntar = "Adjuntar";
$Adjuntos = "Adjuntos";
$Servidor = "Servidor";
$Correo = "Correo";
$Electronico = "Electr&oacute;nico";
$Nuevo = "Nuevo";
$Puerto = "Puerto";
$Remitente = "Remitente";
$Personal = "Personal";
$Marcar = "Marcar";
$Desmarcar = "Desmarcar";
$Enviar = "Enviar";
$Etiqueta = "Etiqueta";
$Para = "Para";
$Por = "Por";

#gestion_inventario.php
$Actuacion = "Actuaci&oacute;n";
$Actuaciones = "Actuaciones";
$Atributo = "Atributo";
$Atributos = "Atributos";

$Dato = "Dato";
$Detalles = "Detalles";
$Dispositivo = "Dispositivo";
$Dispositivos = "Dispositivos";

$Echo = "Echo";
$Esquema = "Esquema";

$Inventario = "Inventario";

$Numero_serie = "N&uacute;mero Serie";

$Orden = "Orden";

$Pendientes = "Pendientes";
$Parada = "Parada";

$Relacion = "Relaci&oacute;n";
$Relaciones = "Relaciones";
$Relacionar = "Relacionar";
$Relacionado = "Relacionado";

$Tipo = "Tipo";
$Trabajador = "Trabajador";

#gestion_contratos.php
$Contratos = "Contratos";
$Contrato = "Contrato";
$desde = "desde";
$hasta = "hasta";
$Inicio = "Inicio";
$Fin = "Fin";
$Firmante = "Firmante";
$a_efecto = "a efecto";
$Corriente = "Corriente";
$Dias = "Dias";
$Responsable_cliente = "Responsable Cliente";
$Responsable_tecnico = "Responsable T&eacute;cnico";

#gestion_articulos.php
$Articulos = "Articulos";
$Articulo = "Art&iacute;culo";
$articulo = "art&iacute;culo";
$antiguo = "antiguo";
$Antiguo = "Antiguo";
$abreviatura = "abreviatura";
$Abreviatura = "Abreviatura";
$actualizado = "actualizado";

$Calculo = "C&aacute;lculo";
$Costes = "Costes";
$Coincidencias = "Coincidencias";
$Clasificar = "Clasificar";
$Codificado = "Codificado";
$Caracteristica = "Caracter&iacute;stica";
$carpeta = "carpeta";
$Configurar = "Configurar";
$Catalogar = "Catalogar";
$Calculadora = "Calculadora";

$Descatalogados = "Descatalogados";
$Descatalogar = "Descatalogar";
$Desde_archivo = "Desde archivo";
$disponible = "disponible";
$Duplicar = "Duplicar";
$Duplicado = "Duplicado";

$Escandallo = "Escandallo";
$escandallo = "escandallo";
$Externo = "Externo";
$Estanterias = "Estanterias";
$Estanteria = "Estanteria";
$Entrada = "Entrada";

$Familia = "Familia";
$Familias = "Familias";
$Fabricante = "Fabricante";
$Forzado = "Forzado";

$Generales = "Generales";

$Herramienta = "Herramienta";

$Imagenes = "Imagenes";
$Impresion = "Impresi&oacute;n";
$info = "info";
$incluido = "incluido";

$Logistica = "Log&iacute;stica";
$Localizacion = "Localizaci&oacute;n";

$Marca = "Marca";
$Marcas = "Marcas";
$Minimo = "M&iacute;nimo";
$Maximo = "M&aacute;ximo";
$Multivia_com = "Multivia.com";

$nuevo = "nuevo";

$Original = "Original";

$Pasillo = "Pasillo";
$Precios = "Precios";
$Precio = "Precio";
$proveedor = "proveedor";
$PVP = "PVP";
$PVD = "PVD";
$Posibles = "Posibles";
$por_codigo = "por codigo";
$por_defecto = "por defecto";
$provisional = "provisional";
$Postal = "Postal";
$Por_clientes = "Por clientes";

$Referencias = "Referencias";

$Stock = "Stock";
$Subfamilia = "Subfamilia";
$Subfamilias = "Subfamilias";
$Subir = "Subir";
$Seccion = "Secci&oacute;n";
$Secciones = "Secciones";
$Sin_dependencias = "Sin Dependencia";
$Sin_clasificar = "Sin Clasificar";

$Traduccion = "Traducci&oacute;n";
$Todas = "Todas";

$Unidad = "Unidad";
$Unidades = "Unidades";

$Visualizar = "Visualizar";


#gestion_licencias.php
$Lincencias = "Licencias";
$Lincencia = "Licencia";



#errores
$UsuarioNoAutorizado = "USUARIO NO AUTORIZADO";
$error_incidencia_busqueda = "Para buscar por fechas, es necesario mes y a&ntilde;o.";

$UseNoAutorizado = "<br><center><table><tr><td>Usuario no autorizado.</td></tr></table></center>";
$UsuarioYaReg = "<br>Nombre de usuario ya registrado.";
$NombreIncorrecto = "<br>Valor en el campo nombre de usuario incorrecto.";
$MailYaReg = "<br>Direcci&oacute;n e-mail ya registrada.";
$MailIncorrecto = "<br>Valor en el campo e-mail incorrecto.";
$PassIncorrecto = "<br>Valor en los campos passwords incorrecto.";
$CompletaCorrect = "Completa correctamente todos los campos obligatorios.";
$InvalidarUsuario = "<br><br>¿Seguro que desea invalidar este usuario?";
$ErrorPass = "<br><br>Error al introducir las contraseñas<br><br>";
$NotasDes = "No se puede introducir una incidencia sin texto";
$cambiarDatos = "Si desea cambiar los datos de la empresa, pongase en contacto con ";

$definicionesIncompatibles = "La definici&oacute;n de cliente es incompatible con las ya existentes para este cliente. ¿Que desea hacer?";

$ValorDefectoCaracteristicaArt = "Si existe una valor por defecto se ignorará la tabla de opciones de dicha característica.";
$ErrorEscandallo = "No es posible incluir un articulo en si mismo.";
$ErrorAnadirArticulo = "No es posible a&ntilde;adir el articulo. El codigo o el nombre del articulo ya existen.";
$duplicarArticulo = "Se duplicar&aacute; el articulo y todos los datos relacionados con este (almacenage, stock, precios, etc.).";
$eliminarArticulo = "Se eliminar&aacute; el articulo en todo el progama. Todos los datos relacionados con &eacute;l ser&aacute;n inaccesible.";
$descatalogarArticulo = "El articulo estar&aacute; descatalogado, pero ser&aacute; accesible tanto a &eacute;l como a los datos relacionados.";
$catalogarArticulo = "El articulo volver&aacute; al cat&aacute;logo habitual de articulos con todas sus funciones y datos relacionados.";

$errorSubirArchivoDuplicado = "No se puede a&ntilde;adir archivo por que ya existe uno con el mismo nombre.";
$errorSubirArchivoTamany = "No ha podido transferirse el archivo.<br>No puede exceder el tama&ntilde;o indicado";
$errorSubirArchivo = "No ha podido transferirse el archivo.";

$CambioPassCorrecto = "La contrase&ntilde;a se ha modificado correctamente";

$ErrorRelacion = "El valor introducido no es numérico o no pertenece a un ID de una incidencia existente";
$ErrorSinTextoDuracion = "No se puede introducir una incidencia sin texto y/o duraci&oacute;n";
$ErrorSinServicioCliente = "No se puede introducir una incidencia sin servicio SIM o cliente";
$ErrorSinAsunto = "No se puede introducir una incidencia sin asunto";
$ErrorFinalizacion = "No se puede finalizar una incidencia sin notas de conclusi&oacute;n o sin servicio";
$ErrorBusqueda = "Para buscar por fechas, es necesario mes y a&ntilde;o.";

$Redireccion10seg = "En 10 segundos ser&aacute; redirigido por motivos de seguridad.";
$ContrasenyaAppErrorEliminar = "No se puede eliminar esta aplicacion, ya que existen contraseñas asociadas.";

$Explicacion_cobertura = "El periodo de cobertura de un servicio es la disponibilidad para la recepci&oacute;n de incidencias. Siendo el primer valor las horas al dia y el segundo los dias a la semana. Ejemplo : 8x5 equivale a 8 horas al dia 5 dias a la semana.";
$Explicacion_respuesta = "El tiempo de respuesta de una incidencia empieza a contar una vez recepcionada. Si es NBD, una vez fuera del horario laboral, la incidencia pasa al siguiente dia laboral.";

$pregunta_eliminar = "¿Desea eliminar el elemento seleccionado?";
$mesaje_modificar = "Se ha modificado correctamente";

?>