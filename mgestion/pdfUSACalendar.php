...
function isHoliday($date)
{
	...
	if ($month == 1 && $day == 6)
		return "Reis";
	if ($month == 5 && $day == 1)
		return "Dia Treballador";
	if ($month == 6 && $day == 24)
		return "Sant Joan";
	if ($month == 9 && $day == 11)
		return "Diada de Catalunya";
	if ($month == 10 && $day == 12)
		return "Dia de l'hispanitat";
	if ($month == 11 && $day == 1)
		return "Tots sants";
	if ($month == 12 && $day == 6)
		return "Constitució";
...
function tinyCalendar($date, $square)
{
    ...
    $mes = strtoupper(gmdate ("m", jdtounix($date)));
	$any = strtoupper(gmdate ("Y", jdtounix($date)));
	...
	$x = $this->x;
	$y = $this->y;
	$this->Cell(7*$square, $square, $mesesp, 0, 0, "C");
	$y+=$square;
	$this->SetXY($x, $y);
	$this->SetFontSize(8);
    for ($i = 0; $i < 7; ++$i)
        {
		$day = "LMXJVSD";
        $this->Cell($square, $square, $day[$i], 0,0,"C");
        }
	$y+=$square;
	$this->SetXY($x, $y);
	for ($k=0;$k<6;$k++)
...
function printMonth($date)
{
	...
	$this->SetXY($this->lMargin,$this->tMargin);
	$this->SetFont("Times", "B", $this->headerFontSize);
	$this->Cell($width, $firstLine, $mesesp." ".$any, 0,0, "C");
	...
	for ($i = 1; $i <= 7; ++$i)
		...
		if ($i > 5) { $fill = true;} else { $fill = false;}
		$this->Cell($verticalLines[$i],6,$dia[$i],0,0,"C",$fill);
		}
