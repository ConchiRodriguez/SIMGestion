...
class PDF extends FPDF
{
	...
	function factura_print($id,$tipo){
		...
		$sqldiv = "select * from sgm_divisas_canvis where id_divisa_origen=".$rowdi["id"];
...
#			$this->Cell(90,5,$rowcabezera["numero"]." / ".$rowcabezera["version"],1,0);
		...
		while ($row = mysqli_fetch_array($result)) {
			...
			$this->SetXY($X+129,$Y);
			$this->Cell(17,$height,number_format($row["pvp"], 2, ',', '.').$rowdi["simbolo"],'B',0,'R');
			$this->SetXY($X+146,$Y);
			$this->Cell(15,$height,number_format($row["descuento"], 2, ',', '.')."%",'RB',0);
			$this->SetXY($X+161,$Y);
			$this->Cell(19,$height,number_format($row["total"], 2, ',', '.').$rowdi["simbolo"],'RB',1,'R');
...
#		$this->Cell(180,5,'','LBR',1);
		...
		if ($rowcabezera["id_divisa"] != $rowd["id"]){
			...
			$div_importe = $rowcabezera["subtotal"] * $rowdiv["canvi"];
			$div_decuento = ($rowcabezera["subtotal"]-$rowcabezera["subtotaldescuento"]) * $rowdiv["canvi"];
			$div_subtotal = $rowcabezera["subtotaldescuento"] * $rowdiv["canvi"];
			$div_total = $rowcabezera["total"] * $rowdiv["canvi"];

		
			$this->Cell(15,5,"1".$rowd["simbolo"]."=".number_format($rowdiv["canvi"], 2, ',', '.').$rowdi["simbolo"],'LBR',0);
