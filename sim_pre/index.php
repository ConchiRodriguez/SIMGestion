...
$userid = 0;
if ($soption == 666) {
	setcookie("musername", "", time()-1, "/sim_pre");
	setcookie("mpassword", "", time()-1, "/sim_pre");
...
if ($option == 667) {
	setcookie("idioma", "".$_GET["i"]."", time()+31536000*$cookiestime, "/sim_pre", $domain);
	header("Location: ".$urlmgestion);
}
	...
	if ($_POST["user"] != "") {
		...
		if ($row["total"] == 1) {
			...
			if (crypt($_POST["pass"], $row["pass"]) == $row["pass"]){
				...
				$userid = $row["id"];
				$sgm = $row["sgm"];
				setcookie("musername", $_POST["user"], time()+60*$cookiestime, "/sim_pre");
				setcookie("mpassword", $row["pass"], time()+60*$cookiestime, "/sim_pre");
	...
	if ($_GET["idp"] != "") {
		...
		while ($row = mysqli_fetch_array($result)){
			if ($_GET["idp"] == $row["pass"]){
				...
				$userid = $row["id"];
				$sgm = $row["sgm"];
				setcookie("musername", $row["usuario"], time()+60*$cookiestime, "/sim_pre");
				setcookie("mpassword", $row["pass"], time()+60*$cookiestime, "/sim_pre");
