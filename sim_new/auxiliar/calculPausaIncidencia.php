...
function calculPausaIncidencia ($id_incidencia){
	...
	$sql = "select id_usuario_registro,pausada,id from sim_incidencias where id_incidencia=".$id_incidencia." and visible=1 order by fecha_inicio";
	$result = mysqli_query($dbhandle,$sql);
	while ($row = mysqli_fetch_array($result)){
		if ($row["id_usuario_registro"] > 0) {
			$sqlu = "select id from sim_users where id=".$row["id_usuario_registro"]." and validado=1 and activo=1 and sgm=1";
