	...
	global $db,$dbhandle,$Contrasenas,$Contrato,$Aplicacion,$Buscar,$Acceso,$Usuario,$Contrasena,$Descripcion,$userid,$ssoption,$simclau,$ErrorPass,$PassIncorrecto,$id_aplicacion,$id_contrato;
		if ($ssoption == 1) {
			if (($_POST["pass1"] != $_POST["pass2"]) OR $_POST["pass1"] == "") {
				...
				insertFunction ("sim_contrasenas",$camposInsert,$datosInsert);

				$sqlcc = "select id from sim_contrasenas where id_contrato=".$_POST["id_contrato"]." and id_aplicacion=".$_POST["id_aplicacion"]." and acceso='".$_POST["acceso"]."' order by id desc";
		...
		if ($ssoption == 2) {
			if ($_POST["passold"] != "") {
				...
				$sqlx = "select id from sim_contrasenas WHERE id=".$_GET["id_con"]." and pass='".$clau."'";
				...
				if ($rowx){
					if (($_POST["pass1"] != "") and ($_POST["pass1"] == $_POST["pass2"])) {
						...
						updateFunction ("sim_contrasenas",$_GET["id_con"],$camposUpdate,$datosUpdate);
					} else {
						echo mensaje_error($PassIncorrecto);
					}
				} else {
		...
		if (($id_contrato != 0) or ($id_aplicacion != 0) or ($_GET["id"] > 0)) {
			echo "<tr style=\"background-color:silver\">";
				...
				echo "<th></th>";
			echo "</tr>";
			$sqlcc = "select * from sim_contrasenas where visible=1";
		}
		...
		if ($_GET["id"] > 0) {$sqlcc .= " and id_contrato in (select id from sim_contratos where id_cliente=".$_GET["id"].")";}
...
#		echo $sqlcc;
		...
		while ($rowcc = mysqli_fetch_array($resultcc)){
			...
			echo "<tr>";
				...
				$sql = "select id,id_cliente_final,descripcion from sim_contratos where visible=1 and id=".$rowcc["id_contrato"];
				$result = mysqli_query($dbhandle,convertSQL($sql));
				$row = mysqli_fetch_array($result);
				$sqlc = "select nombre,apellido1,apellido2 from sim_clientes where id=".$row["id_cliente_final"];
				$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
				$rowc = mysqli_fetch_array($resultc);
				echo "<td nowrap><a href=\"index.php?op=1011&sop=100&id=".$row["id"]."\">".$rowc["nombre"]." ".$rowc["apellido1"]." ".$rowc["apellido2"]." - ".$row["descripcion"]."</a></td>";
				$sqlam = "select aplicacion from sim_contrasenas_apliciones where visible=1 and id=".$rowcc["id_aplicacion"];
