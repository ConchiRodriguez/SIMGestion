...
#	print_r($datosInsert);
	if (!array_key_exists('numero', $datosInsert)){
		$sqltipo = "select * from sim_factura_tipos where v_recibos=1";
		$resulttipo = mysqli_query($dbhandle,convertSQL($sqltipo));
		$rowtipo = mysqli_fetch_array($resulttipo);
		$sqlxx = "select * from sim_factura where visible=1 AND tipo=".$rowtipo["id"]." order by numero desc";
		...
		$sqltipo = "select * from sim_factura_tipos where id=".$tipo;
		$resulttipo = mysqli_query($dbhandle,convertSQL($sqltipo));
		$rowtipo = mysqli_fetch_array($resulttipo);
		$v_recibos=$rowtipo["v_recibos"];
	}
	$sqlcc = "select count(*) as total from sim_factura where visible=1 and numero=".$numero." and version=".$version." and tipo=".$tipo;
	$resultcc = mysqli_query($dbhandle,convertSQL($sqlcc));
	$rowcc = mysqli_fetch_array($resultcc);
	if ($rowcc["total"] == 0){
		$sql = "select * from sim_clientes where id=".$datosInsert['id_cliente'];
		$result = mysqli_query($dbhandle,convertSQL($sql));
		$row = mysqli_fetch_array($result);
		$sqlta = "select * from sim_tarifas where id=(select id_tarifa from sim_tarifas_clients where id_cliente=".$row["id"]." and predeterminado=1)";
		$resultta = mysqli_query($dbhandle,convertSQL($sqlta));
		$rowta = mysqli_fetch_array($resultta);
		$sqltipos = "select * from sim_factura_tipos where id=".$tipo;
		...
		if ($row["id_direccion_envio"] <> 0) {
			$sql3 = "select * from sim_clientes_envios where id=".$row["id_direccion_envio"];
		...
		$sql2 = "select * from sim_dades_origen_factura";
		$result2 = mysqli_query($dbhandle,convertSQL($sql2));
		$row2 = mysqli_fetch_array($result2);
		$sqld = "select * from sim_divisas where predefinido=1";
		$resultd = mysqli_query($dbhandle,convertSQL($sqld));
		$rowd = mysqli_fetch_array($resultd);
		$sqlc = "select * from sim_clientes_contactos where pred=1 and id_client=".$datosInsert['id_cliente'];
		$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
		$rowc = mysqli_fetch_array($resultc);
		if (!array_key_exists('id_dades_origen_factura_iban', $datosInsert)){
			$sqldo = "select * from sim_dades_origen_factura_iban where predefinido=1";
		...
		$camposInsert = "numero,iva,version,numero_rfq,numero_cliente,fecha,fecha_prevision,fecha_entrega,fecha_vencimiento,tipo,subtipo,nombre,nif,direccion,poblacion,cp,provincia,id_pais,mail,telefono,id_cliente,id_usuario,id_divisa,div_canvi,cnombre,cmail,ctelefono,id_contrato,id_licencia,id_dades_origen_factura_iban,notas,total_forzado,recibos,confirmada,confirmada_cliente,descuento";
		$datosInsert2 = array($numero,$row2["iva"],$version,$datosInsert["numero_rfq"],$datosInsert["numero_cliente"],$fecha,$fecha_prevision,$fecha_entrega,$fecha_vencimiento,$tipo,$datosInsert['subtipo'],$row["nombre"]." ".$row["cognom1"]." ".$row["cognom2"],$row["nif"],$row["direccion"],$row["poblacion"],$row["cp"],$row["provincia"],$row["id_pais"],$row["mail"],$row["telefono"],$datosInsert['id_cliente'],$userid,$rowd["id"],$rowd["canvi"],$rowc["nombre"]." ".$rowc["apellido1"]." ".$rowc["apellido2"],$rowc["mail"],$rowc["telefono"],$datosInsert['id_contrato'],$datosInsert['id_licencia'],$id_dades_origen_factura_iban,$datosInsert['notas'],$datosInsert['total_forzado'],$datosInsert['recibos'],$datosInsert['confirmada'],$datosInsert['confirmada_cliente'],$rowta["porcentage"]);
		insertFunction ("sim_factura",$camposInsert,$datosInsert2);
