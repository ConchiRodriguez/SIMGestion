...
function modificarContrasenya($url_volver){
	...
	if ($ssoption == 1) {
		...
		$sql = "select Count(*) AS total from sim_contrasenas WHERE id='".$_GET["id_con"]."' and pass='".$cadena."'";
	...
	$sqlc = "select id_contrato,id_aplicacion from sim_contrasenas where id=".$_GET["id_con"];
