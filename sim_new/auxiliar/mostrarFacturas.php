...
function mostrarFacturas($row){
	...
	$sqltipos = "select * from sim_factura_tipos where id=".$row["tipo"];
...
#Por Articulo
	if (($soption == 200) and ($_POST["articulo"] != '')){
		$sqlcc = "select id from sim_cuerpo where idfactura=".$row["id"]." and nombre like '%".$_POST["articulo"]."%'";
...
#Por Codigo
	if (($soption == 200) and ($_POST["codigo"] != '')){
		$sqlcc = "select id from sim_cuerpo where idfactura=".$row["id"]." and codigo like '%".$_POST["codigo"]."%'";
		$resultcc = mysqli_query($dbhandle,convertSQL($sqlcc));
		$rowcc = mysqli_fetch_array($resultcc);
		if ($rowcc){ $ver = 1;  } else { $ver = 0; }
	}
...
#pedido proveedor
		...
		if ($rowtipos["tipo_ot"] == 1) {
			...
			$sqlc = "select fecha_prevision from sim_cuerpo where idfactura=".$row["id"]." and id_estado<>-1 and facturado<>1 order by  fecha_prevision";
		...
		if ($rowtipos["presu"] == 1) {
			...
			$fecha_aviso2 = $fecha_aviso;
			$sqlc = "select fecha_prevision from sim_cuerpo where idfactura=".$row["id"]."";
			$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
		...
		if ($rowtipos["v_recibos"] == 1) {
			...
			if ($row["cobrada"] == 1) {
				...
				$sqld = "select count(*) as total from sim_recibos where visible=1 and cobrada=0 and id_tipo_pago=4 AND id_factura =".$row["id"];
				$resultd = mysqli_query($dbhandle,convertSQL($sqld));
				$rowd = mysqli_fetch_array($resultd);
				if ($rowd["total"] > 0) { $color = "#0000FF"; }
				$sqlcalc = "select total from sim_recibos where visible=1 and cobrada=1 AND id_factura=".$row["id"];
		...
		echo "<tr style=\"background-color:".$color."\">";
			...
			if (($rowtipos["v_recibos"] == 0) and ($rowtipos["v_fecha_prevision"] == 0) and ($rowtipos["v_rfq"] == 0) and ($rowtipos["presu"] == 0) and ($rowtipos["dias"] == 0) and ($rowtipos["v_fecha_vencimiento"] == 0) and ($rowtipos["v_numero_cliente"] == 0) and ($rowtipos["tipo_ot"] == 0)) {
				$sqlca = "select count(*) as total from sim_files where id_elemento in (select id from sim_cuerpo where idfactura=".$row["id"].")";
...
############ CALCULOS SOBRE LAS FECHAS OFICIALES
				...
				if ($date["year"]."-".$date["mon"]."-".$date["mday"] < $row["fecha"]) {
					...
					$fecha_proxima = $row["fecha"];
					$multiplica = 0;
					$sql00 = "select fecha from sim_facturas_relaciones where id_plantilla=".$row["id"]." order by fecha";
				...
				if ($rowtipos["tpv"] == 0) {
					...
					if ($rowtipos["v_subtipos"] == 1) {
						...
						echo "<select style=\"width:100%\" name=\"subtipo\" disabled>";
							...
							$sqlsss = "select id,subtipo from sim_factura_subtipos where visible=1 and id_tipo=".$row["tipo"]." order by subtipo";
				...
				$sqldi = "select id,abrev from sim_divisas where id=".$row["id_divisa"];
				...
				if ($rowtipos["v_recibos"] == 1)  {
					$sqlr = "select SUM(total) as total from sim_recibos where visible=1 and id_factura=".$row["id"]." order by numero desc, numero_serie desc";
...
#		echo "</tr></table>";
		...
		if ($_GET["filtra"] == 1){
			...
			$sqlxx = "select count(*) as total from sim_cuerpo where idfactura=".$id_fac."";
				...
				echo "<tr>";
					echo "<td colspan=\"17\">";
						echo "<table cellpadding=\"1\" cellspacing=\"0\" width=\"100%\">";
							...
							echo "</tr>";
							$sqlxx = "select * from sim_cuerpo where idfactura=".$id_fac."";
							if (($_GET["hist"] == 0) or ($_POST["hist"] == 0)) { $sqlxx = $sqlxx." and id_estado<>-1 and facturado<>1";}
							if ($_POST["data_desde1"] != 0) { $sqlxx = $sqlxx." AND fecha_prevision>='".cambiarFormatoFechaYMD($_POST["data_desde1"])."'"; }
							if ($_POST["data_fins1"] != 0) { $sqlxx = $sqlxx." AND fecha_prevision<='".cambiarFormatoFechaYMD($_POST["data_fins1"])."'"; }
							if ($_POST["data_desde2"] != 0) { $sqlxx = $sqlxx." AND fecha_prevision>='".cambiarFormatoFechaYMD($_POST["data_desde2"])."'"; }
							...
							while ($rowxx = mysqli_fetch_array($resultxx)) {
								...
								echo "<tr style=\"background-color : ".$color.";\">";
									...
									if ($row["pvd"] == 0){
										$sqla = "select preu_cost from sim_articles_costos where visible=1 and id_cuerpo=".$rowxx["id"]." and aprovat=1";
