...
{
	global $db,$dbhandle;
	$sql = "select count(*) as total from sim_cuerpo where id=".$id_linia."";
	$result = mysqli_query($dbhandle,convertSQL($sql));
	$row = mysqli_fetch_array($result);
	$sql2 = "select count(*) as total2 from sim_cuerpo where id=".$id_linia." and aprovat=1";
	$result2 = mysqli_query($dbhandle,convertSQL($sql2));
	$row2 = mysqli_fetch_array($result2);
	if ($row["total"] == $row2["total2"]){
		$sqlc = "update sim_factura set ";
