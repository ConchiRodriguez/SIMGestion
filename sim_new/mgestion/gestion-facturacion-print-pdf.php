...
class PDF extends FPDF
{
	...
	function factura_print($id,$tipo){
		...
		$sqlcabezera = "select * from sim_factura where id=".$id;
		$resultcabezera = mysqli_query($dbhandle,convertSQL($sqlcabezera));
		$rowcabezera = mysqli_fetch_array($resultcabezera);
	
		$sqlele2 = "select * from sim_factura_dades_origen_iban where id=".$rowcabezera["id_dades_origen_factura_iban"];
		$resultele2 = mysqli_query($dbhandle,convertSQL($sqlele2));
		$rowele2 = mysqli_fetch_array($resultele2);

		$sqldi = "select * from sim_divisas where id=".$rowcabezera["id_divisa"];
		$resultdi = mysqli_query($dbhandle,convertSQL($sqldi));
		$rowdi = mysqli_fetch_array($resultdi);

		$sqldiv = "select * from sim_divisas_mod_cambio where id_divisa=".$rowdi["id"]." order by fecha desc";
		...
		$sqlx = "select * from sim_factura_tipos where id=".$rowcabezera["id_tipo"];
		$resultx = mysqli_query($dbhandle,convertSQL($sqlx));
		$rowx = mysqli_fetch_array($resultx);
		if ($idioma == "es"){
			$fac_tipo = $rowx["id_tipo"];
		} else {
			$sqlid = "select * from sim_idiomas where idioma='".$idioma."'";
			$resultid = mysqli_query($dbhandle,convertSQL($sqlid));
			$rowid = mysqli_fetch_array($resultid);
			$sqli = "select * from sim_factura_tipos_idiomas where id_tipo=".$rowx["id"]." and id_idioma=".$rowid["id"];
			$resulti = mysqli_query($dbhandle,convertSQL($sqli));
			$rowi = mysqli_fetch_array($resulti);
			$fac_tipo = $rowi["id_tipo"];
		}
		...
		$unidades2 = 0;
		$lineas = 0;
		$sql = "select * from sim_factura_cuerpo where id_factura=".$id." order by linea";
...
#		$this->Cell(180,5,'','LBR',1);
		...
		if ($rowcabezera["id_divisa"] != $rowd["id"]){
			$this->Cell(15,5,$cambio.": 1".$rowd["simbolo"]."=".number_format($rowdi["cambio"], 2, ',', '.').$rowdi["simbolo"],'LTR',0);
			$this->Cell(25,5,"".$importe."",'LTR',0);
			$this->Cell(35,5,"".$descuento."",'LTR',0);
			$this->Cell(25,5,"".$subtotal."",'LTR',0);
			$this->Cell(25,5,"IVA(".number_format($rowcabezera["iva"],0)."%)",'LTR',0);
			...
			$div_importe = $rowcabezera["subtotal"] * $rowdi["cambio"];
			$div_decuento = ($rowcabezera["subtotal"]-$rowcabezera["subtotaldescuento"]) * $rowdi["cambio"];
			$div_subtotal = $rowcabezera["subtotaldescuento"] * $rowdi["cambio"];
			$div_total = $rowcabezera["total"] * $rowdi["cambio"];
...
#	$pdf->AddFont('codi_barres','','../../archivos_comunes/font/codi_barres.php');
	...
	if ($_POST["data_desde"] == ""){
		$pdf->AddPage();
		$pdf->factura_print($_GET["id"],$_POST["id_tipo"]);
	} else {
		$sql = "select * from sim_factura where tipo=".$_POST["id_tipo"]." and fecha_prevision between '".cambiarFormatoFechaYMD($_POST["data_desde"])."' and '".cambiarFormatoFechaYMD($_POST["data_fins"])."' and visible=1";
		$result = mysqli_query($dbhandle,convertSQL($sql));
		while ($row = mysqli_fetch_array($result)){
			$pdf->AddPage();
			$pdf->factura_print($row["id"],$_POST["id_tipo"]);
		}
	}
