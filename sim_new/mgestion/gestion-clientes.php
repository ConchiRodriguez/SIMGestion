	...
	if (($soption == 0) or ($soption == 200)) {
		...
		if ($soption == 0) {
			...
			if ($_GET["filtro"] > 0) {
				...
				if ($_GET["filtro"] == 2) {
					$sql = "select * from sim_clientes_busquedas where id=".$_GET["id"];
					...
					$tipos = str_split(str_replace(",","",$tipo));
					$sectores = str_split(str_replace(",","",$sector));
					$origenes = str_split(str_replace(",","",$origen));
					$paises = str_split(str_replace(",","",$pais));
					$comunidades = str_split(str_replace(",","",$comunidad));
					$provincias = str_split(str_replace(",","",$provincia));
					$regiones = str_split(str_replace(",","",$region));
					$chars = str_split($row["letras"]);
				}
#				echo $tipo."- ".$sector."- ".$origen."- ".$pais."- ".$comunidad."- ".$provincia."- ".$region."- ".var_dump($chars);
			...
			echo "<br>";
				echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
					...
					echo "<tr>";
						...
						echo "<td style=\"color:black;text-align: leftt;width:160px\"><input type=\"Text\" name=\"likenombre\" value=\"".$likenombre."\" style=\"width:300px\"></td>";
		...
		if (($soption == 0) or (($soption == 200) and ($_GET["filtro"] > 0) and ((isset($tipo)) or (isset($sector)) or (isset($origen)) or (isset($pais)) or (isset($comunidad)) or (isset($provincia)) or (isset($region)) or ($likenombre != "") or ($ok == 1)))) {
			$ver = true;
			if ($_GET["id_tipo"] != 0){
			...
			for ($i = 48; $i <= 90; $i++) {
				if ((($i >= 48) and ($i <= 57)) or (($i >= 65) and ($i <= 90))) {
					...
					if ($likenombre != "") { $sql = $sql." and (nombre like '%".$likenombre."%' or apellido1 like '%".$likenombre."%' or apellido2 like '%".$likenombre."%')";}
...
#					echo $sql."<br>";
					...
					while ($row = mysqli_fetch_array($result)) {
						...
						if ($likenombre != "") { $sqlsub = $sqlsub." and (nombre like '%".$likenombre."%' or apellido1 like '%".$likenombre."%' or apellido2 like '%".$likenombre."%')";}
...
#						echo $sqlsub."<br>";
						...
						while ($rowsub = mysqli_fetch_array($resultsub)) {
							...
							if (($ver == true) and (($_GET["id_classificacio"] != 0) OR ($id_classificacio != 0))) {
								if (($_GET["id_classificacio"] != "") OR ($id_classificacio != 0))  {
									...
									$sqlcxx = "select count(*) as total  from sim_clientes_classificacio where id_client=".$rowsub["id"]." and visible=1 and id_clasificacio_tipus in (".$id_classificacio.")";
									$resultcxx = mysqli_query($dbhandle,convertSQL($sqlcxx));
									$rowcxx = mysqli_fetch_array($resultcxx);
									if ($rowcxx["total"] <= 0) { $ver = false; }
									$sqlcxx2 = "select id from sim_clientes_classificacio_tipus where id_origen in (".$id_classificacio.")";
									$resultcxx2 = mysqli_query($dbhandle,convertSQL($sqlcxx2));
									while ($rowcxx2 = mysqli_fetch_array($resultcxx2)) {
										$sqlcxx3 = "select count(*) as total  from sim_clientes_classificacio where id_client=".$rowsub["id"]." and visible=1 and id_clasificacio_tipus=".$rowcxx2["id"];
										$resultcxx3 = mysqli_query($dbhandle,convertSQL($sqlcxx3));
										$rowcxx3 = mysqli_fetch_array($resultcxx3);
										if ($rowcxx3["total"] > 0) { $ver = true; }
										$sqlcxx4 = "select id from sim_clientes_classificacio_tipus where id_origen=".$rowcxx2["id"];
										$resultcxx4 = mysqli_query($dbhandle,convertSQL($sqlcxx4));
										while ($rowcxx4 = mysqli_fetch_array($resultcxx4)) {
											$sqlcxx5 = "select count(*) as total  from sim_clientes_classificacio where id_client=".$rowsub["id"]." and visible=1 and id_clasificacio_tipus=".$rowcxx4["id"];
											$resultcxx5 = mysqli_query($dbhandle,convertSQL($sqlcxx5));
											$rowcxx5 = mysqli_fetch_array($resultcxx5);
											if ($rowcxx5["total"] > 0) { $ver = true; }
										}
	...
	if ($soption == 1) {
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			if ($_GET["filtro"] == "") {
				...
				for ($i = 48; $i <= 90; $i++) {
					if ((($i >= 48) and ($i <= 57)) or(($i >= 65) and ($i <= 90))) {
						$sqlxt = "select count(*) as total from sim_clientes_contactos where visible=1 and nombre like '".chr($i)."%'  order by nombre";
			...
			echo "<tr>";
				...
				echo "<td>";
					...
					echo "<select name=\"id_client\" style=\"width:300px\">";
						...
						$sqle = "select id,nombre,cognom1,cognom2 from sim_clientes where visible=1 order by nombre";
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
		 for ($i = 48; $i <= 90; $i++) {
			...
			$sqlxt = "select count(*) as total from sim_clientes_contactos where visible=1 and nombre like '".chr($i)."%'";
	...
	#		$sqlxt = $sqlxt." order by nombre";
			...
			$rowxt = mysqli_fetch_array($resultxt);
				if ($rowxt["total"] > 0) {
					...
					echo "</tr>";
					$cambio = 0;
					$sqlt = "select * from sim_clientes_contactos where visible=1 and nombre like '".chr($i)."%'";
					if ($_POST["id_client"] > 0){
					...
					while ($rowt = mysqli_fetch_array($result)) {
						$sqltr = "select trato from sim_clientes_tratos where id=".$rowt["id_trato"];
						$resultr = mysqli_query($dbhandle,convertSQL($sqltr));
						$rowtr = mysqli_fetch_array($resultr);
						$sqlc = "select nombre,cognom1,cognom2 from sim_clientes where id=".$rowt["id_client"];
						$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
						$rowc = mysqli_fetch_array($resultc);
						...
						echo "<tr style=\"background-color:".$color."\">";
							...
							echo "<td style=\"white-space:nowrap;\">".$rowt["cargo"]."</td>";
						echo "</tr>";
					}
				}
			}
	...
	if (($soption >= 100) and ($soption < 200) and (($_GET["id"] != 0) or ($ssoption != 0))) {
		if (($soption == 100) and ($ssoption == 1)) {
			$camposInsert="nombre,apellido1,apellido2,nif,direccion,poblacion,cp,provincia,id_pais,tipo_identificador,tipo_persona,tipo_residencia,dir3_oficina_contable,dir3_organo_gestor,dir3_unidad_tramitadora,alias";
			$datosInsert = array($_POST["nombre"],$_POST["apellido1"],$_POST["apellido2"],$_POST["nif"],$_POST["direccion"],$_POST["poblacion"],$_POST["cp"],$_POST["provincia"],$_POST["id_pais"],$_POST["tipo_identificador"],$_POST["tipo_persona"],$_POST["tipo_residencia"],$_POST["dir3_oficina_contable"],$_POST["dir3_organo_gestor"],$_POST["dir3_unidad_tramitadora"],$_POST["alias"]);
		...
		if (($soption == 100) and ($ssoption == 2)) {
			$camposUpdate = array('nombre','apellido1','apellido2','nif','direccion','poblacion','cp','provincia','id_pais','tipo_identificador','tipo_persona','tipo_residencia','dir3_oficina_contable','dir3_organo_gestor','dir3_unidad_tramitadora','alias');
			$datosUpdate = array($_POST["nombre"],$_POST["apellido1"],$_POST["apellido2"],$_POST["nif"],$_POST["direccion"],$_POST["poblacion"],$_POST["cp"],$_POST["provincia"],$_POST["id_pais"],$_POST["tipo_identificador"],$_POST["tipo_persona"],$_POST["tipo_residencia"],$_POST["dir3_oficina_contable"],$_POST["dir3_organo_gestor"],$_POST["dir3_unidad_tramitadora"],$_POST["alias"]);
			updateFunction("sim_clientes",$_GET["id"],$camposUpdate,$datosUpdate);
...
#		echo $sql;
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			echo "<tr>";
				...
				echo "<td style=\"vertical-align:top\">";
					echo "<table cellpadding=\"0\">";
						echo "<tr><td class=\"ficha\"><a href=\"index.php?op=1008&sop=155&id=".$row["id"]."\" style=\"color:white;\">".$Datos_Fiscales."</a></td></tr>";
...
#Dades administratives
	if ($soption == 105) {
		if ($ssoption == 1) {
			$camposUpdate = array('mail','telefono','telefono2','fax','web','id_idioma','id_origen','id_agrupacion','cliente_fid','cliente_vip');
			$datosUpdate = array($_POST["mail"],$_POST["telefono"],$_POST["telefono2"],$_POST["fax"],$_POST["web"],$_POST["id_idioma"],$_POST["id_origen"],$_POST["id_agrupacio"],$_POST["cliente_fid"],$_POST["cliente_vip"]);
			updateFunction("sim_clientes",$_GET["id"],$camposUpdate,$datosUpdate);
			...
			if ($_POST["id_origen_cliente"] != -1){
				$sql = "select * from sim_clientes_rel_origen where id_cliente=".$id_client." and id_origen=".$_POST["id_origen_cliente"]." and tipo_origen=1";
				$result = mysqli_query($dbhandle,convertSQL($sql));
				$row = mysqli_fetch_array($result);
				if (!$row){
			...
			if ($_POST["id_cliente_origen"] != -1){
				$sql = "select * from sim_clientes_rel_origen where id_cliente=".$id_client." and id_origen=".$_POST["id_cliente_origen"]." and tipo_origen=2";
				$result = mysqli_query($dbhandle,convertSQL($sql));
				$row = mysqli_fetch_array($result);
				if (!$row){
					$camposInsert = "id_cliente,id_origen,tipo_origen";
			...
			if ($_POST["id_contacto_origen"] != -1){
				$sql = "select * from sim_clientes_rel_origen where id_cliente=".$id_client." and id_origen=".$_POST["id_contacto_origen"]." and tipo_origen=3";
				$result = mysqli_query($dbhandle,convertSQL($sql));
				$row = mysqli_fetch_array($result);
				if (!$row){
					$camposInsert = "id_cliente,id_origen,tipo_origen";
			...
			if ($_POST["id_empleado_origen"] != -1){
				echo "zz";
				echo $sql = "select * from sim_clientes_rel_origen where id_cliente=".$id_client." and id_origen=".$_POST["id_empleado_origen"]." and tipo_origen=4";
				$result = mysqli_query($dbhandle,convertSQL($sql));
				$row = mysqli_fetch_array($result);
				if (!$row){
					echo "zz";
			...
			if ($_POST["otro_origen"] != ''){
				$sql = "select * from sim_clientes_rel_origen where id_cliente=".$id_client." and otro_origen='".$_POST["otro_origen"]."' and tipo_origen=5";
				$result = mysqli_query($dbhandle,convertSQL($sql));
				$row = mysqli_fetch_array($result);
				if (!$row){
			...
			if ($_POST["id_ubicacion_pais"] != -1){
				$sql = "select * from sim_clientes_rel_ubicacion where id_cliente=".$id_client." and id_ubicacion=".$_POST["id_ubicacion_pais"]." and tipo_ubicacion=1";
				$result = mysqli_query($dbhandle,convertSQL($sql));
				$row = mysqli_fetch_array($result);
				if (!$row){
			...
			if ($_POST["id_ubicacion_comunidad"] != -1){
				$sql = "select * from sim_clientes_rel_ubicacion where id_cliente=".$id_client." and id_ubicacion=".$_POST["id_ubicacion_comunidad"]." and tipo_ubicacion=2";
				$result = mysqli_query($dbhandle,convertSQL($sql));
				$row = mysqli_fetch_array($result);
				if (!$row){
					$camposInsert = "id_cliente,id_ubicacion,tipo_ubicacion";
			...
			if ($_POST["id_ubicacion_provincia"] != -1){
				$sql = "select * from sim_clientes_rel_ubicacion where id_cliente=".$id_client." and id_ubicacion=".$_POST["id_ubicacion_provincia"]." and tipo_ubicacion=3";
				$result = mysqli_query($dbhandle,convertSQL($sql));
				$row = mysqli_fetch_array($result);
				if (!$row){
					$camposInsert = "id_cliente,id_ubicacion,tipo_ubicacion";
			...
			if ($_POST["id_ubicacion_region"] != -1){
				$sql = "select * from sim_clientes_rel_ubicacion where id_cliente=".$id_client." and id_ubicacion=".$_POST["id_ubicacion_region"]." and tipo_ubicacion=4";
				$result = mysqli_query($dbhandle,convertSQL($sql));
				$row = mysqli_fetch_array($result);
				if (!$row){
					$camposInsert = "id_cliente,id_ubicacion,tipo_ubicacion";
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\" style=\"width:100%;\">";
			echo "<tr>";
				echo "<td style=\"width:100%;\" colspan=\"4\"><input type=\"Submit\" value=\"".$Guardar."\" style=\"height:30px;width:100%;\"></td>";
				...
				echo "<td style=\"vertical-align:top;width:20%;\">";
					echo "<table class=\"formclient\">";
						...
						echo "<tr><th class=\"formclient\">".$Idioma."</th><td>";
							...
							echo "<option value=\"0\">-</option>";
							$sqli = "select id,idioma from sim_idiomas where visible=1 order by idioma";
					...
					echo "<table class=\"formclient\">";
						...
						echo "<tr><th class=\"formclient2\">".$Delegacion." ".$Central."</th><td>";
							echo "<select name=\"id_origen\" style=\"width:100%\"><option value=\"0\">-</option>";
								$sqlo = "select id,nombre from sim_clientes where visible=1 order by nombre";
								$resulto = mysqli_query($dbhandle,convertSQL($sqlo));
								while ($rowo = mysqli_fetch_array($resulto)) {
									if ($rowo["id"] != $row["id"]) {
						...
						echo "<tr><th class=\"formclient2\">".$Contacto." ".$Principal."</th><td>";
							echo "<select name=\"id_agrupacion\" style=\"width:100%\"><option value=\"0\">-</option>";
								$sqlo = "select id,nombre from sim_clientes where visible=1 order by nombre";
								$resulto = mysqli_query($dbhandle,convertSQL($sqlo));
								while ($rowo = mysqli_fetch_array($resulto)) {
									if ($rowo["id"] != $row["id"]) {
					...
					echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
						...
						echo "<tr>";
							...
							echo "<td style=\"width:16%;\"><select style=\"width:100%;\" name=\"id_contacto_origen\">";
								echo "<option value=\"-1\">Indeterminado</option>";
								$sql3 = "select id,nombre,apellido1,apellido2 from sim_clientes_contactos order by nombre,apellido1,apellido2";
							...
							echo "<td style=\"width:16%;\"><select style=\"width:100%;\" name=\"id_empleado_origen\">";
								echo "<option value=\"-1\">Indeterminado</option>";
								$sql4 = "select id,nombre from sim_rrhh_empleado order by nombre";
						...
						echo "<tr>";
							...
							echo "<td style=\"text-align:center;vertical-align:top;\">";
								...
								while ($rowori3 = mysqli_fetch_array($resultori3)) {
									$sql3 = "select id,nombre,apellido1,apellido2 from sim_clientes_contactos where id=".$rowori3["id_origen"];
									$result3 = mysqli_query($dbhandle,convertSQL($sql3));
							...
							echo "<td style=\"text-align:center;vertical-align:top;\">";
								...
								while ($rowori4 = mysqli_fetch_array($resultori4)) {
									$sql4 = "select id,nombre from sim_rrhh_empleado where id=".$rowori4["id_origen"];
									$result4 = mysqli_query($dbhandle,convertSQL($sql4));
...
#Dades facturació
	if ($soption == 110) {
		if ($ssoption == 1) {
			$camposUpdate = array("cuenta_bancaria","dia_facturacion","dia_recibo","unidad_vencimiento","vencimiento","id_medio_facturacion","destino_facturacion","id_formato_facturacion","id_contacto_facturacion","notas_facturacion");
			$datosUpdate = array($_POST["cuenta_bancaria"],$_POST["dia_facturacion"],$_POST["dia_recibo"],$_POST["unidad_vencimiento"],$_POST["vencimiento"],$_POST["id_medio_facturacion"],$_POST["destino_facturacion"],$_POST["id_formato_facturacion"],$_POST["id_contacto_facturacion"],$_POST["notas_facturacion"]);
			updateFunction ("sim_clientes",$_GET["id"],$camposUpdate,$datosUpdate);
			if ($_POST["dia"] != 0){
				$camposInsert = "dia,id_cliente";
				$datosInsert = array($_POST["dia"],$_GET["id"]);
				insertFunction ("sim_clientes_dias_vencimiento",$camposInsert,$datosInsert);
			}
		}
		if ($ssoption == 2) {
			deleteFunction ("sim_clientes_dias_vencimiento",$_GET["id_dia"]);
		}
		if ($ssoption == 4) {
			$sqltc = "select count(*) as total from sim_clientes_tarifas where predeterminado=1";
			...
			if ($rowtc["total"]== 0){
				...
				insertFunction ("sim_clientes_tarifas",$camposInsert,$datosInsert);
			} else {
		...
		if ($ssoption == 5) {
			$sql = "update sim_clientes_tarifas set ";
		...
		if ($ssoption == 6) {
			deleteFunction ("sim_clientes_tarifas",$_GET["id_tarifa"]);
		}

		$sql = "select dia_facturacion,dia_recibo,unidad_vencimiento,vencimiento,cuenta_bancaria,id_medio_facturacion,destino_facturacion,id_formato_facturacion,id_contacto_facturacion,notas_facturacion from sim_clientes where id=".$_GET["id"];
		$result = mysqli_query($dbhandle,convertSQL($sql));
		$row = mysqli_fetch_array($result);
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			echo "<tr><th>".$Datos_Facturacion." :</th></tr>";
			echo "<form action=\"index.php?op=1008&sop=110&ssop=1&id=".$_GET["id"]."\" method=\"post\">";
			echo "<tr><td style=\"width:45%;vertical-align:top;\">";
				echo "<table style=\"width:100%\">";
					echo "<tr>";
						echo "<th class=\"formclient\">IBAN</td>";
						echo "<td class=\"formclient2\"><input type=\"Text\" name=\"cuenta_bancaria\" style=\"width:100%\" value=\"".$row["cuenta_bancaria"]."\"></td>";
					echo "</tr><tr>";
					...
					echo "</tr><tr>";
						...
						echo "<td class=\"formclient2\"><input type=\"numbre\" name=\"unidad_vencimiento\" style=\"width:50px\" value=\"".$row["unidad_vencimiento"]."\">";
					...
					echo "</tr><tr>";
						...
						echo "</select></td>";
					echo "</tr>";
					$sqldv = "select dia,id from sim_clientes_dias_vencimiento where id_cliente=".$_GET["id"];
					$resultdv = mysqli_query($dbhandle,convertSQL($sqldv));
					while ($rowdv = mysqli_fetch_array($resultdv)){
						echo "<tr>";
							echo "<th class=\"formclient\"><a href=\"index.php?op=1008&sop=110&ssop=2&id=".$_GET["id"]."&id_dia=".$rowdv["id"]."\"><img src=\"mgestion/pics/icons-mini/page_white_delete.png\" alt=\"Eliminar\" border=\"0\"></a></td>";
							echo "<td class=\"formclient2\">".$rowdv["dia"]."</td>";
						echo "</tr>";
					}
				echo "</table>";
			echo "</td>";
			...
			echo "<td style=\"width:45%;vertical-align:top;\">";
				echo "<table style=\"width:100%\">";
					echo "<tr>";
						echo "<th class=\"formclient\">".$Medio." ".$Comunicacion."</td>";
						echo "<td class=\"formclient2\">";
					...
					echo "</tr><tr>";
						...
						echo "<td class=\"formclient2\">";
							echo "<select name=\"id_contacto_facturacion\" style=\"width:100%\">";
								echo "<option value=\"0\">-</option>";
								$sql3 = "select id,nombre,apellido1,apellido2 from sim_clientes_contactos where visible=1 and id_cliente=".$_GET["id"]." order by nombre,apellido1,apellido2";
								$result3 = mysqli_query($dbhandle,convertSQL($sql3));
								while ($row3 = mysqli_fetch_array($result3)) {
			...
			echo "</td></tr>";
			echo "<tr>";
				echo "<td colspan=\"3\"><input type=\"submit\" value=\"".$Guardar."\" style=\"width:100%\"></td>";
			echo "</tr>";
			echo "</form>";
			echo "</tr><td>&nbsp;</td><tr>";
		echo "</table>";
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "<tr>";
				...
				echo "<td><select style=\"width:300px\" name=\"id_tarifa\">";
					$sqlt = "select id,nombre from sim_tarifas where visible=1";
					$resultt = mysqli_query($dbhandle,convertSQL($sqlt));
					while ($rowt = mysqli_fetch_array($resultt)){
						echo "<option value=\"".$rowt["id"]."\">".$rowt["nombre"]."</option>";
					}
				...
				echo "</form>";
			echo "</tr>";
			$sqltc = "select id,predeterminado,id_tarifa from sim_clientes_tarifas where id_cliente=".$row["id"]."";
			$resulttc = mysqli_query($dbhandle,convertSQL($sqltc));
				...
				echo "<tr style=\"background-color : ".$color."\">";
					echo "<td>";
						echo "<table style=\"width:100%\">";
							...
							echo "</tr>";
						echo "</table>";
					$sqlt = "select nombre from sim_tarifas where visible=1 and id=".$rowtc["id_tarifa"]."";
					$resultt = mysqli_query($dbhandle,convertSQL($sqlt));
...
#Contactes
	if ($soption == 120) {
		if ($ssoption == 1){
			$camposInsert = "id_trato,nombre,apellido1,apellido2,cargo,departamento,telefono,fax,mail,mobil,notas,id_cliente,id_idioma";
			$datosInsert = array($_POST["id_trato"],comillas($_POST["nombre"]),comillas($_POST["apellido1"]),comillas($_POST["apellido2"]),comillas($_POST["cargo"]),comillas($_POST["departamento"]),$_POST["telefono"],$_POST["fax"],$_POST["mail"],$_POST["mobil"],comillas($_POST["notas"]),$_GET["id"],$_POST["id_idioma"]);
			insertFunction ("sim_clientes_contactos",$camposInsert,$datosInsert);
		}
		if ($ssoption == 2){
			$camposUpdate = array("id_trato","nombre","apellido1","apellido2","cargo","departamento","telefono","fax","mail","mobil","notas","id_idioma");
			$datosUpdate = array($_POST["id_trato"],comillas($_POST["nombre"]),comillas($_POST["apellido1"]),comillas($_POST["apellido2"]),comillas($_POST["cargo"]),comillas($_POST["departamento"]),$_POST["telefono"],$_POST["fax"],$_POST["mail"],$_POST["mobil"],comillas($_POST["notas"]),$_POST["id_idioma"]);
			updateFunction ("sim_clientes_contactos",$_GET["id_contacto"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 3){
			updateFunction ("sim_clientes_contactos",$_GET["id_contacto"],array("visible"),array("0"));
		}
		if ($ssoption == 4) {
			$sql = "update sim_clientes_contactos set ";
			$sql = $sql."pred = 0";
			$sql = $sql." WHERE id<>".$_GET["id_contacto"]." and id_cliente=".$_GET["id"];
			mysqli_query($dbhandle,convertSQL($sql));
			updateFunction ("sim_clientes_contactos",$_GET["id_contacto"],array("pred"),array("1"));
		}
		if ($ssoption == 5) {
			updateFunction ("sim_clientes_contactos",$_GET["id_contacto"],array("pred"),array("0"));
		}
		echo "<h4>".$Contactos."</h4>";
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "</tr>";
			$sql = "select * from sim_clientes_contactos where id_cliente=".$_GET["id"]." and visible=1 order by nombre";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)){
				$color = "white";
				$colorl = "black";
				if ($row["pred"] == 1) { $color = "#FF4500"; $colorl = "white";}
					...
					echo "<td style=\"text-align:center;\"><a href=\"index.php?op=1008&sop=121&id=".$row["id_client"]."&id_contacto=".$row["id"]."\"><img src=\"mgestion/pics/icons-mini/page_white_delete.png\" style=\"border:0px\"></a></td>";
						$sqlt = "select trato from sim_clientes_tratos where id=".$row["id_trato"];
						$resultt = mysqli_query($dbhandle,convertSQL($sqlt));
						$rowt = mysqli_fetch_array($resultt);
						echo "<td style=\"color:".$colorl.";\">".$rowt["trato"]." ".$row["nombre"]." ".$row["apellido1"]." ".$row["apellido2"]."</td>";
						echo "<td style=\"color:".$colorl.";\">".$row["cargo"]."</td>";
						echo "<td style=\"color:".$colorl.";\">".$row["telefono"]."</td>";
						echo "<td style=\"text-align:center;color:".$colorl.";\">".$row["mobil"]."</td>";
...
#Detall de contacte
	if ($soption == 122) {
		$sqlc = "select * from sim_clientes_contactos where id=".$_GET["id_contacto"];
		$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
		$rowc = mysqli_fetch_array($resultc);
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "</tr><tr>";
				...
				echo "<td><select name=\"id_trato\" style=\"width:50px\">";
						$sqlt = "select * from sim_clientes_tratos order by trato";
						$resultt = mysqli_query($dbhandle,convertSQL($sqlt));
						while ($rowt = mysqli_fetch_array($resultt)) {
			...
			echo "</tr><tr>";
				...
				echo "<td><input type=\"text\" name=\"departamento\" style=\"width:200px\" value=\"".$rowc["departamento"]."\"></td>";
			echo "</tr><tr>";
				echo "<th>".$Cargo.": </th>";
				echo "<td><input type=\"text\" name=\"cargo\" style=\"width:200px\" value=\"".$rowc["cargo"]."\"></td>";
			echo "</tr><tr>";
				echo "<th>".$Telefono.": </th>";
			...
			echo "</tr><tr>";
				echo "<th>".$Movil.": </th>";
				echo "<td><input type=\"text\" name=\"mobil\" style=\"width:200px\" value=\"".$rowc["mobil"]."\"></td>";
			echo "</tr><tr>";
				echo "<th>".$Idioma.": </th>";
				echo "<td><select name=\"id_idioma\" style=\"width:200px\">";
				echo "<option value=\"0\">-</option>";
				$sql = "select id,idioma from sim_idiomas where visible=1";
				$result = mysqli_query($dbhandle,convertSQL($sql));
				while ($row = mysqli_fetch_array($result)){
					$sqlcl = "select id_idioma from sim_clientes where visible=1 and id=".$rowc["id_client"];
					$resultcl = mysqli_query($dbhandle,convertSQL($sqlcl));
	...
	if ($soption == 140) {
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			echo "<tr style=\"background-color:silver\">";
				...
				echo "<th></th>";
			echo "</tr>";
			$sqlcc = "select id,id_cliente,id_cliente_final,descripcion from sim_contratos where visible=1 and id_cliente=".$_GET["id"]."";
			if ($_GET["hist"] == 0) { $sqlcc .= " and activo=1";}
			$resultcc = mysqli_query($dbhandle,convertSQL($sqlcc));
			while ($rowcc = mysqli_fetch_array($resultcc)){
				echo "<tr>";
					$sql = "select nombre,cognom1,cognom2 from sim_clientes where visible=1 and id=".$rowcc["id_cliente"]."";
					$result = mysqli_query($dbhandle,convertSQL($sql));
					$row = mysqli_fetch_array($result);
					echo "<td>".$row["nombre"]." ".$row["cognom1"]." ".$row["cognom2"]."</td>";
					$sql = "select nombre,cognom1,cognom2 from sim_clientes where visible=1 and id=".$rowcc["id_cliente_final"]."";
					$result = mysqli_query($dbhandle,convertSQL($sql));
					$row = mysqli_fetch_array($result);
...
#Detall del Contracte
	if ($soption == 141) {
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
		$sqlc = "select * from sim_contratos where visible=1 and id=".$_GET["id_con"];
		$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
		$rowc = mysqli_fetch_array($resultc);
		$numeroc = $rowc["num_contrato"];
			echo "<tr><th>".$Numero." :</th><td>".$numeroc."</td></tr>";
			echo "<tr><th>".$Contrato." :</th>";
				$sql = "select nombre from sim_contratos_tipos where visible=1 and id=".$rowc["id_contrato_tipo"];
				$result = mysqli_query($dbhandle,convertSQL($sql));
				$row = mysqli_fetch_array($result);
				echo "<td>".$row["nombre"]."</td>";
			echo "</tr>";
			echo "<tr><th>".$Cliente." :</th>";
				$sqla = "select nombre,cognom1,cognom2 from sim_clientes where visible=1 and id=".$rowc["id_cliente"];
			...
			echo "<tr><th>".$Cliente." ".$Final." :</th>";
				$sqlb = "select nombre,cognom1,cognom2 from sim_clientes where visible=1 and id=".$rowc["id_cliente_final"];
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "<tr style=\"background-color:silver\">";
				...
				echo "<th></th>";
			echo "</tr>";
			$sql = "select * from sim_contratos_servicio where visible=1 and id_contrato=".$_GET["id_con"];
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)) {
				echo "<tr>";
					echo "<td>".$row["servicio"]."</td>";
				$sqls = "select nombre from sim_contratos_sla_cobertura where visible=1 and id=".$row["id_cobertura"];
				$results = mysqli_query($dbhandle,convertSQL($sqls));
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			echo "<tr>";
				echo "<td style=\"width:70%;vertical-align:top;\">";
					...
					$result = mysqli_query($dbhandle,convertSQL($sql));
					while ($row = mysqli_fetch_array($result)) {
						$sqlele = "select * from sim_files where id_tipo=".$row["id"]." and tipo_id_elemento=3 and id_elemento=".$_GET["id_con"];
...
#Usuaris
	if ($soption == 150) {
		...
		mostrarUsuari("select * from sim_users where id in (select id_usuario from sim_users_clients where id_client=".$_GET["id"].") and activo=1 order by usuario","151&id=".$_GET["id"],"152&id=".$_GET["id"],"153&id=".$_GET["id"]);
	}
...
#Facturació
	if ($soption == 160) {
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
		$sql = "select id,tipo from sim_factura_tipos where visible=1 and tpv=0 order by orden,tipo";
		$result = mysqli_query($dbhandle,convertSQL($sql));
		while ($row = mysqli_fetch_array($result)) {
			echo "<tr><th colspan=\"2\">".$row["tipo"]." :</th></tr>";
			$sqlf = "select id,numero,fecha,total,cobrada,cerrada,confirmada,confirmada_cliente from sim_cabezera where tipo=".$row["id"]." and visible=1 and id_cliente=".$_GET["id"]." order by fecha";;
			$resultf = mysqli_query($dbhandle,convertSQL($sqlf));
			$poner_cabezera = 1;
...
#Incidències
	if ($soption == 180) {
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\" style=\"background-color:silver;\">";
			...
			echo "<form action=\"index.php?op=1008&sop=180&id=".$_GET["id"]."\" method=\"post\">";
				echo "<td><select name=\"id_servicio\" style=\"width:600px\" >";
					...
					$sqlc = "select id,id_cliente,id_cliente_final from sim_contratos where visible=1 and id_cliente=".$_GET["id"]." order by num_contrato";
					$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
					while ($rowc = mysqli_fetch_array($resultc)) {
						$sqlcl = "select nombre,cognom1,cognom2 from sim_clientes where visible=1 and id=".$rowc["id_cliente"]." order by nombre";
						$resultcl = mysqli_query($dbhandle,convertSQL($sqlcl));
						$rowcl = mysqli_fetch_array($resultcl);
						$sqlcli = "select nombre,cognom1,cognom2 from sim_clientes where visible=1 and id=".$rowc["id_cliente_final"]." order by nombre";
						$resultcli = mysqli_query($dbhandle,convertSQL($sqlcli));
						$rowcli = mysqli_fetch_array($resultcli);
						$sqls = "select id,servicio from sim_contratos_servicio where visible=1 and id_contrato=".$rowc["id"];
						$results = mysqli_query($dbhandle,convertSQL($sqls));
						while ($rows = mysqli_fetch_array($results)){
				...
				echo "<td><select name=\"id_usuario\" style=\"width:150px\">";
						echo "<option value=\"0\" selected>-</option>";
					$sqlu = "select id,usuario from sim_users where validado=1 and activo=1";
					$resultu = mysqli_query($dbhandle,convertSQL($sqlu));
					while ($rowu = mysqli_fetch_array($resultu)) {
		...
		echo "<table cellpadding=\"2\" cellspacing=\"0\" class=\"lista\" style=\"background-color:silver;\">";
			...
			$sqls = "select id,id_cliente_final,descripcion from sim_contratos where id_cliente=".$_GET["id"]." order by fecha_ini desc";
			$results = mysqli_query($dbhandle,convertSQL($sqls));
			while ($rows = mysqli_fetch_array($results)){
				$sqlc = "select id,temps_resposta,servicio from sim_contratos_servicio where id_contrato=".$rows["id"];
				if ($_POST["id_servicio"] != 0) { $sqlc = $sqlc." and id=".$_POST["id_servicio"].""; }
				$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
				while ($rowc = mysqli_fetch_array($resultc)) {

					$sql = "select * from sim_incidencias where visible=1 and id_servicio=".$rowc["id"]."";
					if (($_POST["mes"] != 0) or ($_GET["m"] != 0)){
						...
						$sql = $sql." and id in (select id_incidencia from sim_incidencias_notas_desarrollo where data_registro2 between ".$data1." and ".$data2.")";
					}
					if (($_POST["id_usuario"] != 0) or ($_GET["u"] != 0)) { $sql = $sql." and id_usuario=".$_POST["id_usuario"].$_GET["u"].""; }
					$sql = $sql." order by fecha_prevision asc";
					$result = mysqli_query($dbhandle,convertSQL($sql));
					while ($row = mysqli_fetch_array($result)) {
						$sqlcli = "select nombre,cognom1,cognom2 from sim_clientes where visible=1 and id=".$rows["id_cliente_final"]." order by nombre";
						$resultcli = mysqli_query($dbhandle,convertSQL($sqlcli));
						$rowcli = mysqli_fetch_array($resultcli);

						$estado_color = "White";
						$estado_color_letras = "Black";
						$sqlu = "select usuario from sim_users where id=".$row["id_usuario_destino"];
						$resultu = mysqli_query($dbhandle,convertSQL($sqlu));
						$rowu = mysqli_fetch_array($resultu);
						$sqld = "select sum(duracion) as total from sim_incidencias where id_incidencia=".$rowi["id"]." and visible=1";
...
#Archius client
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			echo "<tr>";
				echo "<td style=\"width:400px;vertical-align:top;\">";
					...
					$result = mysqli_query($dbhandle,convertSQL($sql));
					while ($row = mysqli_fetch_array($result)) {
						$sqlele = "select * from sim_files where id_tipo=".$row["id"]." and tipo_id_elemento=2 and id_elemento=".$_GET["id"];
						$resultele = mysqli_query($dbhandle,convertSQL($sqlele));
						while ($rowele = mysqli_fetch_array($resultele)) {
							echo "<tr>";
	...
	if ($soption == 115) {
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" Style=\"width:100%\">";
			echo "<tr>";
		$x = 0;
		$sqlc = "select * from sim_clientes order by nombre";
		$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
		while ($rowc = mysqli_fetch_array($resultc)) {
			if ($x >= 4){
				echo "</tr><tr>";
...
#Adresses d'enviament
	if ($soption == 125) {
		if ($ssoption == 1) {
			...
			insertFunction ("sim_clientes_envios",$camposInsert,$datosInsert);
		}
		if ($ssoption == 2) {
			...
			updateFunction ("sim_clientes_envios",$_GET["id_envio"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 3){
			updateFunction ("sim_clientes_envios",$_GET["id_envio"],array("visible"),array("0"));
		}
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "</tr>";
			echo "<tr><td>&nbsp;</td></tr>";
		$sql = "select * from sim_clientes_envios where visible=1 and id_client=".$_GET["id"];
		$result = mysqli_query($dbhandle,convertSQL($sql));
		while ($row = mysqli_fetch_array($result)) {
			echo "<tr>";
...
#Opcions clients
	...
	if ($soption == 300){
		if ($ssoption == 1) {
			$sql = "select id from sim_clientes where visible=1";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)) {
				$sqlc = "update sim_clientes set ";
...
#echo $sqlc."<br>";
				...
				while ($row = mysqli_fetch_array($result)) {
					...
					echo "<tr>";
						...
						echo "<td><select name=\"id_tipo".$row["id"]."\" style=\"width:100px\">";
							echo "<option value=\"0\">-</option>";
							$sqlct = "select id,tipo from sim_clientes_tipos order by tipo";
							$resultct = mysqli_query($dbhandle,convertSQL($sqlct));
						...
						echo "<td><select name=\"id_grupo".$row["id"]."\" style=\"width:200px\">";
							echo "<option value=\"0\">-</option>";
							$sqlcg = "select id,grupo from sim_clientes_grupos order by grupo";
						...
						echo "<td><select name=\"id_ubicacion".$row["id"]."\" style=\"width:200px\">";
							echo "<option value=\"0\">-</option>";
							$sqlcg = "select id,ubicacion from sim_clientes_ubicacion order by ubicacion";
							$resultcg = mysqli_query($dbhandle,convertSQL($sqlcg));
							while ($rowcg = mysqli_fetch_array($resultcg)) {
	...
	if (($soption == 510) AND ($admin == true)) {
		...
		if (($ssoption == 3) AND ($admin == true)) {
			$sql = "select id from sim_clientes_rel_origen where id_origen=".$_GET["id"];
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)) {
				deleteFunction ("sim_clientes_rel_origen",$row["id"]);
			}
	...
	if (($soption == 540) AND ($admin == true)) {
		if (($ssoption == 1) AND ($admin == true)) {
			...
			insertFunction ("sim_clientes_tratos",$camposInsert,$datosInsert);
		}
		if (($ssoption == 2) AND ($admin == true)) {
			$camposUpdate = array("trato");
			$datosUpdate = array(comillas($_POST["trato"]));
			updateFunction ("sim_clientes_tratos",$_GET["id"],$camposUpdate,$datosUpdate);
		}
		if (($ssoption == 3) AND ($admin == true)) {
			$sql = "update sim_clientes set ";
			$sql = $sql."id_trato=0";
			...
			mysqli_query($dbhandle,convertSQL($sql));
			deleteFunction ("sim_clientes_tratos",$_GET["id"]);
		}
		if (($ssoption == 4) AND ($admin == true)) {
			$sql = "update sim_clientes_tratos set ";
			$sql = $sql."predeterminado = 0";
		...
		echo "<form action=\"index.php?op=1008&sop=540&ssop=1\" method=\"post\">";
			...
			echo "<tr><td>&nbsp;</td></tr>";
			echo "</form>";
			$sql = "select id,trato,predeterminado from sim_clientes_tratos order by trato";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)) {
				$color = "white";
			...
			echo "</tr><tr>";
				...
				echo "<td>";
					echo "<select style=\"width:100px\" name=\"id_tipo_factura\">";
						echo "<option value=\"0\">-</option>";
						$sqlt = "select id,tipo from sim_factura_tipos order by tipo";
						$resultt = mysqli_query($dbhandle,convertSQL($sqlt));
						while ($rowt = mysqli_fetch_array($resultt)) {
	...
	if ($soption == 570) {
		if ($ssoption == 1){
			...
			insertFunction ("sim_cerques",$camposInsert,$datosInsert);
		}
		if ($ssoption == 2){
			$camposUpdate = array("nombre");
			$datosUpdate = array(comillas($_POST["nombre"]));
			updateFunction ("sim_cerques",$_GET["id"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 3){
			deleteFunction ("sim_cerques",$_GET["id"]);
		}
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			echo "<tr style=\"background-color:silver;\">";
				...
				echo "<th></th>";
			echo "</tr>";
			$sql = "select id,nombre from sim_cerques where nombre<>'' order by nombre";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)){
	...
	if ($soption == 580) {
		if ($ssoption == 1) {
			...
			insertFunction ("sim_tarifas",$camposInsert,$datosInsert);
		}
		if ($ssoption == 2) {
			$camposUpdate=array('nombre','porcentage','descuento');
			$datosUpdate=array($_POST["nombre"],$_POST["porcentage"],$_POST["descuento"]);
			updateFunction("sim_tarifas",$_GET["id"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 3) {
			$camposUpdate=array('visible');
			$datosUpdate=array(0);
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "</tr>";
			echo "<tr><td>&nbsp;</td></tr>";
			$sql = "select * from sim_tarifas where visible=1";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)) {
				echo "<tr>";
...
function ver_contacto($id,$color,$contactes) {
	...
	echo "</tr>";
	if ($contactes == 1) {
		$sql = "select nombre,apellido1,apellido2,telefono from sim_clientes_contactos where id_cliente=".$id;
		$result = mysqli_query($dbhandle,convertSQL($sql));
		while ($row = mysqli_fetch_array($result)){
	...
	if ($contactes == 2) {
		$sql = "select nombre,apellido1,apellido2,telefono from sim_clientes_contactos where pred=1 and id_cliente=".$id;
		$result = mysqli_query($dbhandle,convertSQL($sql));
		$row = mysqli_fetch_array($result);
		echo "<tr><td colspan=\"5\">".$row["nombre"]." ".$row["apellido1"]." ".$row["apellido2"]."</td><td>".$row["telefono"]."</td></tr>";
	}
...
{
	global $db,$dbhandle;
	$sql = "select id from sim_clientes_classificacio_tipus where visible=1 and id_origen=".$id;
	$result = mysqli_query($dbhandle,convertSQL($sql));
	while ($row = mysqli_fetch_array($result)){
		$sqlx = "update sim_clientes_classificacio_tipus set ";
		$sqlx = $sqlx."color='".$color."'";
		$sqlx = $sqlx.",color_lletra='".$color_lletra."'";
		$sqlx = $sqlx." WHERE id=".$row["id"]."";
		mysqli_query($dbhandle,convertSQL($sqlx));
		$sqlc = "select id from sim_clientes_classificacio_tipus where visible=1 and id_origen=".$row["id"];
		$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
		while ($rowc = mysqli_fetch_array($resultc)){
			$sqlxx = "update sim_clientes_classificacio_tipus set ";
