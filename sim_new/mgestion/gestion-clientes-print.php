...
class PDF extends FPDF
{
	...
	for ($i = 0; $i <= 127; $i++) {
		if ( ((($i >= 48) and ($i <= 57)) or(($i >= 65) and ($i <= 90))) and (($_POST[chr($i)] == "true") or ($ver_todas_letras == true)) ) {
			...
			$color = "white";
			$calidad = 0;
			$sql = "select * from sim_clientess where visible=1";
			...
			while ($row = mysqli_fetch_array($result)) {
				...
				if (($ver == true) and (($_GET["id_classificacio"] != "") OR ($_POST["id_classificacio"] != ""))) {
					if (($_GET["id_classificacio"] != "") OR ($_POST["id_classificacio"] != 0))  {
						...
						$sqlcxx = "select count(*) as total  from sim_clientess_classificacio where id_client=".$row["id"]." and visible=1 and id_clasificacio_tipus in (".$id_classificacio.")";
						$resultcxx = mysqli_query($dbhandle,convertSQL($sqlcxx));
						$rowcxx = mysqli_fetch_array($resultcxx);
						if ($rowcxx["total"] <= 0) { $ver = false; }
						$sqlcxx2 = "select * from sim_clientess_classificacio_tipus where id_origen in (".$id_classificacio.")";
						$resultcxx2 = mysqli_query($dbhandle,convertSQL($sqlcxx2));
						while ($rowcxx2 = mysqli_fetch_array($resultcxx2)) {
							$sqlcxx3 = "select count(*) as total  from sim_clientess_classificacio where id_client=".$row["id"]." and visible=1 and id_clasificacio_tipus=".$rowcxx2["id"];
							$resultcxx3 = mysqli_query($dbhandle,convertSQL($sqlcxx3));
							$rowcxx3 = mysqli_fetch_array($resultcxx3);
							if ($rowcxx3["total"] > 0) { $ver = true; }
							$sqlcxx4 = "select * from sim_clientess_classificacio_tipus where id_origen=".$rowcxx2["id"];
							$resultcxx4 = mysqli_query($dbhandle,convertSQL($sqlcxx4));
							while ($rowcxx4 = mysqli_fetch_array($resultcxx4)) {
								$sqlcxx5 = "select count(*) as total  from sim_clientess_classificacio where id_client=".$row["id"]." and visible=1 and id_clasificacio_tipus=".$rowcxx4["id"];
				...
				if ($ver == true) {
					...
					$z++;
					$sqlc = "select * from sim_clientess where id=".$row["id"];
					$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
					$rowc = mysqli_fetch_array($resultc);
					if ($_POST["colorsi"] == true){
						$sqlctt = "select * from sim_clientess_classificacio where visible=1 and id_clasificacio_tipus<>0 and id_client=".$row["id"]." and predeterminado=1";
						...
						if ($rowctt) {
							...
							$sqlcc = "select * from sim_clientess_classificacio where id_client=".$row["id"];
						...
						if ($id_class_tipus > 0){
							$sqlct = "select * from sim_clientess_classificacio_tipus where id=".$id_class_tipus;
					...
					if ($rowc["id_carrer"] > 0){
						$sqld = "select * from sim_clientess_carrer_tipo where id=".$rowc["id_tipo_carrer"]." and visible=1";
						$resultd = mysqli_query($dbhandle,convertSQL($sqld));
						$rowd = mysqli_fetch_array($resultd);
						$sqldd = "select * from sim_clientess_carrer where id=".$rowc["id_carrer"]." and visible=1";
						$resultdd = mysqli_query($dbhandle,convertSQL($sqldd));
						$rowdd = mysqli_fetch_array($resultdd);
						$sqlddd = "select * from sim_clientess_sector_zf where id=".$rowc["id_sector_zf"]." and visible=1";
						$resultddd = mysqli_query($dbhandle,convertSQL($sqlddd));
						$rowddd = mysqli_fetch_array($resultddd);
