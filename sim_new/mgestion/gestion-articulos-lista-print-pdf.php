<?php 
	...
	if ($_POST["subfamilia"] > 0){ 
		$sqlsf = "select * from sim_articulos_subgrupos where id=".$_POST["subfamilia"];
		$resultsf = mysqli_query($dbhandle,convertSQL($sqlsf));
		$rowsf = mysqli_fetch_array($resultsf);

		$sqlf = "select * from sim_articulos_grupos where id=".$rowsf["id_grupo"];
	...
	$pdf->Cell(20,5,"".$unidades."",'TB',0);
	$pdf->Cell(20,5,"PVD",'TB',0);
	$pdf->Cell(20,5,"PVP",'TBR',1);

	$sql = "select * from sim_articulos where visible=1 ";
	if ($_POST["subfamilia"] > 0){ $sql = $sql."and id_subgrupo=".$_POST["subfamilia"];	}
	$sql = $sql." order by codigo";
	$result = mysqli_query($dbhandle,convertSQL($sql));
	while ($row = mysqli_fetch_array($result)) {
		$sqls = "select * from sgm_stock WHERE vigente=1 and id_articulo=".$row["id"];
