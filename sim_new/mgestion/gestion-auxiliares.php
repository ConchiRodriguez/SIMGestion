...
if (($option == 1023) AND ($autorizado == true)) {
	...
	if ($soption == 130){
		if ($ssoption == 1) {
			...
			insertFunction ("sim_idiomas",$camposInsert,$datosInsert);
		}
		if ($ssoption == 2) {
			$camposUpdate=array('idioma','descripcion','imagen');
			$datosUpdate=array($_POST["idioma"],$_POST["descripcion"],$_POST["imagen"]);
			updateFunction("sim_idiomas",$_GET["id"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 3) {
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "<tr>";
				...
				echo "</form>";
			echo "</tr>";
			$sql = "select * from sim_idiomas where visible=1";
