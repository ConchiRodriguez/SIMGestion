...
function factura_print_e($id)
{
		...
		$sqlx = "select * from sim_clients where id=".$rowcabezera["id_cliente"];
		...
		$sqldiv = "select abrev from sim_divisas where id=".$rowcabezera["id_divisa"];
		$resultdiv = mysqli_query($dbhandle,convertSQL($sqldiv));
		$rowdiv = mysqli_fetch_array($resultdiv);

		$sqlcc = "select nombre,apellido1,apellido2 from sim_clients_contactos where pred=1 and id_client=".$rowcabezera["id_cliente"];
...
#		$content .= "				<TotalGeneralSurcharges></TotalGeneralSurcharges>\n";
	...
	$sqlco = "select * from sim_contratos where id=".$rowcabezera["id_contrato"];
	$resultco = mysqli_query($dbhandle,convertSQL($sqlco));
	$rowco = mysqli_fetch_array($resultco);

	$sqlcuerpo = "select * from sim_factura_cuerpo where id_factura=".$rowcabezera["id"];
...
#		$content .= "					</SpecialTaxableEvent>\n";
	...
	$sqliban = "select * from sim_factura_dades_origen_iban where id=".$rowcabezera["id_dades_origen_factura_iban"];
