...
class PDF extends FPDF
{
	...
	{
		global $dbhandle;
		$sqlele = "select * from sim_dades_origen_factura";
	...
	function informe_print($id_cliente, $id_contrato){
		...
		$sqlcli = "select * from sim_clientes where id=".$id_cliente;
#		echo $sqlcli."<br>";
		$resultcli = mysqli_query($dbhandle,convertSQL($sqlcli));
		$rowcli = mysqli_fetch_array($resultcli);

		$sqlcon = "select * from sim_contratos where visible=1 and id=".$id_contrato;
...
#		echo $sqlcon."<br>";
		...
		$sqlx = "select * from sim_users where id=".$rowcon["id_responsable"];
		$resultx = mysqli_query($dbhandle,convertSQL($sqlx));
		$rowx = mysqli_fetch_array($resultx);

		$sqly = "select * from sim_users where id=".$rowcon["id_tecnico"];
		...
		while (($mes_act>=$mes_ini) and ($mes_act<=$mes_fin)){
			...
			$sqlcs = "select * from sim_contratos_servicio where id_contrato=".$rowcon["id"]." and visible=1 order by servicio";
			$resultcs = mysqli_query($dbhandle,convertSQL($sqlcs));
			while ($rowcs = mysqli_fetch_array($resultcs)) {
				$sqli1 = "select count(*) as abiertas from sim_incidencias where id_servicio=".$rowcs["id"]." and visible=1";
				$sqli1 = $sqli1." and (fecha_inicio between ".$mes_act." and ".$mes_seg." or id IN (select id_incidencia from sim_incidencias where id_incidencia<>0 and fecha_inicio between ".$mes_act." and ".$mes_seg." and visible=1))";
#				echo $sqli1."<br>";
				$resulti1 = mysqli_query($dbhandle,convertSQL($sqli1));
				$rowi1 = mysqli_fetch_array($resulti1);
				$open =$open + $rowi1["abiertas"];
				$sqli2 = "select count(*) as cerradas from sim_incidencias where id_servicio=".$rowcs["id"]." and visible=1 and id_estado=-2";
				$sqli2 = $sqli2." and (fecha_inicio between ".$mes_act." and ".$mes_seg." or id IN (select id_incidencia from sim_incidencias where id_incidencia<>0 and fecha_inicio between ".$mes_act." and ".$mes_seg." and visible=1))";
				$resulti2 = mysqli_query($dbhandle,convertSQL($sqli2));
				$rowi2 = mysqli_fetch_array($resulti2);
				$close =$close + $rowi2["cerradas"];
				$sqli3 = "select count(*) as pendientes from sim_incidencias where id_servicio=".$rowcs["id"]." and visible=1 and id_estado<>-2 and pausada=0";
				$sqli3 = $sqli3." and (fecha_inicio between ".$mes_act." and ".$mes_seg." or id IN (select id_incidencia from sim_incidencias where id_incidencia<>0 and fecha_inicio between ".$mes_act." and ".$mes_seg." and visible=1))";
				$resulti3 = mysqli_query($dbhandle,convertSQL($sqli3));
				$rowi3 = mysqli_fetch_array($resulti3);
				$pendin =$pendin + $rowi3["pendientes"];
				$sqli4 = "select count(*) as pausadas from sim_incidencias where id_servicio=".$rowcs["id"]." and visible=1 and id_estado<>-2 and pausada=1";
				$sqli4 = $sqli4." and (fecha_inicio between ".$mes_act." and ".$mes_seg." or id IN (select id_incidencia from sim_incidencias where id_incidencia<>0 and fecha_inicio between ".$mes_act." and ".$mes_seg." and visible=1))";
				$resulti4 = mysqli_query($dbhandle,convertSQL($sqli4));
				$rowi4 = mysqli_fetch_array($resulti4);
				$stoped =$stoped + $rowi4["pausadas"];
				$sqli5 = "select count(*) as fora_sla from sim_incidencias where id_servicio=".$rowcs["id"]." and visible=1 and id_estado=-2 and sla=1";
				$sqli5 = $sqli5." and (fecha_inicio between ".$mes_act." and ".$mes_seg." or id IN (select id_incidencia from sim_incidencias where id_incidencia<>0 and fecha_inicio between ".$mes_act." and ".$mes_seg." and visible=1))";
				...
				if ($_POST["horas"] == 1){
					$sqli6 = "select sum(duracion) as temps from sim_incidencias where visible=1 and fecha_inicio between ".$mes_act." and ".$mes_seg." and id_incidencia in (select id from sim_incidencias where id_servicio=".$rowcs["id"]." and visible=1)";
#echo $sqli6;
				...
				$sqlcsc = "select * from sim_contratos_sla_cobertura where visible=1 and id=".$rowcs["id_cobertura"];
...
#				$this->CellFitSpace(40,$height,$text,'LTBR',0);
		...
		$resultcs = mysqli_query($dbhandle,convertSQL($sqlcs));
		while ($rowcs = mysqli_fetch_array($resultcs)) {
			$sqli0 = "select * from sim_incidencias where id_servicio=".$rowcs["id"]." and id_estado<>-2 and visible=1";
		...
		while ($rowcs = mysqli_fetch_array($resultcs)) {
			...
			if ($_POST["horas"] == 1){
				$sqli6 = "select sum(duracion) as temps from sim_incidencias where visible=1 and id_incidencia in (select id from sim_incidencias where id_servicio=".$rowcs["id"]." and visible=1)";
				$resulti6 = mysqli_query($dbhandle,convertSQL($sqli6));
				$rowi6 = mysqli_fetch_array($resulti6);
				$time =$time + $rowi6["temps"];
				$hora = $rowi6["temps"]/60;
...
#			$this->CellFitSpace(40,$height,$text,'LTBR',0);
	...
	if ($_POST["id_cliente"] > 0){ $sql .= " and id=".$_POST["id_cliente"]."";} else {$sql .= " and id in (select id_cliente from sim_contratos where visible=1)";}
	$sql .= " order by nombre";
#	echo $sql;
	$result = mysqli_query($dbhandle,convertSQL($sql));
	while ($row = mysqli_fetch_array($result)){
		if ($_POST["id_contrato"] == 0) {$sqlcon = "select * from sim_contratos where visible=1 and id_cliente=".$row["id"];}
		else {$sqlcon = "select * from sim_contratos where visible=1 and id=".$_POST["id_contrato"];}
		$resultcon = mysqli_query($dbhandle,convertSQL($sqlcon));
