...
if (($option == 1002) AND ($autorizado == true)) {
	...
	if ($soption == 0) {
		...
		echo "<table><tr>";
			echo "<td class=\"menu\">";
				if ($_GET["ver"] == 0) { 
					$sql = "select * from sim_usuarios where activo=1 order by usuario";
					echo "<a href=\"index.php?op=1002&sop=0&ver=1\" style=\"color:white;\">".$Ver." ".$Todos."</a>";
				}
				if ($_GET["ver"] == 1) { 
					$sql = "select * from sim_usuarios order by usuario";
			...
			echo "<td class=\"menu\">";
				...
				if ($_GET["sims"] == 1) { 
					$sql = "select * from sim_usuarios where sgm=1 order by usuario";
				}
			echo "</td>";
		echo "</tr></table>";
		echo "<br><br>";
	...
	if ($soption == 3) {
		if ($ssoption == 1) {
			$sql = "select * from sim_usuarios_clients where id_usuario=".$_GET["id_usuario"]." and id_cliente=".$_POST["id_cliente"];
			$result = mysqli_query($dbhandle,convertSQL($sql));
			$row = mysqli_fetch_array($result);
			if (!$row){
				$camposInsert = "id_usuario,id_cliente,admin";
				$datosInsert = array($_GET["id_usuario"],$_POST["id_cliente"],$_POST["admin"]);
				insertFunction ("sim_usuarios_clients",$camposInsert,$datosInsert);
			}
		}
		if ($ssoption == 2) {
			$camposUpdate = array("admin");
			$datosUpdate = array($_POST["admin"]);
			updateFunction ("sim_usuarios_clients",$_GET["id_linea"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 3) {
			deleteFunction ("sim_usuarios_clients",$_GET["id_linea"]);
		}

		$sqluser = "select id,usuario from sim_usuarios WHERE id=".$_GET["id_usuario"];
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			echo "<tr style=\"background-color:silver;\">";
				...
				echo "<th></th>";
			echo "</tr>";
			echo "<form action=\"index.php?op=1002&sop=3&ssop=1&id_usuario=".$rowuser["id"]."\" method=\"post\">";
			echo "<tr>";
				echo "<td></td>";
				echo "<td><select name=\"id_cliente\" style=\"width:500px;\">";
			...
			echo "<tr><td>&nbsp;</td></tr>";
			$sql = "select * from sim_usuarios_clients where id_usuario=".$_GET["id_usuario"];
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)) {
				$sqlc = "select id,nombre from sgm_clients where id=".$row["id_cliente"];
				$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
				$rowc = mysqli_fetch_array($resultc);
				echo "<tr>";
					echo "<td style=\"text-align:center\"><a href=\"index.php?op=1002&sop=3&ssop=3&id_usuario=".$_GET["id_usuario"]."&id_linea=".$row["id"]."\"><img src=\"mgestion/pics/icons-mini/page_white_delete.png\" style=\"border:0px;\"></a></td>";
					echo "<td style=\"width:200px;\">".$rowc["nombre"]."</td>";
					echo "<form action=\"index.php?op=1002&sop=3&ssop=2&id_usuario=".$_GET["id_usuario"]."&id_linea=".$row["id"]."\" method=\"post\">";
					echo "<td><select name=\"admin\" style=\"width:40px\">";
						if ($row["admin"] == 1) {
					...
					echo "</select></td>";
					echo "<input type=\"Hidden\" name=\"id_cliente\" value=\"".$rowc["id"]."\">";
	...
	if (($soption == 510) and ($admin == true)) {
		if ($ssoption == 1){
			...
			insertFunction ("sim_usuarios_tipus","tipus",$datosInsert);
		}
		if ($ssoption == 2){
			$camposUpdate = array("tipus");
			$datosUpdate = array($_POST["tipus"]);
			updateFunction ("sim_usuarios_tipus",$_GET["id"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 3){
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "</form>";
			echo "<tr><td>&nbsp;</td></tr>";
		$sqlut = "select * from sim_usuarios_tipus where visible=1 order by tipus";
	...
	if (($soption == 512) and ($admin == true)) {
		if ($ssoption == 1) {
			$sql = "select id from sim_usuarios_rel_modulos_modulos where visible=1 order by nombre";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)) {
				if ($_POST["permiso".$row["id"]] == 0) {
					$sqlt = "select * from sim_usuarios_rel_modulos where id_tipus=".$_GET["id"]." and id_modulo=".$_POST["id_modulo".$row["id"]];
					$resultt = mysqli_query($dbhandle,convertSQL($sqlt));
					$rowt = mysqli_fetch_array($resultt);
					deleteFunction ("sim_usuarios_rel_modulos",$rowt["id"]);
				}
				if ($_POST["permiso".$row["id"]] == 1) {
					...
					if (!$rowt){
						...
						insertFunction ("sim_usuarios_rel_modulos",$camposInsert,$datosInsert);
					}
				}
				$sqlt = "select * from sim_usuarios_rel_modulos where id_tipus=".$_GET["id"]." and id_modulo=".$_POST["id_modulo".$row["id"]];
				$resultt = mysqli_query($dbhandle,convertSQL($sqlt));
				...
				updateFunction ("sim_usuarios_rel_modulos",$rowt["id"],$camposUpdate,$datosUpdate);
			}
		}

		$sql = "select * from sim_usuarios_tipus where id=".$_GET["id"];
		$result = mysqli_query($dbhandle,convertSQL($sql));
		$row = mysqli_fetch_array($result);
		...
		$sql = "select id,nombre,id_modulo from sim_usuarios_rel_modulos_modulos where visible=1 order by nombre";
		$result = mysqli_query($dbhandle,convertSQL($sql));
		while ($row = mysqli_fetch_array($result)) {
			$sqlt = "select count(*) as total from sim_usuarios_rel_modulos where id_tipus=".$_GET["id"]." and id_modulo=".$row["id_modulo"];
			$resultt = mysqli_query($dbhandle,convertSQL($sqlt));
			$rowt = mysqli_fetch_array($resultt);
			echo "<tr>";
				...
				echo "</select></td>";
				$sqlad = "select * from sim_usuarios_rel_modulos where id_tipus=".$_GET["id"]." and id_modulo=".$row["id_modulo"];
