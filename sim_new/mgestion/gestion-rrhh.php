...
if (($option == 1020) AND ($autorizado == true)) {
	...
	if (($soption == 0) or ($soption == 1) or ($soption == 200)) {
		if ($ssoption == 3) {
			...
			$datosUpdate = array("0");
			updateFunction ("sim_rrhh_empleado",$_GET["id"],$camposUpdate,$datosUpdate);
		}
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			if ($soption == 200){
				...
				echo "<tr style=\"background-color:silver;\">";
					echo "<td><select style=\"width:250px\" name=\"id_departamento\">";
						echo "<option value=\"0\">-</option>";
							$sqlt = "select id,departamento from sim_rrhh_departamento where visible=1 order by departamento";
					...
					echo "<td><select style=\"width:250px\" name=\"id_puesto\">";
						echo "<option value=\"0\">-</option>";
							$sqlg = "select id,puesto from sim_rrhh_puesto_trabajo where visible=1 and activo=1";
					...
					echo "<td><select style=\"width:500px\" name=\"id_cliente\">";
						echo "<option value=\"0\">-</option>";
						$sql = "select id,nombre,cognom1,cognom2 from sim_clientess where visible=1 order by nombre";
				...
				echo "<tr>";
					...
					for ($i = 0; $i <= 127; $i++) {
						if ((($i >= 48) and ($i <= 57)) or(($i >= 65) and ($i <= 90))) {
							$sqlxt = "select count(*) as total from sim_rrhh_empleado where visible=1 and nombre like '".chr($i)."%'  order by nombre";
			...
			for ($i = 48; $i <= 90; $i++) {
				if ((($i >= 48) and ($i <= 57)) or (($i >= 65) and ($i <= 90))) {
					...
					if ($ver == true){
						$contador = 0;
						$cambio = 0;
						$sql = "select * from sim_rrhh_empleado where visible=1 and nombre like '".chr($i)."%'";
						if (($_POST["id_departamento"] > 0) or ($_POST["id_puesto"] > 0)) {
							$sql .= " and id in (";
							$sql .= "select id_empleado from sim_rrhh_empleado_puesto where visible=1 ";
							if ($_POST["id_puesto"] > 0) { $sql .= " and id_puesto=".$_POST["id_puesto"]; }
							elseif ($_POST["id_departamento"] > 0) { $sql .= " and id_puesto in (select id from sim_rrhh_puesto_trabajo where id_departamento=".$_POST["id_departamento"].")"; }
							$sql .= ")";
						}
						if ($_POST["id_cliente"] > 0) { $sql .= " and id_usuario in (select id_usuario from sim_users_clients where id_client=".$_POST["id_cliente"].")"; }
#						echo $sql;
						$result = mysqli_query($dbhandle,convertSQL($sql));
						while ($row = mysqli_fetch_array($result)) {
							if ($contador==0){
							...
							echo "<tr style=\"background-color:".$color."\">";
								...
								$sqlp = "select id_departamento,puesto from sim_rrhh_puesto_trabajo where visible=1 and id in (select id_puesto from sim_rrhh_empleado_puesto where visible=1 and id_empleado=".$row["id"].")";
								$resultp = mysqli_query($dbhandle,convertSQL($sqlp));
								while ($rowp = mysqli_fetch_array($resultp)){
									$sqld = "select departamento from sim_rrhh_departamento where visible=1 and id=".$rowp["id_departamento"];
	...
	if (($soption >= 100) and ($soption < 200) and  (($_GET["id"] != 0) or ($ssoption == 1))) {
		if (($soption == 100) and ($ssoption == 1)) {
			...
			insertFunction ("sim_rrhh_empleado",$camposInsert,$datosInsert);
			$sql = "select id from sim_rrhh_empleado where visible=1 and nombre='".$_POST["nombre"]."' and nif='".$_POST["nif"]."' order by id desc";
			$result = mysqli_query($dbhandle,convertSQL($sql));
		...
		$sql = "select * from sim_rrhh_empleado where id=".$id_empleado;
		$result = mysqli_query($dbhandle,convertSQL($sql));
		$row = mysqli_fetch_array($result);
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			echo "<tr>";
	...
	if ($soption == 100) {
		if ($id_empleado > 0) {
			$sqlem = "select * from sim_rrhh_empleado where id=".$id_empleado;
				...
				echo "<td style=\"vertical-align:top;\">";
					echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
						...
						echo "<tr><th style=\"text-align:right;\">".$Usuario.": </th><td>";
							...
							echo "<option value=\"0\">-</option>";
							$sqlu = "select id,usuario from sim_users where validado=1 and activo=1 and sgm=1 order by usuario";
			...
			echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
				echo "<tr>";
					echo "<td style=\"width:50%;vertical-align:top;\">";
						...
						$result = mysqli_query($dbhandle,convertSQL($sql));
						while ($row = mysqli_fetch_array($result)) {
							$sqlele = "select id,name,size from sim_files where id_tipo=".$row["id"]." and tipo_id_elemento=6 and id_elemento=".$_GET["id"];
	...
	if ($soption == 120){
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "</tr>";
			$sqlf = "select nombre,fecha_inicio,fecha_fin from sim_rrhh_formacion where id in (select id_curso from sim_rrhh_formacion_empleado where id_empleado=".$_GET["id"]." and visible=1)";
			$resultf = mysqli_query($dbhandle,convertSQL($sqlf));
	...
	if ($soption == 130){
		if ((($_POST["fecha_inicio"] > $_POST["fecha_fin"]) and ($_POST["fecha_fin"] != "") and ($_POST["fecha_fin"] != "0000-00-00")) or ($_POST["hora_ini"] > $_POST["hora_fi"]) or ($_POST["hora_ini2"] > $_POST["hora_fi2"])){
			echo $_POST["fecha_fin"].mensageError($ErrorEmpleadoHorario);
		} else {
			if ($ssoption == 1) {
				$camposinsert = "id_empleado,fecha_inicio,fecha_fin,hora_ini,hora_fi,hora_ini2,hora_fi2,dia_setmana";
				$datosInsert = array($_GET["id"],$_POST["fecha_inicio"],$_POST["fecha_fin"],$_POST["hora_ini"],$_POST["hora_fi"],$_POST["hora_ini2"],$_POST["hora_fi2"],$_POST["dia_setmana"]);
				insertFunction ("sim_rrhh_empleado_horario",$camposinsert,$datosInsert);
			}
			if ($ssoption == 2) {
				$camposUpdate = array("fecha_inicio","fecha_fin","hora_ini","hora_fi","hora_ini2","hora_fi2","dia_setmana");
				$datosUpdate = array($_POST["fecha_inicio"],$_POST["fecha_fin"],$_POST["hora_ini"],$_POST["hora_fi"],$_POST["hora_ini2"],$_POST["hora_fi2"],$_POST["dia_setmana"]);
				updateFunction ("sim_rrhh_empleado_horario",$_GET["id_horari"],$camposUpdate,$datosUpdate);
			}
		}
		if ($ssoption == 3) {
			$camposUpdate = array("visible");
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "<tr>";
				...
				echo "<td></td>";
				echo "<td><input type=\"text\" name=\"fecha_inicio\" style=\"width:100px;\" value=\"".date("Y-m-d")."\"></td>";
				echo "<td><input type=\"text\" name=\"fecha_fin\" style=\"width:100px;\" value=\"\"></td>";
			...
			echo "<tr><td>&nbsp;</td></tr>";
			$fecha = date("Y-m-d");
			$sql = "select * from sim_rrhh_empleado_horario where visible=1 and id_empleado=".$_GET["id"]." order by fecha_inicio desc";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)){
				$color = "white";
				if (($fecha >= $row["fecha_inicio"]) and (($fecha <= $row["fecha_fin"]) or ($row["fecha_fin"] == "0000-00-00"))) { $color = "green"; }
				echo "<tr style=\"background-color:".$color."\">";
					...
					echo "<td><input type=\"text\" name=\"fecha_inicio\" style=\"width:100px;\" value=\"".$row["fecha_inicio"]."\"></td>";
					echo "<td><input type=\"text\" name=\"fecha_fin\" style=\"width:100px;\" value=\"".$row["fecha_fin"]."\"></td>";
	...
	if ($soption == 140){
		if ($ssoption == 1) {
			...
			insertFunction ("sim_rrhh_empleado_puesto",$camposInsert,$datosInsert);
		}
		if ($ssoption == 2){
			$camposUpdate = array("fecha_alta","fecha_baja");
			$datosUpdate = array($_POST["fecha_alta"],$_POST["fecha_baja"]);
			updateFunction ("sim_rrhh_empleado_puesto",$_GET["puesto"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 3) {
			$camposUpdate = array("visible");
			$datosUpdate = array("0");
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "<tr>";
				...
				echo "<option value=\"0\">-</option>";
				$sqlpt = "select id,id_departamento,puesto from sim_rrhh_puesto_trabajo where visible=1 and activo=1 and id NOT IN (select id_puesto from sim_rrhh_empleado_puesto where fecha_baja='0000-00-00' and visible=1) order by puesto";
				$resultpt = mysqli_query($dbhandle,convertSQL($sqlpt));
				while ($rowpt = mysqli_fetch_array($resultpt)) {
					$sqld = "select departamento from sim_rrhh_departamento where visible=1 and id=".$rowpt["id_departamento"];
					$resultd = mysqli_query($dbhandle,convertSQL($sqld));
					$rowd = mysqli_fetch_array($resultd);
			...
			echo "</tr>";
			echo "<tr><td>&nbsp;</td></tr>";
			$sqlpe = "select * from sim_rrhh_empleado_puesto where id_empleado=".$_GET["id"]." and visible=1";
			$resultpe = mysqli_query($dbhandle,convertSQL($sqlpe));
			while ($rowpe = mysqli_fetch_array($resultpe)){
				$sqle = "select puesto from sim_rrhh_puesto_trabajo where id=".$rowpe["id_puesto"];
	...
	if ($soption == 150) {
		if (date(U,strtotime($_POST["fecha_inicio"])) > date(U,strtotime($_POST["fecha_fin"]))) {
			echo mensageError($ErrorEmpleadoCalendario);
		} else {
			if ($ssoption == 1) {
				$camposInsert = "id_empleado,fecha_inicio,fecha_fin,total";
				$datosInsert = array($_GET["id"],date(U,strtotime($_POST["fecha_inicio"])),date(U,strtotime($_POST["fecha_fin"])),$_POST["total"]);
				insertFunction ("sim_rrhh_empleado_nominas",$camposInsert,$datosInsert);
			}
			if ($ssoption == 2){
				$camposUpdate = array("fecha_inicio","fecha_fin","total");
				$datosUpdate = array(date(U,strtotime($_POST["fecha_inicio"])),date(U,strtotime($_POST["fecha_fin"])),$_POST["total"]);
				updateFunction ("sim_rrhh_empleado_nominas",$_GET["id_nom"],$camposUpdate,$datosUpdate);
			}
		}
		if ($ssoption == 3) {
			deleteFunction ("sim_rrhh_empleado_nominas",$_GET["id_nom"]);
		}
		echo "<h4>".$Nominas."</h4>";
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			echo "<tr style=\"background-color:silver;\">";
			...
			echo "<tr>";
				...
				echo "<td><input name=\"fecha_inicio\" type=\"Text\" style=\"width:100px\" value=\"".date('d-m-Y')."\"></td>";
				echo "<td><input name=\"fecha_fin\" type=\"Text\" style=\"width:100px\" value=\"".date('d-m-Y',time()+(30*24*60*60))."\"></td>";
				echo "<td><input name=\"total\" type=\"Text\" style=\"width:150px\" value=\"0.00\"></td>";
				echo "<td><input type=\"Submit\" value=\"".$Anadir."\"></td>";
				echo "</form>";
			echo "</tr>";
			echo "<tr><td>&nbsp;</td></tr>";
			$sql = "select * from sim_rrhh_empleado_nominas where id_empleado=".$_GET["id"]." order by fecha_inicio";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)) {
				echo "<tr>";
					...
					echo "<td><input name=\"fecha_inicio\" type=\"Text\" style=\"width:100px\" value=\"".date('d-m-Y',$row["fecha_inicio"])."\"></td>";
					if ($row["fecha_fin"] > 0) {
						echo "<td><input name=\"fecha_fin\" type=\"Text\" style=\"width:100px\" value=\"".date('d-m-Y',$row["fecha_fin"])."\"></td>";
					} else {
						echo "<td><input name=\"fecha_fin\" type=\"Text\" style=\"width:100px\"></td>";
					}
	...
	if ($soption == 160){
		if (($_POST["fecha_fin"] != '') and (date(U,strtotime($_POST["fecha_inicio"])) > date(U,strtotime($_POST["fecha_fin"])))) {
			echo mensageError($ErrorEmpleadoCalendario);
		} else {
			$total_di = dif_dias_fechas(date(U,strtotime($_POST["fecha_inicio"])),date(U,strtotime($_POST["fecha_fin"])));
			if ($ssoption == 1) {
				$camposInsert = "id_empleado,fecha_inicio,fecha_fin,total_dias,motivo,vacaciones,remunerado";
				$datosInsert = array($_GET["id"],date(U,strtotime($_POST["fecha_inicio"])),date(U,strtotime($_POST["fecha_fin"])),$total_di,$_POST["motivo"],$_POST["vacaciones"],$_POST["remunerado"]);
				insertFunction ("sim_rrhh_empleado_calendario",$camposInsert,$datosInsert);
			}
			if ($ssoption == 2){
				$camposUpdate = array("fecha_inicio","fecha_fin","total_dias","motivo","vacaciones","remunerado");
				$datosUpdate = array(date(U,strtotime($_POST["fecha_inicio"])),date(U,strtotime($_POST["fecha_fin"])),$total_di,$_POST["motivo"],$_POST["vacaciones"],$_POST["remunerado"]);
				updateFunction ("sim_rrhh_empleado_calendario",$_GET["id_dia"],$camposUpdate,$datosUpdate);
			}
		}
		if ($ssoption == 3) {
			deleteFunction ("sim_rrhh_empleado_calendario",$_GET["id_dia"]);
		}
			...
			echo "<tr>";
				echo "<td style=\"vertical-align:top;\">";
					echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
						...
						echo "</tr>";
						echo "<tr><td>&nbsp;</td></tr>";
						$sql = "select * from sim_rrhh_empleado_calendario where id_empleado=".$_GET["id"]." order by fecha_inicio desc";
						$result = mysqli_query($dbhandle,convertSQL($sql));
						while ($row = mysqli_fetch_array($result)) {
							echo "<tr>";
				...
				echo "</td><td  style=\"vertical-align:top;\">";
					$sqlv = "select * from sim_rrhh_num_dias_vacaciones";
					...
					echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
						...
						for ($i=2012;$i<=(date('Y')+1);$i++){
							$dias_vac=0;
							$sql = "select fecha_fin,fecha_inicio,total_dias from sim_rrhh_empleado_calendario where id_empleado=".$_GET["id"]." and fecha_inicio between ".date("U", mktime(0, 0, 0, 1, 1, $i))." and ".date("U", mktime(0, 0, 0, 12, 31, $i))." and vacaciones=1";
							$result = mysqli_query($dbhandle,convertSQL($sql));
							while ($row = mysqli_fetch_array($result)) {
								if (($row["fecha_fin"] > 0) and (date("Y",$row["fecha_fin"]) > $i)){
									$dias_vac += dif_dias_fechas($row["fecha_inicio"],date("U", mktime(0, 0, 0, 12, 31, $i)));
								} else {
									$dias_vac += $row["total_dias"];
								}
							}
							$sql2 = "select fecha_fin,fecha_inicio from sim_rrhh_empleado_calendario where id_empleado=".$_GET["id"]." and fecha_fin between ".date("U", mktime(0, 0, 0, 1, 1, $i))." and ".date("U", mktime(0, 0, 0, 12, 31, $i))." and vacaciones=1";
							$result2 = mysqli_query($dbhandle,convertSQL($sql2));
							while ($row2 = mysqli_fetch_array($result2)) {
								if (date("Y",$row2["fecha_inicio"]) < $i){
									$dias_vac += dif_dias_fechas(date("U", mktime(0, 0, 0, 1, 1, $i)),$row2["fecha_fin"]);
								}
							}
							echo "<tr>";
				...
				echo "</td><td style=\"vertical-align:top;width:50px;\">";
				echo "</td><td  style=\"vertical-align:top;\">";
					$sqll = "select * from sim_rrhh_num_dias_libres";
					...
					echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
						...
						for ($i=2012;$i<=date('Y');$i++){
							$sql = "select fecha_fin,fecha_inicio,total_dias from sim_rrhh_empleado_calendario where id_empleado=".$_GET["id"]." and fecha_inicio between ".date("U", mktime(0, 0, 0, 1, 1, $i))." and ".date("U", mktime(0, 0, 0, 12, 31, $i))." and vacaciones=0 and remunerado=0";
							$result = mysqli_query($dbhandle,convertSQL($sql));
							while ($row = mysqli_fetch_array($result)) {
								if (($row["fecha_fin"] > 0) and (date("Y",$row["fecha_fin"]) > $i)){
									$dias_lib += dif_dias_fechas($row["fecha_inicio"],date("U", mktime(0, 0, 0, 12, 31, $i)));
								} else {
	...
	if ($soption == 170) {
		...
		$v_recibo = 0;
		$tds = 3;
		$sqltipo = "select * from sim_factura_tipos where id IN (9,5)";
		...
		echo "<table cellpadding=\"2\" cellspacing=\"0\" class=\"lista\">";
			...
			$totalSenseIVA = 0;
			$totalPagat = 0;
			$sqldiv = "select id,abrev from sim_divisas where predefinido=1";
			$resultdiv = mysqli_query($dbhandle,convertSQL($sqldiv));
			$rowdiv = mysqli_fetch_array($resultdiv);
			$sqlem = "select id_usuario from sim_rrhh_empleado where id=".$_GET["id"];
			$resultem = mysqli_query($dbhandle,convertSQL($sqlem));
			$rowem = mysqli_fetch_array($resultem);
			$sqlca = "select * from sim_cabezera where visible=1 and id_pagador=".$rowem["id_usuario"]." order by numero desc,version desc,fecha desc";
	...
	if (($soption == 510) AND ($admin == true)) {
		if ($ssoption == 1) {
			$camposInsert = "dia,mes,descripcion";
			$datosInsert = array($_POST["dia"],$_POST["mes"]+1,$_POST["descripcion"]);
			insertFunction ("sim_calendario",$camposInsert,$datosInsert);
		}
		if ($ssoption == 2) {
			deleteFunction ("sim_calendario",$_GET["id"]);
		}
		if ($ssoption == 3) {
			if ($_GET["id"] == 0){
				$camposInsert = "num_dias_vac";
				$datosInsert = array($_POST["num_dias_vac"]);
				insertFunction ("sim_rrhh_num_dias_vacaciones",$camposInsert,$datosInsert);
			} else {
				$camposUpdate=array('num_dias_vac');
				$datosUpdate=array($_POST["num_dias_vac"]);
				updateFunction("sim_rrhh_num_dias_vacaciones",$_GET["id"],$camposUpdate,$datosUpdate);
			}
		}
		if ($ssoption == 4) {
			if ($_GET["id"] == 0){
				...
				insertFunction ("sim_rrhh_num_dias_libres",$camposInsert,$datosInsert);
			} else {
				$camposUpdate=array('num_dias_lib');
				$datosUpdate=array($_POST["num_dias_lib"]);
				updateFunction("sim_rrhh_num_dias_libres",$_GET["id"],$camposUpdate,$datosUpdate);
			}
		}
		if ($ssoption == 5) {
			if ($_GET["id"] == 0){
				...
				insertFunction ("sim_rrhh_jornada_anual",$camposInsert,$datosInsert);
			} else {
				$camposUpdate=array('num_horas_any');
				$datosUpdate=array($_POST["num_horas_any"]);
				updateFunction("sim_rrhh_jornada_anual",$_GET["id"],$camposUpdate,$datosUpdate);
			}
		}
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			echo "<tr>";
				echo "<td style=\"vertical-align:top;\">";
					echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
						...
						echo "<tr>";
							...
							echo "</select></td>";
							echo "<td><input name=\"descripcion\" type=\"Text\" style=\"width:150px\"></td>";
							echo "<td><input type=\"Submit\" value=\"".$Anadir."\"></td>";
							echo "</form>";
						echo "</tr>";
						echo "<tr><td>&nbsp;</td></tr>";
						$sql = "select * from sim_calendario order by mes, dia";
						$result = mysqli_query($dbhandle,convertSQL($sql));
						while ($row = mysqli_fetch_array($result)) {
							echo "<tr>";
				...
				echo "</td><td style=\"vertical-align:top;width:50px;\">";
				echo "</td><td  style=\"vertical-align:top;\">";
					$sqlj = "select * from sim_rrhh_jornada_anual";
	...
	if (($soption == 520) AND ($admin == true)) {
		if ($ssoption == 1) {
			...
			updateFunction("sim_calendario_horario",$_GET["id"],$camposUpdate,$datosUpdate);
		}
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			echo "<tr style=\"background-color:silver;\">";
				...
				echo "<th></th>";
			echo "</tr>";
			$sql = "select * from sim_calendario_horario order by id";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)) {
	...
	if (($soption == 530) AND ($admin == true)) {
		if ($ssoption == 1){
			...
			insertFunction ("sim_rrhh_departamento",$camposinsert,$datosInsert);
		}
		if ($ssoption == 2){
			$camposUpdate = array("id_departamento","departamento");
			$datosUpdate = array($_POST["id_departamento"],$_POST["departamento"]);
			updateFunction ("sim_rrhh_departamento",$_GET["id"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 3) {
			$camposUpdate = array("visible");
			$datosUpdate = array("0");
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "</tr><tr>";
				...
				echo "<td><select name=\"id_departamento\" style=\"width:200px\">";
					...
					$sql = "select * from sim_rrhh_departamento where visible=1";
					$result = mysqli_query($dbhandle,convertSQL($sql));
					while ($row = mysqli_fetch_array($result)){
			...
			echo "</tr>";
			echo "<tr><td>&nbsp;</td></tr>";
			$sql = "select * from sim_rrhh_departamento where visible=1 order by departamento";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)){
				echo "<tr style=\"text-align:center\">";
					...
					echo "<td><select name=\"id_departamento\" style=\"width:200px\">";
						echo "<option value=\"0\" selected>-</option>";
						$sqld = "select * from sim_rrhh_departamento where visible=1";
						$resultd = mysqli_query($dbhandle,convertSQL($sqld));
	...
	if (($soption == 540) AND ($admin == true)) {
		if ($ssoption == 1){
			...
			insertFunction ("sim_rrhh_puesto_trabajo",$camposinsert,$datosInsert);
		}
		if ($ssoption == 2){
			...
			updateFunction ("sim_rrhh_puesto_trabajo",$_GET["id"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 3) {
			$camposUpdate = array("visible");
			$datosUpdate = array("0");
		...
		if ($ssoption == 4) {
			$sql = "select * from sim_rrhh_puesto_trabajo where id=".$_GET["id"];
			$result = mysqli_query($dbhandle,convertSQL($sql));
			$row = mysqli_fetch_array($result);
		...
		if ($ssoption == 5) {
			echo $sql = "select id from sim_rrhh_empleado_puesto where fecha_baja='0000-00-00' and visible=1 and id_puesto in (select * from sim_rrhh_puesto_trabajo where id=".$_GET["id"].")";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			$row = mysqli_fetch_array($result);
			if ($row){
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\" width=\"100%\">";
			echo "<tr>";
				echo "<td style=\"vertical-align:top;\">";
					echo "<table cellpadding=\"2\" cellspacing=\"0\" class=\"lista\">";
						echo "<tr style=\"background-color: Silver;\">";
							...
							echo "<th></th>";
						echo "</tr>";
						$sql = "select * from sim_rrhh_puesto_trabajo where visible=1 order by puesto";
						$result = mysqli_query($dbhandle,convertSQL($sql));
						while ($row = mysqli_fetch_array($result)){
							echo "<tr>";
								...
								$sqld = "select departamento from sim_rrhh_departamento where visible=1 and id=".$row["id_departamento"];
								$resultd = mysqli_query($dbhandle,convertSQL($sqld));
								$rowd = mysqli_fetch_array($resultd);
								echo "<td style=\"text-align:left;\">".$rowd["departamento"]."</td>";
								$sqlp = "select id,nombre from sim_rrhh_empleado where id=(select id_empleado from sim_rrhh_empleado_puesto where visible=1 and fecha_baja='0000-00-00' and id_puesto=".$row["id"].")";
								$resultp = mysqli_query($dbhandle,convertSQL($sqlp));
				...
				echo "<td>";
					if ($_GET["edit"] > 0){
						$sql = "select * from sim_rrhh_puesto_trabajo where visible=1 and id=".$_GET["id"]."";
						$result = mysqli_query($dbhandle,convertSQL($sql));
						$row = mysqli_fetch_array($result);
						echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
						if ($_GET["edit"] == 1){
						...
						if ($_GET["edit"] == 2) {
							...
							echo "</tr><tr>";
								...
								echo "<td><select name=\"id_departamento\" style=\"width:500px\">";
										echo "<option value=\"0\" selected>-</option>";
										$sqld = "select id,departamento from sim_rrhh_departamento where visible=1 order by departamento";
										$resultd = mysqli_query($dbhandle,convertSQL($sqld));
										while ($rowd = mysqli_fetch_array($resultd)){
	...
	if (($soption == 542) AND ($admin == true)) {
		$sql = "select id,puesto from sim_rrhh_puesto_trabajo where id=".$_GET["id"];
		$result = mysqli_query($dbhandle,convertSQL($sql));
		$row = mysqli_fetch_array($result);
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "</tr>";
			$sqlpe = "select * from sim_rrhh_empleado_puesto where id_puesto=".$row["id"];
			$resultpe = mysqli_query($dbhandle,convertSQL($sqlpe));
			while ($rowpe = mysqli_fetch_array($resultpe)){
				$sqle = "select nombre from sim_rrhh_empleado where id=".$rowpe["id_empleado"];
				$resulte = mysqli_query($dbhandle,convertSQL($sqle));
				while ($rowe = mysqli_fetch_array($resulte)){
					echo "<tr>";
	...
	if (($soption == 550) AND ($admin == true)) {
		if ($ssoption == 1){
			...
			insertFunction ("sim_rrhh_formacion",$camposinsert,$datosInsert);
		}
		if ($ssoption == 2){
			...
			updateFunction ("sim_rrhh_formacion",$_GET["id"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 3) {
			$camposUpdate = array("visible");
			$datosUpdate = array("0");
		...
		if ($ssoption == 4) {
			$sql = "select * from sim_rrhh_formacion where id=".$_GET["id"];
			$result = mysqli_query($dbhandle,convertSQL($sql));
			$row = mysqli_fetch_array($result);
			$camposinsert = "fecha,fecha_inicio,fecha_fin,numero,nombre,tipo,impartidor,duracion,temario,observaciones,coste,planificado,realizado";
				...
				echo "<td style=\"vertical-align:top;\">";
					echo "<table cellpadding=\"2\" cellspacing=\"0\" class=\"lista\">";
						echo "<tr style=\"background-color: Silver;\">";
							...
							echo "<th></th>";
						echo "</tr>";
						$sql = "select * from sim_rrhh_formacion where visible=1";
						$result = mysqli_query($dbhandle,convertSQL($sql));
						while ($row = mysqli_fetch_array($result)){
							echo "<tr>";
								echo "<td style=\"text-align:center\">";
								$sqlp = "select count(*) as total from sim_rrhh_formacion_empleado where visible=1 and id_curso=".$row["id"];
								$resultp = mysqli_query($dbhandle,convertSQL($sqlp));
								$rowp = mysqli_fetch_array($resultp);
								if ($rowp["total"] == 0){
				...
				echo "<td>";
					if ($_GET["edit"] > 0){
						...
						if ($_GET["edit"] == 2) {
							...
							$sqlx = "select * from sim_rrhh_formacion where id=".$_GET["id"]."";
	...
	if (($soption == 552) AND ($admin == true)) {
		if ($ssoption == 1) {
			$sqlt2 = "select count(*) as total from sim_rrhh_formacion_empleado where visible=1 and id_curso=".$_GET["id"]." and id_empleado=".$_POST["id_empleado"];
			...
			if ($rowt2["total"] == 0) {
				...
				insertFunction ("sim_rrhh_formacion_empleado",$camposinsert,$datosInsert);
			}
		}
		if ($ssoption == 2) {
			$sqlp = "select id from sim_rrhh_puesto_trabajo where visible=1 and id_departamento=".$_POST["id_departamento"];
			$resultp = mysqli_query($dbhandle,convertSQL($sqlp));
			while ($rowp = mysqli_fetch_array($resultp)){
				$sql = "select id_empleado,fecha_baja from sim_rrhh_empleado_puesto where visible=1 and id_puesto=".$rowp["id"];
				$result = mysqli_query($dbhandle,convertSQL($sql));
				$row = mysqli_fetch_array($result);
				$sqlt2 = "select count(*) as total from sim_rrhh_formacion_empleado where visible=1 and id_curso=".$_GET["id"]." and id_empleado=".$row["id_empleado"];
				$resultt2 = mysqli_query($dbhandle,convertSQL($sqlt2));
				$rowt2 = mysqli_fetch_array($resultt2);
				if ($rowt2["total"] == 0) {
		...
		if ($ssoption == 3) {
			$sql = "update sim_rrhh_formacion_empleado set visible=0 WHERE id_curso=".$_GET["id"]." and id_empleado=".$_GET["id_empleado"];
			mysqli_query($dbhandle,convertSQL($sql));
#			echo $sql;
		}
		$sqlf = "select nombre from sim_rrhh_formacion where visible=1 and id=".$_GET["id"];
		$resultf = mysqli_query($dbhandle,convertSQL($sqlf));
		$rowf = mysqli_fetch_array($resultf);
					...
					echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
						echo "<tr>";
							...
							echo "<td><select name=\"id_empleado\" style=\"width:200px\">";
								echo "<option value=\"0\">-</option>";
								$sql = "select id,nombre from sim_rrhh_empleado where visible=1";
								$result = mysqli_query($dbhandle,convertSQL($sql));
								while ($row = mysqli_fetch_array($result)){
						...
						echo "</tr><tr>";
							...
							echo "<td><select name=\"id_departamento\" style=\"width:200px\">";
								echo "<option value=\"0\">-</option>";
								$sqld = "select id,departamento from sim_rrhh_departamento where visible=1";
								$resultd = mysqli_query($dbhandle,convertSQL($sqld));
								while ($rowd = mysqli_fetch_array($resultd)){
									echo "<option value=\"".$rowd["id"]."\">".$rowd["departamento"]."</option>";
								}
				...
				echo "<td style=\"vertical-align:top;\">";
					echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
						echo "<tr style=\"background-color: Silver;\">";
							...
							echo "<th>".$Empleados."</th>";
						echo "</tr>";
						$sqle = "select id,nombre from sim_rrhh_empleado where visible=1 and id in (select id_empleado from sim_rrhh_formacion_empleado where visible=1 and id_curso=".$_GET["id"].")";
						$resulte = mysqli_query($dbhandle,convertSQL($sqle));
						while ($rowe = mysqli_fetch_array($resulte)){
							echo "<tr>";
...
{
	global $db,$dbhandle;
	$sqlp = "select id from sim_rrhh_empleado_puesto where visible=1 and fecha_baja='0000-00-00' and id_puesto in (select id from sim_rrhh_puesto_trabajo where visible=1 and activo=1 and id=".$id.")";
	$resultp = mysqli_query($dbhandle,convertSQL($sqlp));
	while ($rowp = mysqli_fetch_array($resultp)){
		if ($rowp){
			return 1;
...
{ 
	...
	echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\" style=\"width:100%;\">";
		echo "<tr>";
		$sql = "select id,departamento from sim_rrhh_departamento where visible=1 and id_departamento=".$id." order by departamento";
		$result = mysqli_query($dbhandle,convertSQL($sql));
		while ($row = mysqli_fetch_array($result)){
			echo "<td style=\";vertical-align:top;\">";
				echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\" style=\"width:100%;border-right:1px solid black;border-left:1px solid black\">";
					...
					echo "<tr><td>&nbsp;</td></tr>";
					if ($emple == 1){
						$sqlp = "select id,puesto from sim_rrhh_puesto_trabajo where visible=1 and id_departamento=".$row["id"]." order by puesto";
						$resultp = mysqli_query($dbhandle,convertSQL($sqlp));
						while ($rowp = mysqli_fetch_array($resultp)){
							$sqle = "select nombre from sim_rrhh_empleado where visible=1 and id=(select id_empleado from sim_rrhh_empleado_puesto where visible=1 and id_puesto=".$rowp["id"]." and fecha_baja= '0000-00-00')";
							$resulte = mysqli_query($dbhandle,convertSQL($sqle));
