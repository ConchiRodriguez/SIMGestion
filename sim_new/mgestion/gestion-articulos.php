...
if (($option == 1004) AND ($autorizado == true)) {
	...
	if (($soption == 0) or ($soption == 200)) {
		if ($ssoption == 1) {
			$sql = "select count(*) as total from sim_articulos where codigo='".$_POST["codigo"]."' or nombre='".$_POST["nombre"]."'";
		...
		if ($ssoption == 3) {
			...
			$datosUpdate=array(0);
			updateFunction("sim_articulos",$_GET["id"],$camposUpdate,$datosUpdate);
		}
		if ($soption == 0) {
			echo "<h4>".$Articulos."</h4>";
			echo "<table><tr>";
				echo "<td class=\"menu\">";
					if ($_GET["ver"] == 0) { 
						$sql = "select * from sim_users where activo=1 order by usuario";
						echo "<a href=\"index.php?op=1004&sop=0&ver=1\" style=\"color:white;\">".$Descatalogados."</a>";
					}
					if ($_GET["ver"] == 1) { 
						$sql = "select * from sim_users order by usuario";
...
#					echo "<th>".$Fecha."</th>";
			...
			if ($soption == 200) {
				...
				echo "<tr>";
					...
					echo "<td><select name=\"id_subgrupo\" style=\"width:300px\">";
						...
						$sqli = "select id,grupo from sim_articulos_grupos";
						$resulti = mysqli_query($dbhandle,convertSQL($sqli));
						while ($rowi = mysqli_fetch_array($resulti)) {
							$sqls = "select id,subgrupo from sim_articulos_subgrupos where id_grupo=".$rowi["id"];
				...
				echo "<tr>";
					...
					echo "<td><select name=\"id_subgrupo\" style=\"width:300px\">";
						...
						$sqlg1 = "select id,grupo from sim_articulos_grupos order by grupo";
						$resultg1 = mysqli_query($dbhandle,convertSQL($sqlg1));
						while ($rowg1 = mysqli_fetch_array($resultg1)) {
							$sqlsg1 = "select id,subgrupo from sim_articulos_subgrupos where id_grupo=".$rowg1["id"]." order by subgrupo";
		...
		if (($soption == 0) or (($soption == 200) and (($_POST["codigo"] != 0) or ($_POST["nombre"] != 0) or ($_POST["notas"] != 0) or ($_POST["id_subgrupo"] != 0)))) {
			$sql = "select id,id_subgrupo,codigo,nombre from sim_articulos where visible=1";
...
#			echo $sql;
			...
			while ($row = mysqli_fetch_array($result)) {
				$sqlsg = "select id,id_grupo,subgrupo from sim_articulos_subgrupos where id=".$row["id_subgrupo"]." order by subgrupo";
				$resultsg = mysqli_query($dbhandle,convertSQL($sqlsg));
				$rowsg = mysqli_fetch_array($resultsg);
				$sqlg = "select id,grupo from sim_articulos_grupos where id=".$rowsg["id_grupo"]." order by grupo";
				$resultg = mysqli_query($dbhandle,convertSQL($sqlg));
				$rowg = mysqli_fetch_array($resultg);
				$sqlst = "select pvd,pvp,id_divisa_pvd,id_divisa_pvp from sim_stock where vigente=1 and id_articulo=".$row["id"]."";
...
#						echo "<td>".$ultima_fecha."</td>";
					...
					$sqlsg1 = "select id_grupo,subgrupo from sim_articulos_subgrupos where id=".$row["id_subgrupo"];
					$resultsg1 = mysqli_query($dbhandle,convertSQL($sqlsg1));
					$rowsg1 = mysqli_fetch_array($resultsg1);
					$sqlg1 = "select grupo from sim_articulos_grupos where id=".$rowsg1["id_grupo"];
					$resultg1 = mysqli_query($dbhandle,convertSQL($sqlg1));
					$rowg1 = mysqli_fetch_array($resultg1);
					echo "<td>".$rowg1["grupo"]." - ".$rowsg1["subgrupo"]."</td>";
					$sqld = "select id,abrev from sim_divisas where visible=1 and id=".$rowst["id_divisa_pvd"];
					$resultd = mysqli_query($dbhandle,convertSQL($sqld));
					$rowd = mysqli_fetch_array($resultd);
					echo "<td style=\"text-align:right;\">".$rowst["pvd"]." ".$rowd["abrev"]."</td>";
					$sqld = "select id,abrev from sim_divisas where visible=1 and id=".$rowst["id_divisa_pvp"];
					$resultd = mysqli_query($dbhandle,convertSQL($sqld));
					$rowd = mysqli_fetch_array($resultd);
	...
	if (($soption >= 100) and ($soption < 200) and ($_GET["id"] > 0)) {
		...
		$sqld = "select abrev from sim_divisas where predefinido=1";
		$resultd = mysqli_query($dbhandle,convertSQL($sqld));
		$rowd = mysqli_fetch_array($resultd);
		$sql = "select id,codigo,nombre,id_subgrupo,escandall from sim_articulos where id=".$_GET["id"];
		$result = mysqli_query($dbhandle,convertSQL($sql));
		$row = mysqli_fetch_array($result);
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			echo "<tr>";
				...
				echo "<td style=\"vertical-align:top\">";
					echo "<table cellpadding=\"0\">";
						echo "<tr><td class=\"ficha\"><a href=\"index.php?op=1004&sop=130&id=".$row["id"]."\" style=\"color:white;\">".$Precios."</a></td></tr>";
				...
				echo "<td style=\"vertical-align : top;background-color : Silver; margin : 5px 10px 10px 10px;width:500px;padding : 5px 10px 10px 10px;\">";
					...
					$sqls = "select id_grupo,subgrupo from sim_articulos_subgrupos where id=".$row["id_subgrupo"];
					$results = mysqli_query($dbhandle,convertSQL($sqls));
					$rows = mysqli_fetch_array($results);
					$sqlg = "select grupo from sim_articulos_grupos where id=".$rows["id_grupo"];
					$resultg = mysqli_query($dbhandle,convertSQL($sqlg));
					$rowg = mysqli_fetch_array($resultg);
					$sqlst = "select pvd,pvp from sim_stock where vigente=1 and id_articulo=".$row["id"]."";
					$resultst = mysqli_query($dbhandle,convertSQL($sqlst));
					$rowst = mysqli_fetch_array($resultst);
	...
	if ($soption == 100) {
		if ($ssoption == 3) {
			...
			insertFunction ("sim_articulos_rel_proveedores",$camposInsert,$datosInsert);
		}
		if ($ssoption == 4) {
			$camposUpdate = array("id_proveedor","codigo_proveedor");
			$datosUpdate = array($_POST["id_proveedor"],$_POST["codigo_proveedor"]);
			updateFunction ("sim_articulos_rel_proveedores",$_GET["id_prov"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 5) {
			$camposUpdate=array('visible');
			$datosUpdate=array(0);
		...
		if ($ssoption == 6) {
			$sqlv = "select id from sim_articulos_rel_caracteristicas where id_articulo=".$_GET["id"];
			$resultv = mysqli_query($dbhandle,convertSQL($sqlv));
			while ($rowv = mysqli_fetch_array($resultv)) {
				$camposUpdate = array("valor");
				$datosUpdate = array($_POST["valor".$rowv["id"]]);
				updateFunction ("sim_articulos_rel_caracteristicas",$rowv["id"],$camposUpdate,$datosUpdate);
			}
		}

		$sqla = "select id,id_subgrupo,nombre,notas,codigo from sim_articulos where id=".$_GET["id"];
			...
			echo "<tr>";
				echo "<td style=\"vertical-align:top;\">";
					echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
						...
						$sqlsg1 = "select id_grupo,codigo from sim_articulos_subgrupos where id=".$rowa["id_subgrupo"];
						$resultsg1 = mysqli_query($dbhandle,convertSQL($sqlsg1));
						$rowsg1 = mysqli_fetch_array($resultsg1);
						$sqlg1 = "select codigo from sim_articulos_grupos where id=".$rowsg1["id_grupo"];
						$resultg1 = mysqli_query($dbhandle,convertSQL($sqlg1));
						$rowg1 = mysqli_fetch_array($resultg1);
						...
						echo "<tr><th style=\"text-align:right;vertical-align:top;\">".$Familia."</th><td><select name=\"id_subgrupo\" style=\"width:300px\">";
							echo "<option value=\"0\">".$Sin." ".$Clasificar."</option>";
							$sqlg2 = "select id,grupo from sim_articulos_grupos order by grupo";
							$resultg2 = mysqli_query($dbhandle,convertSQL($sqlg2));
							while ($rowg2 = mysqli_fetch_array($resultg2)) {
								$sqlsg2 = "select id,subgrupo from sim_articulos_subgrupos where id_grupo=".$rowg2["id"]." order by subgrupo";
						...
						echo "<tr><td></td><td><input type=\"Submit\" value=\"".$Modificar."\" style=\"width:300px\"></td></tr>";
						echo "</form>";
					echo "</table>";
					...
					echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
						...
						echo "</tr><tr>";
							...
							echo "<td><select name=\"id_proveedor\" style=\"width:300px\">";
								$sqlg1 = "select id,nombre from sim_clients where visible=1 order by nombre";
								$resultg1 = mysqli_query($dbhandle,convertSQL($sqlg1));
								while ($rowg1 = mysqli_fetch_array($resultg1)) {
						...
						echo "</form>";
						echo "<tr><td>&nbsp;</td></tr>";
						$sqlp = "select id,codigo_proveedor,id_proveedor from sim_articulos_rel_proveedores where id_articulo=".$_GET["id"]." and visible=1";
						...
						echo "</tr>";
						$sqlf = "select id_caracteristica from sim_articulos_subgrupos_caracteristicas where id_subgrupo=".$rowa["id_subgrupo"]."";
						$resultf = mysqli_query($dbhandle,convertSQL($sqlf));
						while ($rowf = mysqli_fetch_array($resultf)) {
							$sqlc = "select id,nombre,valor,unidad,unidad_abr from sim_articulos_caracteristicas where id_caracteristica=0 and id=".$rowf["id_caracteristica"]." and visible=1 order by nombre";
							...
							## CARACTERISTICAS
								echo "<tr style=\"background-color : Silver;\">";
									echo "<td colspan=\"2\"><strong>".$rowc["nombre"]."</strong></td>";
										...
										$valor_inicial = false;
										$sqlvt = "select count(*) as total from sim_articulos_rel_caracteristicas where id_articulo=".$rowa["id"]." and id_valor=".$rowc["id"];
										$resultvt = mysqli_query($dbhandle,convertSQL($sqlvt));
										$rowvt = mysqli_fetch_array($resultvt);
										if ($rowvt["total"] == 0) {
											$sql = "insert into sim_articulos_rel_caracteristicas (id_articulo,id_valor,valor) values (".$rowa["id"].",".$rowc["id"].",'".$rowc["valor"]."')";
											mysqli_query($dbhandle,convertSQL($sql));
											$valor_inicial = true;
										}
										$sqlv = "select id,valor from sim_articulos_rel_caracteristicas where id_articulo=".$rowa["id"]." and id_valor=".$rowc["id"];
										$resultv = mysqli_query($dbhandle,convertSQL($sqlv));
										...
										$tabla = false;
										$sqlvt = "select count(*) as total from sim_articulos_caracteristicas_tablas where visible=1 and  id_caracteristica=".$rowc["id"];
										$resultvt = mysqli_query($dbhandle,convertSQL($sqlvt));
										$rowvt = mysqli_fetch_array($resultvt);
									...
									if ($tabla == false) {	echo "<td><input type=\"Text\" name=\"valor".$rowv["id"]."\" value=\"".$valor."\" style=\"width:300px\"></td>"; }
										...
										$sqlvta = "select id,valor from sim_articulos_caracteristicas_tablas where visible=1 and  id_caracteristica=".$rowc["id"];
								...
								echo "</tr>";
								###################################### SUB
								$sqlsc = "select id,nombre,valor,unidad,unidad_abr from sim_articulos_caracteristicas where id_caracteristica=".$rowc["id"]." and visible=1 order by nombre";
								...
								## CARACTERISTICAS
									echo "<tr style=\"background-color : white;\">";
										echo "<td colspan=\"2\"><strong>".$rowsc["nombre"]."</strong></td>";
											### RECUPERO EL VALOR O PONGO EL PREDEFINIDO
											$valor_inicial = false;
											$sqlvt = "select count(*) as total from sim_articulos_rel_caracteristicas where id_articulo=".$rowa["id"]." and id_valor=".$rowsc["id"];
											$resultvt = mysqli_query($dbhandle,convertSQL($sqlvt));
											$rowvt = mysqli_fetch_array($resultvt);
											if ($rowvt["total"] == 0) {
												$sql = "insert into sim_articulos_rel_caracteristicas (id_articulo,id_valor,valor) values (".$rowa["id"].",".$rowsc["id"].",'".$rowsc["valor"]."')";
												mysqli_query($dbhandle,convertSQL($sql));
												$valor_inicial = true;
											}
											$sqlv = "select id,valor from sim_articulos_rel_caracteristicas where id_articulo=".$rowa["id"]." and id_valor=".$rowsc["id"];
											$resultv = mysqli_query($dbhandle,convertSQL($sqlv));
											$rowv = mysqli_fetch_array($resultv);
											$valor = $rowv["valor"];
											...
											### COMPROBACION DE TABLA SI / NO
											$tabla = false;
											$sqlvt = "select count(*) as total from sim_articulos_caracteristicas_tablas where visible=1 and  id_caracteristica=".$rowsc["id"];
											$resultvt = mysqli_query($dbhandle,convertSQL($sqlvt));
											$rowvt = mysqli_fetch_array($resultvt);
											if ($rowvt["total"] >= 1) { $tabla = true;} else { $tabla = false; }
											### ECHO EN FUNCION DE TABLA SI / NO
										if ($tabla == false) {	echo "<td><input type=\"Text\" name=\"valor".$rowv["id"]."\" value=\"".$valor."\" style=\"width:300px\"></td>"; }
											...
											echo "<td><select name=\"valor".$rowv["id"]."\" style=\"width:300px\">";
											$sqlvta = "select id,valor from sim_articulos_caracteristicas_tablas where visible=1 and  id_caracteristica=".$rowsc["id"];
											$resultvta = mysqli_query($dbhandle,convertSQL($sqlvta));
											while ($rowvta = mysqli_fetch_array($resultvta)) {
												if ($rowvta["id"] == $valor) { echo "<option value=\"".$rowvta["id"]."\" selected>".$rowvta["valor"]."</option>"; }
													else { echo "<option value=\"".$rowvta["id"]."\">".$rowvta["valor"]."</option>"; }
	...
	if ($soption == 105) {
		...
		echo "<table cellpadding=\"1\" cellspacing=\"1\" class=\"lista\">";
			echo "<tr>";
				...
				echo "</td>";
				echo "<td style=\"vertical-align:top;Width:33%;text-align:center;background-color:silver;\">";
					$sql = "select descatalogat from sim_articulos where id=".$_GET["id"];
					$result = mysqli_query($dbhandle,convertSQL($sql));
					$row = mysqli_fetch_array($result);
	...
	if ($soption == 106) {
		$sql = "select * from sim_articulos where id=".$_GET["id"];
		$result = mysqli_query($dbhandle,convertSQL($sql));
		$row = mysqli_fetch_array($result);
		$sqlgc = "select codigo from sim_articulos where codigo<>'' order by codigo desc";
		...
		$sqlart = "select id from sim_articulos where visible=1 order by id desc";
		$resultart = mysqli_query($dbhandle,convertSQL($sqlart));
		$rowart = mysqli_fetch_array($resultart);

		$sqles = "select id_articulo,unidades,visible from sim_articulos_escandall where visible=1 and id_escandall=".$_GET["id"];
		$resultes = mysqli_query($dbhandle,convertSQL($sqles));
		while ($rowes = mysqli_fetch_array($resultes)) {
			$camposInsert = "id_escandall,id_articulo,unidades,visible";
			$datosInsert = array($rowart["id"],$rowes["id_articulo"],$rowes["unidades"],$rowes["visible"]);
			insertFunction ("sim_articulos_escandall",$camposInsert,$datosInsert);
		}

		$sqlcl = "select id_client,stock_max,stock_min from sim_articulos_clients where id_articulo=".$_GET["id"];
		$resultcl = mysqli_query($dbhandle,convertSQL($sqlcl));
		while ($rowcl = mysqli_fetch_array($resultcl)) {
			$camposInsert = "id_client,id_articulo,stock_max,stock_min";
			$datosInsert = array($rowcl["id_client"],$rowart["id"],$rowcl["stock_max"],$rowcl["stock_min"]);
			insertFunction ("sim_articulos_clients",$camposInsert,$datosInsert);
		}

		$sqlco = "select * from sim_articulos_costos where visible=1 and id_articulo=".$_GET["id"];
		$resultco = mysqli_query($dbhandle,convertSQL($sqlco));
		while ($rowco = mysqli_fetch_array($resultco)) {
			$camposInsert = "id_articulo,preu_cost,pes_cost,visible,descripcio,temps,fecha,id_cuerpo,aprovat";
			...
			insertFunction ("sim_articulos_costos",$camposInsert,$datosInsert);

			$sqlcost = "select id from sim_articulos_costos order by id desc";
			$resultcost = mysqli_query($dbhandle,convertSQL($sqlcost));
			$rowcost = mysqli_fetch_array($resultcost);


			$sqlfr = "select * from sim_articulos_fases_recursos where visible=1 and id_coste=".$rowco["id"];
			$resultfr = mysqli_query($dbhandle,convertSQL($sqlfr));
			while ($rowfr = mysqli_fetch_array($resultfr)) {
				$camposInsert = "id_fase,id_recurso,id_coste,temps,ordre,visible,preu_fases,calc_temps";
				...
				insertFunction ("sim_articulos_fases_recursos",$camposInsert,$datosInsert);

				$sqlfare = "select id from sim_articulos_fases_recursos order by id desc";
				$resultfare = mysqli_query($dbhandle,convertSQL($sqlfare));
				$rowfare = mysqli_fetch_array($resultfare);

				$sqlest = "select orden,descripcio,visible from sim_articulos_estacada where visible=1 and id_fase_recurso=".$rowfr["id"];
				$resultest = mysqli_query($dbhandle,convertSQL($sqlest));
				while ($rowest = mysqli_fetch_array($resultest)) {
					...
					insertFunction ("sim_articulos_estacada",$camposInsert,$datosInsert);

					$sqlesta = "select id from sim_articulos_estacada order by id desc";
					$resultesta = mysqli_query($dbhandle,convertSQL($sqlesta));
					$rowesta = mysqli_fetch_array($resultesta);

					$sqlop = "select * from sim_articulos_operaciones visible=1 and where id_estacada=".$rowest["id"];
					$resultop = mysqli_query($dbhandle,convertSQL($sqlop));
					while ($rowop = mysqli_fetch_array($resultop)) {
						$camposInsert = "id_operacion,id_estacada,temps,descripcio,visible";
						$datosInsert = array($rowop["id_operacion"],$rowesta["id"],$rowop["temps"],$rowop["descripcio"],$rowop["visible"]);
						insertFunction ("sim_articulos_operaciones",$camposInsert,$datosInsert);
					}
				}
			}
		}

		$sqlse = "select * from sim_articulos_seccions where visible=1 and id_articulo=".$_GET["id"];
		$resultse = mysqli_query($dbhandle,convertSQL($sqlse));
		while ($rowse = mysqli_fetch_array($resultse)) {
			$camposInsert = "id_articulo,id_seccion,id_estanteria,id_pasillo,id_almacen,visible";
			$datosInsert = array($rowse["id_articulo"],$rowse["id_seccion"],$rowse["id_estanteria"],$rowse["id_pasillo"],$rowse["id_almacen"],$rowse["visible"]);
			insertFunction ("sim_articulos_seccions",$camposInsert,$datosInsert);
		}

		$sqlca = "select id_valor,valor from sim_articulos_rel_caracteristicas where id_articulo=".$_GET["id"];
		...
		while ($rowca = mysqli_fetch_array($resultca)) {
			...
			insertFunction ("sim_articulos_rel_caracteristicas",$camposInsert,$datosInsert);
		}

		$sqlpr = "select * from sim_articulos_rel_proveedores where visible=1 and id_articulo=".$_GET["id"];
		...
		$sqlst = "select * from sim_stock where id_articulo=".$_GET["id"];
		$resultst = mysqli_query($dbhandle,convertSQL($sqlst));
		while ($rowst = mysqli_fetch_array($resultst)) {
			$camposInsert = "id_articulo,id_almacen,unidades,pvd,pvp,web,vigente,fecha,id_usuario,id_compte_entradas";
			$datosInsert = array($rowart["id"],$rowst["id_almacen"],$rowst["unidades"],$rowst["pvd"],$rowst["pvp"],$rowst["web"],$rowst["vigente"],$rowst["fecha"],$rowst["id_usuario"],$rowst["id_compte_entradas"]);
			insertFunction ("sim_stock",$camposInsert,$datosInsert);
		}
		echo "<center>";
			...
			echo "<tr>";
				echo "<td style=\"width:70%;vertical-align:top;\">";
					echo "<h4>".$Archivos." :</h4>";
						...
						$result = mysqli_query($dbhandle,convertSQL($sql));
						while ($row = mysqli_fetch_array($result)) {
							$sqlele = "select id,name,size from sim_files where id_tipo=".$row["id"]." and tipo_id_elemento=1 and id_elemento=".$_GET["id"];
				...
				echo "</td><td style=\"width:200px;vertical-align:top;\">";
					...
					echo "<select name=\"id_tipo\" style=\"width:200px\">";
						$sql = "select id,nombre,limite_kb from sim_files_tipos order by nombre";
						$result = mysqli_query($dbhandle,convertSQL($sql));
						while ($row = mysqli_fetch_array($result)) {
	...
	if ($soption == 120) {
		if ($ssoption == 1) {
			...
			$lim_tamano = 200*1000;
			$sqlt = "select count(*) as total from sim_articulos where img1='".$archivo_name."' or img2='".$archivo_name."' or img3='".$archivo_name."'";
		...
		$sqla = "select id,img1,img2,img3 from sim_articulos where id=".$_GET["id"];
		$resulta = mysqli_query($dbhandle,convertSQL($sqla));
		$rowa = mysqli_fetch_array($resulta);
		$id_articulo = $rowa["id"];
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
	...
	if ($soption == 130) {
		if ($ssoption == 1){
			$camposInsert = "id_articulo,unidades,pvd,pvp,id_divisa_pvd,id_divisa_pvp,vigente,fecha,id_usuario";
		...
		if ($ssoption == 2) {
			$sql = "update sim_stock set ";
			$sql = $sql."vigente=0";
			$sql = $sql." WHERE id_articulo=".$_GET["id"]."";
			mysqli_query($dbhandle,convertSQL($sql));
		...
		if ($ssoption == 3) {
			...
			updateFunction ("sim_stock",$_GET["id_stock"],$camposUpdate,$datosUpdate);
		}
		...
		echo "</table>";
		echo "<br><br>";
		$sqlt = "select count(*) as total from sim_stock where id_articulo=".$_GET["id"];
		$resultt = mysqli_query($dbhandle,convertSQL($sqlt));
		$rowt = mysqli_fetch_array($resultt);
		$sqlst = "select count(*) as total from sim_stock where id_articulo=".$_GET["id"]." and vigente=1";
		$resultst = mysqli_query($dbhandle,convertSQL($sqlst));
		$rowst = mysqli_fetch_array($resultst);
		$vigente = 1;
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "<tr>";
				...
				echo "<td><select name=\"id_divisa_pvd\" type=\"Text\" style=\"width:100px\">";
					echo "<option value=\"0\">-</option>";
					$sqlp= "select id,divisa from sim_divisas";
					$resultp = mysqli_query($dbhandle,convertSQL($sqlp));
					while ($rowp = mysqli_fetch_array($resultp)){
			...
			$totalpvd = 0;
			$totalpvp = 0;
			$sqls = "select * from sim_stock where id_articulo=".$_GET["id"]." order by fecha desc, id asc";
			$results = mysqli_query($dbhandle,convertSQL($sqls));
			while ($rows = mysqli_fetch_array($results)) {
				$color = "white";
	...
	if ($soption == 135) {
		if ($ssoption == 1) {
			$camposInsert = "id_articulo,id_client,stock_min,stock_max";
			$datosInsert = array($_GET["id"],$_POST["id_client"],$_POST["stock_min"],$_POST["stock_max"]);
			insertFunction ("sim_articulos_clients",$camposInsert,$datosInsert);
		}
		if ($ssoption == 2) {
			$camposUpdate = array("id_client","stock_min","stock_max");
			$datosUpdate = array($_POST["id_client"],$_POST["stock_min"],$_POST["stock_max"]);
			updateFunction ("sim_articulos_clients",$_GET["id_stock"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 3){
			deleteFunction ("sim_articulos_clients",$_GET["id_stock"]);
		}

		echo "<h4>".$Stock." ".$Clientes." : </h4>";
		echo boton(array("op=1004&sop=130&id=".$_GET["id"]),array("&laquo; ".$Volver));
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			echo "<tr style=\"background-color: Silver;\">";
				echo "<th></th>";
				echo "<th>".$Cliente."</th>";
				echo "<th>".$Stock." ".$Minimo."</th>";
				echo "<th>".$Stock." ".$Maximo."</th>";
				echo "<th></th>";
			echo "</tr>";
			echo "<tr>";
				echo "<form action=\"index.php?op=1004&sop=135&ssop=1&id=".$_GET["id"]."\" method=\"post\">";
				echo "<td></td>";
				echo "<td><select name=\"id_client\" type=\"Text\" style=\"width:400px\">";
					echo "<option value=\"0\">-</option>";
					$sqlp= "select id,nombre from sim_clients";
					$resultp = mysqli_query($dbhandle,convertSQL($sqlp));
					while ($rowp = mysqli_fetch_array($resultp)){
			...
			echo "</tr>";
			echo "<tr><td>&nbsp;</td></tr>";
			$sql= "select id,id_client,stock_min,stock_max from sim_articulos_clients";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)){
				echo "<tr>";
				echo "<td><a href=\"index.php?op=1004&sop=136&id_stock=".$row["id"]."&id=".$_GET["id"]."\"><img src=\"mgestion/pics/icons-mini/page_white_delete.png\" border=\"0\"></a></td>";
	...
	if ($soption == 136) {
		echo "<center>";
		echo "<br><br>".$pregunta_eliminar;
		echo boton(array("op=1004&sop=135&ssop=3&id_stock=".$_GET["id_stock"]."&id=".$_GET["id"],"op=1004&sop=135&id=".$_GET["id"]),array($Si,$No));
		echo "</center>";
	}

	if ($soption == 140) {
		if ($ssoption == 1) {
			$sqls = "select id_estanteria,id from sim_stock_almacenes_pasillo_estanteria_seccion where id=".$_POST["id_seccion"];
			$results = mysqli_query($dbhandle,convertSQL($sqls));
			$rows = mysqli_fetch_array($results);
			$sqle = "select id_pasillo,id from sim_stock_almacenes_pasillo_estanteria where id=".$rows["id_estanteria"];
			$resulte = mysqli_query($dbhandle,convertSQL($sqle));
			$rowe = mysqli_fetch_array($resulte);
			$sqlp = "select id_almacen,id from sim_stock_almacenes_pasillo where id=".$rowe["id_pasillo"];
			$resultp = mysqli_query($dbhandle,convertSQL($sqlp));
			$rowp = mysqli_fetch_array($resultp);
			$sqla = "select id from sim_stock_almacenes where id=".$rowp["id_almacen"];
			$resulta = mysqli_query($dbhandle,convertSQL($sqla));
			$rowa = mysqli_fetch_array($resulta);
			$camposUpdate = "id_articulo,id_seccion,id_estanteria,id_pasillo,id_almacen";
			$datosInsert = array($_GET["id"],$rows["id"],$rowe["id"],$rowp["id"],$rowa["id"]);
			insertFunction ("sim_articulos_seccions",$camposUpdate,$datosInsert);
		}
		if ($ssoption == 3) {
			$camposUpdate = array("visible");
			$datosUpdate = array("0");
			updateFunction ("sim_articulos_seccions",$_GET["id_seccion_articulo"],$camposUpdate,$datosUpdate);
		}
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "</tr><tr>";
				...
				$sqla = "select id,nombre from sim_stock_almacenes order by nombre";
				$resulta = mysqli_query($dbhandle,convertSQL($sqla));
				while ($rowa = mysqli_fetch_array($resulta)) {
					$sqlp = "select id,nombre from sim_stock_almacenes_pasillo where visible=1 and id_almacen=".$rowa["id"]." order by orden";
					$resultp = mysqli_query($dbhandle,convertSQL($sqlp));
					while ($rowp = mysqli_fetch_array($resultp)) {
						$sqle = "select id,nombre from sim_stock_almacenes_pasillo_estanteria where visible=1 and id_pasillo=".$rowp["id"]." order by orden";
						$resulte = mysqli_query($dbhandle,convertSQL($sqle));
						while ($rowe = mysqli_fetch_array($resulte)) {
							$sqls = "select id,nombre from sim_stock_almacenes_pasillo_estanteria_seccion where visible=1 and id_estanteria=".$rowe["id"]." order by orden";
							$results = mysqli_query($dbhandle,convertSQL($sqls));
							while ($rows = mysqli_fetch_array($results)) {
		...
		echo "<br><br><br>";
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			$sqlas = "select id_seccion,id_almacen,id from sim_articulos_seccions where id_articulo=".$_GET["id"]." and visible=1";
			$resultas = mysqli_query($dbhandle,convertSQL($sqlas));
			while ($rowas = mysqli_fetch_array($resultas)) {
				$sqls = "select id_estanteria,nombre from sim_stock_almacenes_pasillo_estanteria_seccion where id=".$rowas["id_seccion"];
				$results = mysqli_query($dbhandle,convertSQL($sqls));
				$rows = mysqli_fetch_array($results);
				$sqle = "select id_pasillo,nombre from sim_stock_almacenes_pasillo_estanteria where id=".$rows["id_estanteria"];
				$resulte = mysqli_query($dbhandle,convertSQL($sqle));
				$rowe = mysqli_fetch_array($resulte);
				$sqlp = "select id_almacen,nombre,id from sim_stock_almacenes_pasillo where id=".$rowe["id_pasillo"];
				$resultp = mysqli_query($dbhandle,convertSQL($sqlp));
				$rowp = mysqli_fetch_array($resultp);
				$sqla = "select nombre from sim_stock_almacenes where id=".$rowp["id_almacen"];
				$resulta = mysqli_query($dbhandle,convertSQL($sqla));
				$rowa = mysqli_fetch_array($resulta);
				echo "<tr style=\"background-color:silver;\">";
					echo "<th>".$Eliminar."</th>";
				...
				echo "</tr><tr>";
					echo "<td></td>";
					echo "<td>";
						$sqlp = "select id,nombre from sim_stock_almacenes_pasillo where id=".$rowp["id"];
						$resultp = mysqli_query($dbhandle,convertSQL($sqlp));
						while ($rowp = mysqli_fetch_array($resultp)) {
							echo "Pasillo : <strong>".$rowp["nombre"]."</strong><br><br>";
							echo "<table cellpadding=\"0\" cellspacing=\"0\">";
							$sqlet = "select count(*) as total from sim_stock_almacenes_pasillo_estanteria where id_pasillo=".$rowp["id"]." and visible=1 order by orden";
							...
							if ($rowet["total"] == 0) {
								...
								$sqle = "select id,nombre from sim_stock_almacenes_pasillo_estanteria where id_pasillo=".$rowp["id"]." and visible=1 order by orden";
								$resulte = mysqli_query($dbhandle,convertSQL($sqle));
								while ($rowe = mysqli_fetch_array($resulte)) {
									...
									$sqlst = "select count(*) as total from sim_stock_almacenes_pasillo_estanteria_seccion where id_estanteria=".$rowe["id"]." and visible=1 order by orden";
									$resultst = mysqli_query($dbhandle,convertSQL($sqlst));
									$rowst = mysqli_fetch_array($resultst);
									if ($rowst["total"] == 0) {
										echo "<td style=\"width:100%;background-color: Black;height:25px\">&nbsp;</td>";
									} else {
										$sqls = "select id,porcentage from sim_stock_almacenes_pasillo_estanteria_seccion where id_estanteria=".$rowe["id"]." and visible=1 order by orden";
										$results = mysqli_query($dbhandle,convertSQL($sqls));
										while ($rows = mysqli_fetch_array($results)) {
	...
	if ($soption == 145) {
		$sql = "select id,nombre from sim_stock_almacenes where id=".$_GET["id_almacen"];
		$result = mysqli_query($dbhandle,convertSQL($sql));
		$row = mysqli_fetch_array($result);
	...
	if ($soption == 150) {
		$sql= "select id from sim_articulos where codigo='".$_POST["codigo"]."'";
		$result = mysqli_query($dbhandle,convertSQL($sql));
		$row = mysqli_fetch_array($result);
		if ($ssoption == 1) {
			$sqls= "select pvd,pvp from sim_stock where id_articulo=".$row["id"]." and vigente=1";
			$results = mysqli_query($dbhandle,convertSQL($sqls));
			$rows = mysqli_fetch_array($results);
			if ($_GET["id"] != $row["id"]){
				$camposUpdate = "id_escandall,id_articulo,unidades,pvd_forzado,pvp_forzado";
				if ($rows["pvd"] != 0){ $pvd = $rows["pvd"]; } else { $pvd = 0; }
				if ($rows["pvp"] != 0){ $pvp = $rows["pvp"]; } else { $pvp = 0; }
				$datosInsert = array($_GET["id"],$row["id"],$_POST["unidades"],$pvd,$pvp);
				insertFunction ("sim_articulos_escandall",$camposUpdate,$datosInsert);
		...
		if ($ssoption == 2) {
			if ($_GET["id"] != $row["id"]){
				$camposUpdate = array("id_articulo","unidades","pvd_forzado","pvp_forzado");
				$datosUpdate = array($row["id"],$_POST["unidades"],$_POST["pvd_forzado"],$_POST["pvp_forzado"]);
				updateFunction ("sim_articulos_escandall",$_GET["id_escandall"],$camposUpdate,$datosUpdate);
			} else {
				echo mensageError($ErrorEscandallo);
			}
		}
		if ($ssoption == 3) {
			...
			$sqle= "select count(*) as total from sim_articulos where id=".$_GET["id"]." and escandall=1";
			$resulte = mysqli_query($dbhandle,convertSQL($sqle));
			$rowe = mysqli_fetch_array($resulte);
			if ($rowe["total"]==0){
				$camposUpdate = array("escandall");
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			echo "<tr>";
				...
				$sql = "select recalc_escandall from sim_articulos where id=".$_GET["id"];
				$result = mysqli_query($dbhandle,convertSQL($sql));
				$row = mysqli_fetch_array($result);
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "<tr>";
				...
				echo "<td><input name=\"unidades\" type=\"Text\" style=\"width:70px\"></td>";
			...
			echo "<tr><td>&nbsp;</td></tr>";
			$total=0;
			$sqle= "select * from sim_articulos_escandall where id_escandall=".$_GET["id"]." and visible=1";
			$resulte = mysqli_query($dbhandle,convertSQL($sqle));
			while ($rowe = mysqli_fetch_array($resulte)){
				echo "<tr>";
					...
					$sql= "select id,nombre,codigo from sim_articulos where id=".$rowe["id_articulo"]."";
					$result = mysqli_query($dbhandle,convertSQL($sql));
					$row = mysqli_fetch_array($result);
					...
					echo "<td><input name=\"unidades\" type=\"Text\" value=\"".$rowe["unidades"]."\" style=\"width:70px;text-align:right;\"></td>";
					$sqls= "select pvd,pvp from sim_stock where id_articulo=".$row["id"]." and vigente=1";
					$results = mysqli_query($dbhandle,convertSQL($sqls));
					$rows = mysqli_fetch_array($results);
					echo "<td style=\"text-align:right;width:70px;\">".$rows["pvd"]."</td>";
					echo "<td style=\"text-align:right;width:70px;\">".$rows["pvp"]."</td>";
					echo "<td><input name=\"pvd_forzado\" type=\"Text\" value=\"".$rowe["pvd_forzado"]."\" style=\"width:70px;text-align:right;\"></td>";
					echo "<td><input name=\"pvp_forzado\" type=\"Text\" value=\"".$rowe["pvp_forzado"]."\" style=\"width:70px;text-align:right;\"></td>";
					$_total=$rowe["unidades"]*$rowe["pvp_forzado"];
	...
	if (($soption == 510) and ($admin == true)) {
		if ($ssoption == 1){
			...
			insertFunction ("sim_articulos_grupos",$camposUpdate,$datosInsert);
		}
		if ($ssoption == 2){
			$sqlg = "select id from sim_articulos_grupos order by codigo";
			$resultg = mysqli_query($dbhandle,convertSQL($sqlg));
			while ($rowg = mysqli_fetch_array($resultg)) {
				#actualizar familia
				$camposUpdate = array("grupo","codigo");
				$datosUpdate = array($_POST["fgrupo".$rowg["id"]],$_POST["fcodigo".$rowg["id"]]);
				updateFunction ("sim_articulos_grupos",$rowg["id"],$camposUpdate,$datosUpdate);
				#actualizar todas las sufamilias
				$sqls = "select id from sim_articulos_subgrupos where id_grupo=".$rowg["id"]." order by codigo";
				$results = mysqli_query($dbhandle,convertSQL($sqls));
				while ($rows = mysqli_fetch_array($results)) {
					...
					updateFunction ("sim_articulos_subgrupos",$rows["id"],$camposUpdate,$datosUpdate);
				}
			}
		}
		if ($ssoption == 3){
			...
			insertFunction ("sim_articulos_subgrupos",$camposUpdate,$datosInsert);
		}
		if ($ssoption == 4) {
			### Pone a 0 el valor de todos articulos de las subfamilias afectadas
			$sqls = "select id from sim_articulos_subgrupos where id_grupo=".$_GET["id"];
			$results = mysqli_query($dbhandle,convertSQL($sqls));
			while ($rows = mysqli_fetch_array($results)) {
				$sql = "update sim_articulos set ";
			...
			$sql = "delete from sim_articulos_subgrupos WHERE id_grupo=".$_GET["id"];
			mysqli_query($dbhandle,convertSQL($sql));
			#### ELimina las familias
			$sql = "delete from sim_articulos_grupos WHERE id=".$_GET["id"];
			mysqli_query($dbhandle,convertSQL($sql));
		}
		if ($ssoption == 5) {
			### Pone a 0 el valor de todos articulos de las subfamilias afectadas
			...
			mysqli_query($dbhandle,convertSQL($sql));
			#### Elimina las subfamilias
			$sql = "delete from sim_articulos_subgrupos WHERE id=".$_GET["id"];
			mysqli_query($dbhandle,convertSQL($sql));
		}
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "<tr style=\"background-color: Silver;\">";
				...
				echo "<th><input type=\"Submit\" value=\"".$Anadir." ".$Familias."\" style=\"width:150px\"></th>";
				echo "</form>";
			echo "</tr>";
			echo "<tr style=\"background-color: Silver;\">";
				echo "<form action=\"index.php?op=1004&sop=510&ssop=3\" method=\"post\">";
				echo "<th><select name=\"id_grupo\" style=\"width:150px\">";
					$sqlg = "select id,grupo from sim_articulos_grupos order by grupo";
					$resultg = mysqli_query($dbhandle,convertSQL($sqlg));
					while ($rowg = mysqli_fetch_array($resultg)) {
				...
				echo "<th><input type=\"Submit\" value=\"".$Anadir." ".$Subfamilias."\" style=\"width:150px\"></th>";
				echo "</form>";
			echo "</tr>";
		echo "</table>";
		echo "<br><br>";
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			$sql = "select id,codigo,grupo from sim_articulos_grupos order by codigo";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)) {
				...
				echo "</tr>";
				$sqls = "select id,codigo,subgrupo,id_grupo from sim_articulos_subgrupos where id_grupo=".$row["id"]." order by codigo";
				$results = mysqli_query($dbhandle,convertSQL($sqls));
				while ($rows = mysqli_fetch_array($results)) {
					echo "<tr>";
	...
	if (($soption == 515) and ($admin == true)) {
		if ($ssoption == 1) {
			...
			insertFunction ("sim_articulos_subgrupos_caracteristicas",$camposInsert,$datosInsert);
		}
		if ($ssoption == 2) {
			$sql = "delete from sim_articulos_subgrupos_caracteristicas WHERE id_subgrupo=".$_GET["id"]." and id_caracteristica=".$_GET["id_car"]."";
			mysqli_query($dbhandle,convertSQL($sql));
		}
		...
		echo "<table cellspacing=\"0\ style=\"width:300px;\" class=\"lista\">";
			...
			echo "<tr>";
				...
				echo "<td><select name=\"id_caracteristica\" style=\"width:150px\">";
					$sqlg = "select id,nombre from sim_articulos_caracteristicas where visible=1 order by nombre";
					$resultg = mysqli_query($dbhandle,convertSQL($sqlg));
					while ($rowg = mysqli_fetch_array($resultg)) {
			...
			echo "</tr>";
			echo "<tr><td>&nbsp;</td></tr>";
			$sql = "select id_caracteristica from sim_articulos_subgrupos_caracteristicas where id_subgrupo=".$_GET["id"]."";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)) {
				$sqls = "select id,nombre from sim_articulos_caracteristicas where id=".$row["id_caracteristica"]." order by nombre";
				$results = mysqli_query($dbhandle,convertSQL($sqls));
				while ($rows = mysqli_fetch_array($results)) {
					echo "<tr>";
	...
	if (($soption == 520) and ($admin == true)) {
		if ($ssoption == 1) {
			...
			insertFunction ("sim_articulos_caracteristicas",$camposInsert,$datosInsert);
		}
		if ($ssoption == 2) {
			...
			updateFunction ("sim_articulos_caracteristicas",$_GET["id"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 3) {
			$camposUpdate=array('visible');
			$datosUpdate=array(0);
		...
		echo "<tr>";
			...
			echo "<td><select name=\"id_caracteristica\">";
				...
				$sqlg = "select id,nombre from sim_articulos_caracteristicas where id_caracteristica=0 and visible=1 order by nombre";
				$resultg = mysqli_query($dbhandle,convertSQL($sqlg));
				while ($rowg = mysqli_fetch_array($resultg)) {
					echo "<option value=\"".$rowg["id"]."\">".$rowg["nombre"]."</option>";
				}
		...
		echo "</tr>";
		echo "<tr><td>&nbsp;</td></tr>";
		$sql = "select * from sim_articulos_caracteristicas where id_caracteristica=0 and visible=1 order by nombre";
		$result = mysqli_query($dbhandle,convertSQL($sql));
		while ($row = mysqli_fetch_array($result)) {
			echo "<tr>";
				...
				echo "<td><select name=\"id_caracteristica\">";
					echo "<option value=\"0\">".$Sin_dependencias."</option>";
					$sqlg = "select id,nombre from sim_articulos_caracteristicas where id_caracteristica=0 and visible=1 and id<>".$row["id"]." order by nombre";
					$resultg = mysqli_query($dbhandle,convertSQL($sqlg));
					while ($rowg = mysqli_fetch_array($resultg)) {
				...
				echo "</form>";
				if ($row["valor"] == "") {
					$sqlg = "select count(*) as total from sim_articulos_caracteristicas_tablas where id_caracteristica=".$row["id"]." and visible=1";
					$resultg = mysqli_query($dbhandle,convertSQL($sqlg));
					$rowg = mysqli_fetch_array($resultg);
					if ($rowg["total"] == 0) {
			...
			$sql1 = "select * from sim_articulos_caracteristicas where id_caracteristica=".$row["id"]." and visible=1 order by nombre";
			...
			while ($row1 = mysqli_fetch_array($result1)) {
				echo "<tr>";
					...
					echo "</form>";
					if ($row1["valor"] == "") {
						$sqlg = "select count(*) as total from sim_articulos_caracteristicas_tablas where id_caracteristica=".$row1["id"]." and visible=1";
						$resultg = mysqli_query($dbhandle,convertSQL($sqlg));
						$rowg = mysqli_fetch_array($resultg);
						if ($rowg["total"] == 0) {
							echo "<td style=\"width:100px;height:5px;text-align:center;vertical-align:middle;background-color:#4B53AF;border:1px solid black\">";
	...
	if (($soption == 522) and ($admin == true)) {
		if ($ssoption == 1) {
			...
			insertFunction ("sim_articulos_caracteristicas_tablas",$camposUpdate,$datosInsert);
		}
		if ($ssoption == 2) {
			$camposUpdate = array("valor");
			$datosUpdate = array($_POST["valor"]);
			updateFunction ("sim_articulos_caracteristicas_tablas",$_GET["id"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 3) {
			$camposUpdate = array("visible");
			$datosUpdate = array("0");
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "</tr>";
			echo "<tr><td>&nbsp;</td></tr>";
			$sql1 = "select id,valor from sim_articulos_caracteristicas_tablas where id_caracteristica=".$_GET["id_caracteristica"]." and visible=1 order by valor";
			$result1 = mysqli_query($dbhandle,convertSQL($sql1));
			while ($row1 = mysqli_fetch_array($result1)) {
				echo "<tr>";
	...
	if (($soption == 530) and ($admin == true)) {
		if ($ssoption == 1) {
			...
			insertFunction ("sim_stock_almacenes",$camposInsert,$datosInsert);
		}
		if ($ssoption == 2) {
			...
			updateFunction ("sim_stock_almacenes",$_GET["id"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 3) {
			$camposUpdate=array('visible');
			$datosUpdate=array(0);
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\" width=\"80%\">";
			echo "<tr><td style=\"width:47%;vertical-align:top;\">";
				echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
					echo "<tr style=\"background-color: Silver;\">";
						...
						echo "<th>".$Editar."</th>";
					echo "</tr>";
					$sql = "select id,nombre,poblacion,telefono from sim_stock_almacenes where visible=1 order by nombre";
					$result = mysqli_query($dbhandle,convertSQL($sql));
					while ($row = mysqli_fetch_array($result)) {
					echo "<tr>";
			...
			echo "<td style=\"width:47%;vertical-align:top;\">";
				...
				if ($_GET["id"] == 0) {
					...
					$sql = "select * from sim_stock_almacenes where visible=1 and id=".$_GET["id"];
					$result = mysqli_query($dbhandle,convertSQL($sql));
					$row = mysqli_fetch_array($result);
					if ($_GET["id"] == 0) {
	...
	if (($soption == 532) and ($admin == true)) {
		if ($ssoption == 1) {
			...
			insertFunction ("sim_stock_almacenes_pasillo",$camposUpdate,$datosInsert);
		}
		if ($ssoption == 2) {
			$camposUpdate = array("nombre","orden");
			$datosUpdate = array($_POST["nombre"],$_POST["orden"]);
			updateFunction ("sim_stock_almacenes_pasillo",$_GET["id_pasillo"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 3) {
			$camposUpdate = array("visible");
			$datosUpdate = array("0");
		...
		if ($ssoption == 10) {
			...
			insertFunction ("sim_stock_almacenes_pasillo_estanteria",$camposUpdate,$datosInsert);
		}
		if ($ssoption == 20) {
			$camposUpdate = array("nombre","orden");
			$datosUpdate = array($_POST["nombre"],$_POST["orden"]);
			updateFunction ("sim_stock_almacenes_pasillo_estanteria",$_GET["id_estanteria"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 30) {
			$camposUpdate = array("visible");
			$datosUpdate = array("0");
		...
		if ($ssoption == 100) {
			...
			insertFunction ("sim_stock_almacenes_pasillo_estanteria_seccion",$camposUpdate,$datosInsert);
		}
		if ($ssoption == 200) {
			$camposUpdate = array("nombre","orden","porcentage");
			$datosUpdate = array($_POST["nombre"],$_POST["orden"],$_POST["porcentage"]);
			updateFunction ("sim_stock_almacenes_pasillo_estanteria_seccion",$_GET["id_seccion"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 300) {
			$camposUpdate = array("visible");
			$datosUpdate = array("0");
		...
		$sqla = "select nombre from sim_stock_almacenes where id=".$_GET["id"];
		$resulta = mysqli_query($dbhandle,convertSQL($sqla));
		$rowa = mysqli_fetch_array($resulta);
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "</tr>";
			echo "<tr><td>&nbsp;</td></tr>";
			$sql = "select id,nombre,orden from sim_stock_almacenes_pasillo where visible=1 and id_almacen=".$_GET["id"]." order by orden";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)) {
				echo "<tr>";
				...
				echo "</tr>";
				echo "<tr><td>&nbsp;</td></tr>";
				$sqlpe = "select id,nombre,orden from sim_stock_almacenes_pasillo_estanteria where visible=1 and id_pasillo=".$row["id"]." order by orden";
				...
				while ($rowpe = mysqli_fetch_array($resultpe)) {
					...
					echo "</tr>";
					echo "<tr><td>&nbsp;</td></tr>";
					$sqlpes = "select id,nombre,orden,porcentage from sim_stock_almacenes_pasillo_estanteria_seccion where visible=1 and id_estanteria=".$rowpe["id"]." order by orden";
	...
	if ($soption == 539) {
		$sql = "select id,nombre from sim_stock_almacenes where id=".$_GET["id"];
		$result = mysqli_query($dbhandle,convertSQL($sql));
		$row = mysqli_fetch_array($result);
		echo "<h4>".$row["nombre"]."</h4>";
		echo boton(array("op=1004&sop=530"),array("&laquo; ".$Volver));
...
/*
	if ($soption == 400) {
		echo "<table border=\"0\" cellspacing=\"0\" cellpadding=\"0\"><tr><td valign=\"top\"><strong>".$Familias."</strong>";
			...
			$sql = "select * from sim_articulos_grupos order by grupo";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)) {
		...
		echo "</td><td valign=\"top\"><strong>".$Subfamilias."</strong>";
			...
			$sqlg = "select * from sim_articulos_subgrupos where id_grupo=".$_GET['idgrupo']." order by subgrupo";
			$resultg = mysqli_query($dbhandle,convertSQL($sqlg));
			while ($rowg = mysqli_fetch_array($resultg)) {
		...
		echo "</td><td>&nbsp;</td><td style=\"text-align:center;vertical-align:top;witdh:300px\"><strong>".$Articulos."</strong>";
			if (($_GET['idsubgrupo'] != "") and ($_GET["id"] == "")) {
				$sqlt = "select count(*) as total from sim_articulos where visible=1 and descatalogat=0 and id_subgrupo=".$_GET["idsubgrupo"];
				$resultt = mysqli_query($dbhandle,convertSQL($sqlt));
				$rowt = mysqli_fetch_array($resultt);
				echo "<table cellpadding=\"0\" cellspacing=\"0\">";
				if ($rowt["total"] <> 0) {
					$x = 0;
					$sql = "select * from sim_articulos where visible=1 and id_subgrupo=".$_GET["idsubgrupo"]." and descatalogat=0 order by nombre";
					$result = mysqli_query($dbhandle,convertSQL($sql));
					while ($row = mysqli_fetch_array($result)) {
			...
			if ($_GET['descatalogat'] != "") {
				$sqlt = "select count(*) as total from sim_articulos where visible=1 and descatalogat=1";
				$resultt = mysqli_query($dbhandle,convertSQL($sqlt));
				$rowt = mysqli_fetch_array($resultt);
				echo "<table cellpadding=\"0\" cellspacing=\"0\">";
				if ($rowt["total"] <> 0) {
					$x = 0;
					$sql = "select * from sim_articulos where visible=1 and descatalogat=1 order by nombre";
					$result = mysqli_query($dbhandle,convertSQL($sql));
					while ($row = mysqli_fetch_array($result)) {
						if ($x == 0) { echo "<tr style=\"background-color : #D8D8D8;\">"; }
						if ($x == 1) { echo "<tr>"; }
						...
						$sqlpvp = "select * from sim_cuerpo where codigo=".$row["codigo"]." and vigente=1";
...
function mostrarAlmacen($id_almacen,$id_seccion)
{
	...
	$sqlp = "select id,nombre from sim_stock_almacenes_pasillo where id_almacen=".$id_almacen." and visible=1 order by orden";
	$resultp = mysqli_query($dbhandle,convertSQL($sqlp));
	while ($rowp = mysqli_fetch_array($resultp)) {
		echo $Pasillo." : <strong>".$rowp["nombre"]."</strong><br><br>";
		echo "<table cellpadding=\"0\" cellspacing=\"0\">";
