...
#Vista general de monitorizacion
	if (($soption == 0) or ($soption == 1) or ($soption == 200)) {
		if ($ssoption == 1) {
			$sqlin = "select pausada from sim_incidencias where id_incidencia=".$_GET["id_inc_rel"]." and visible=1 order by fecha_inicio desc";
			$resultin = mysqli_query($dbhandle,convertSQL($sqlin));
			$rowin = mysqli_fetch_array($resultin);

			$sqlinc = "select notas_registro from sim_incidencias where id=".$_GET["id"];
			...
			updateFunction ("sim_incidencias",$_GET["id"],$camposUpdate,$datosUpdate);

			$sqlinc2 = "select id from sim_incidencias where id_incidencia=".$_GET["id"];
			...
			while($rowinc2 = mysqli_fetch_array($resultinc2)){
				...
				updateFunction ("sim_incidencias",$rowinc2["id"],$camposUpdate,$datosUpdate);
			}
		}
		if ($ssoption == 2) {
			...
			$sql = "update sim_incidencias set ";
			$sql = $sql."visible=0";
...
#				echo "</td>";
					...
					echo "<td>&nbsp;&nbsp;</td>";
					$counter = 0;
					$sqlcli = "select id,alias from sim_clientes where visible=1 and id in (select id_cliente from sim_incidencias where visible=1 and id_estado<>-2 and id_incidencia=0) order by alias";
			...
			echo "<table cellspacing=\"1\" cellpadding=\"0\" style=\"background-color:silver;\" class=\"lista\">";
				...
				echo "</tr><tr>";
					echo "<td colspan=\"4\"><select name=\"id_cliente\" id=\"id_cliente\" style=\"width:500px\" onchange=\"desplegableCombinado2()\">";
						...
						$sqlcl1 = "select id,nombre from sim_clientes where visible=1 and id in (select id_cliente from sim_contratos where visible=1 and activo=1) order by nombre";
						...
						$sqlcl = "select id,nombre from sim_clientes where visible=1 and id not in (select id_cliente from sim_contratos where visible=1 and activo=1) order by nombre";
...
#				echo "</tr><tr>";
					echo "<td colspan=\"3\"><select name=\"id_servicio\" id=\"id_servicio\" style=\"width:500px;\">";
						echo "<option value=\"0\">-</option>";
							if ($_POST["id_cliente"] != "") {$sqlc = "select id_cliente_final,id,descripcion from sim_contratos where visible=1 and activo=1 and id_cliente=".$_POST["id_cliente"]."";}
							elseif ($_GET["id_cli"] != "") {$sqlc = "select id_cliente_final,id,descripcion from sim_contratos where visible=1 and activo=1 and id_cliente=".$_GET["id_cli"]."";}
							$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
							while ($rowc = mysqli_fetch_array($resultc)){
								$sqlclie = "select nombre from sim_clientes where visible=1 and id=".$rowc["id_cliente_final"]." order by nombre";
								$resultclie = mysqli_query($dbhandle,convertSQL($sqlclie));
								$rowclie = mysqli_fetch_array($resultclie);
								$sqls = "select id,servicio from sim_contratos_servicio where visible=1 and id_contrato=".$rowc["id"]." order by servicio";
...
#				echo "<tr><td>&nbsp;</td></tr>";
				...
				echo "</tr><tr>";
					...
					echo "<td><select name=\"id_estado\" style=\"width:180px\">";
						...
						$sqle = "select id,estado from sim_incidencias_estados where visible=1 order by estado";
		...
		echo "<table cellspacing=\"0\" style=\"width:100%\" class=\"lista\">";
			...
			$temps_total = 0;
			$id_relacio = array();
			$sqle = "select id from sim_incidencias where visible=1 and id_estado<>-2 and id_incidencia=0 order by id desc";
			$resulte = mysqli_query($dbhandle,convertSQL($sqle));
			while ($rowe = mysqli_fetch_array($resulte)){
...
#			echo var_dump($id_relacio);
		...
		#				echo "<th>".$Servicio."</th>";
				...
				$sqli = "select id,id_usuario_destino,id_servicio,id_cliente,id_estado,fecha_registro_cierre,fecha_prevision,fecha_inicio,pausada,asunto,tiempo_pendiente from sim_incidencias where visible=1 and id_incidencia = 0";
			}
			if (($_POST["mes"] != 0) or ($_GET["m"] != 0) or ($_POST["any"] != 0) or ($_GET["a"] != 0)){
				...
				$sqli = $sqli." and (fecha_inicio between ".$data1." and ".$data2." or id IN (select id_incidencia from sim_incidencias where id_incidencia<>0 and fecha_inicio between ".$data1." and ".$data2." and visible=1))";
			}
			if (($_GET["u"] != 0) or ($_POST["id_usuario"] != 0)) { $sqli = $sqli." and (id_usuario_destino=".$_GET["u"].$_POST["id_usuario"]." or id IN (select id_incidencia from sim_incidencias where id_incidencia<>0 and id_usuario_registro=".$_GET["u"].$_POST["id_usuario"]."))"; }
			if ($_POST["id_cliente"] != 0) { $sqli = $sqli." and id_cliente=".$_POST["id_cliente"].""; }
			if ($_GET["id_cli"] != 0) { $sqli = $sqli." and (id_cliente=".$_GET["id_cli"]." or id_servicio in (select id from sim_contratos_servicio where id_contrato in (select id from sim_contratos where id_cliente=".$_GET["id_cli"]." and visible=1) and visible=1))"; }
			...
			if ($_POST["texto"] != "") { $sqli = $sqli." and (notas_registro like '%".$_POST["texto"]."%' or notas_conclusion like '%".$_POST["texto"]."%' or id in (select id_incidencia from sim_incidencias where visible=1 and id_incidencia<>0 and notas_desarrollo like '%".$_POST["texto"]."%')) ";}
...
#			if ($_GET["pausa"] == 0) { $sqli = $sqli." and pausada=0"; }
			...
			elseif ($i == 2) { $sqli = $sqli." and id_servicio <> 0 and fecha_prevision=0 and pausada=0 and id_cliente<>(select id from sim_clientes where nif='B65702227' and visible=1)"; } 
			elseif ($i == 3) { $sqli = $sqli." and id_servicio <> 0 and fecha_prevision=0 and pausada=0 and id_cliente=(select id from sim_clientes where nif='B65702227' and visible=1)"; } 
			elseif ($i == 4) { $sqli = $sqli." and pausada=1"; }
			if ($i == 1) {
				$sqli = $sqli." order by tiempo_pendiente,id_cliente,id_servicio,id_usuario_destino";
			} elseif ($i == 4) {
...
#		echo $sqli."<br>";
			...
			while ($rowi = mysqli_fetch_array($resulti)) {
				...
				$sqlu = "select usuario from sim_users where id=".$rowi["id_usuario_destino"];
				$resultu = mysqli_query($dbhandle,convertSQL($sqlu));
				$rowu = mysqli_fetch_array($resultu);
				$sqlc = "select servicio,id_contrato,temps_resposta from sim_contratos_servicio where id=".$rowi["id_servicio"];
				$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
				$rowc = mysqli_fetch_array($resultc);
				$sqls = "select id_cliente,descripcion from sim_contratos where id=".$rowc["id_contrato"];
				$results = mysqli_query($dbhandle,convertSQL($sqls));
				...
				$sqlcc = "select id,nombre from sim_clientes where id=".$cliente;
				...
				if ($rowc["temps_resposta"] != 0){
					...
					$horax = $rowi["tiempo_pendiente"]/3600;
				...
				$sqlind = "select id_usuario_registro from sim_incidencias where id_incidencia=".$rowi["id"]." and visible=1 order by fecha_inicio desc";
				$resultind = mysqli_query($dbhandle,convertSQL($sqlind));
				$rowind = mysqli_fetch_array($resultind);
				if ($rowind){
					$sqluu = "select * from sim_users where id=".$rowind["id_usuario_registro"];
				...
				$sqldd = "select sum(duracion) as temps from sim_incidencias where visible=1 and fecha_inicio between ".$data1." and ".$data2." and id_incidencia=".$rowi["id"]."";
...
#					echo "</td>";
				echo "</tr>";
				$sqlnd = "select sum(duracion) as total from sim_incidencias where id_incidencia=".$rowi["id"]."";
	...
	if ($soption == 2) {
		$sqlinc = "select id,id_cliente,asunto from sim_incidencias where id=".$_GET["id"]."";
		$resultinc = mysqli_query($dbhandle,convertSQL($sqlinc));
		$rowinc = mysqli_fetch_array($resultinc);
		$sqlcc = "select nombre from sim_clientes where id=".$rowinc["id_cliente"];
		$resultcc = mysqli_query($dbhandle,convertSQL($sqlcc));
		$rowcc = mysqli_fetch_array($resultcc);
		echo "<center>";
	...
	if ($soption == 3) {
		$sqlinc = "select id from sim_incidencias where visible=1 and id_incidencia=0";
		$resultinc = mysqli_query($dbhandle,convertSQL($sqlinc));
...
#detall tickets#
	if ($soption == 100) {
		if ($ssoption > 0){
			$sqli = "select tiempo_pendiente,fecha_inicio,id,pausada,id_usuario_destino,id_usuario_finalizacion,fecha_registro_cierre,fecha_cierre,notas_conclusion,id_estado from sim_incidencias where id=".$_GET["id"];
			$resulti = mysqli_query($dbhandle,convertSQL($sqli));
			$rowi = mysqli_fetch_array($resulti);
			$sqlc = "select temps_resposta,id from sim_contratos_servicio where id in (select id_servicio from sim_incidencias where id=".$_GET["id"].")";
			$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
			$rowc = mysqli_fetch_array($resultc);
			if ($_GET["id"] > 0){
		...
		#update incidencia#
			if (($_POST["id_servicio"] > 0) or ($_POST["id_servicio2"] > 0) or ($_POST["id_cliente"] > 0)){
				if ($_POST["asunto"] != ""){
					...
					$camposUpdate = array("tiempo_pendiente","fecha_prevision");
		...
		#update notes desenvolupament#
			...
			updateFunction ("sim_incidencias",$_GET["id_not"],$camposUpdate,$datosUpdate);
		...
		#insert notes desenvolupament#
			if (($_POST["notas_desarrollo"] != "") and ($_POST["duracion"] != "")){
				...
				insertFunction ("sim_incidencias",$camposInsert,$datosInsert);
		...
		#finalizacion incidencia#
			if ($rowi["id_estado"] == -2){
				...
				$sqld = "select sum(duracion) as total from sim_incidencias where id_incidencia=".$_GET["id"]."";
				$resultd = mysqli_query($dbhandle,convertSQL($sqld));
				$rowd = mysqli_fetch_array($resultd);
				$total = $rowd["total"];
				if ((($_POST["notas_registro"] != "") or ($_POST["notas_conclusion"] != ""))  and ($total > 0) and (strtotime($_POST["fecha_cierre"]) > 0)){
					if ($rowc["temps_resposta"] != 0){
						$tiempo_pendiente=(calculSLA($_GET["id"],1));
						if ($tiempo_pendiente > 0) { $sla = 0;} else { $sla = 1;}
					} else {
						$sqld = "select tiempo_pendiente from sim_incidencias where id_incidencia=".$_GET["id"]."";
						$resultd = mysqli_query($dbhandle,convertSQL($sqld));
						$rowd = mysqli_fetch_array($resultd);
						$sla = 0;
						$tiempo_pendiente = $rowd["tiempo_pendiente"];
					}
					$fecha_cierre = date("U", strtotime($_POST["fecha_cierre"]));
					$fecha_fi = time();
					$camposUpdate = array("notas_conclusion","fecha_registro_cierre","fecha_cierre","id_usuario_finalizacion","id_estado","sla","tiempo_pendiente","pausada");
					$datosUpdate = array($_POST["notas_conclusion"],$fecha_fi,$fecha_cierre,$userid,'-2',$sla,$tiempo_pendiente,0);
					updateFunction ("sim_incidencias",$_GET["id"],$camposUpdate,$datosUpdate);
				} else {
		...
		#insert incidencia#
			if (($_POST["id_servicio"] > 0) or ($_POST["id_servicio2"] > 0) or ($_POST["id_cliente"] > 0)){
				if ($_POST["asunto"] != ""){
					...
					$sql = "select id from sim_incidencias where fecha_prevision=".$fecha_prevision." and id_incidencia=0 order by id desc";
					$result = mysqli_query($dbhandle,convertSQL($sql));
					$row = mysqli_fetch_array($result);

					if ($_POST["duracion"] != ""){
						$sql = "select id from sim_incidencias where fecha_prevision=".$fecha_prevision." order by id desc";
						$result = mysqli_query($dbhandle,convertSQL($sql));
						$row = mysqli_fetch_array($result);
						...
						if ($_POST["pausada"] == 1){
							...
							$datosUpdate = array("0");
							updateFunction ("sim_incidencias",$row["id"],$camposUpdate,$datosUpdate);
						}
						if ($_POST["finalizar"] == 1){
							$sqld = "select sum(duracion) as total from sim_incidencias where id_incidencia=".$row["id"]."";
							$resultd = mysqli_query($dbhandle,convertSQL($sqld));
							$rowd = mysqli_fetch_array($resultd);
							$total = $rowd["total"];
							if ((($_POST["notas_registro"] != "") or ($_POST["notas_conclusion"] != "")) and ($total > 0)){
								$sqlc = "select temps_resposta from sim_contratos_servicio where id=(select id_servicio from sim_incidencias where id=".$row["id"].")";
								$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
								$rowc = mysqli_fetch_array($resultc);
								if ($rowc["temps_resposta"] != 0){
		...
		if ($id_inc != ""){
			$sql = "select * from sim_incidencias where id=".$id_inc;
			$result = mysqli_query($dbhandle,convertSQL($sql));
			$row = mysqli_fetch_array($result);
		...
		$sqlid = "select idioma from sim_idiomas where visible=1 and id=(select id_idioma from sim_clientes where visible=1 ";
		$sqlid .= "and id=(select id_cliente from sim_contratos where visible=1 ";
		$sqlid .= "and id=(select id_contrato from sim_contratos_servicio where visible=1 and id=".$row["id_servicio"].")))";
		...
		echo "<table cellpadding=\"1\" cellspacing=\"10\" class=\"lista\" style=\"width:100%;\">";
			echo "<tr><td style=\"text-align:left;vertical-align:top;width:23%;\">";
					echo "<table cellspacing=\"0\" style=\"width:100%;\">";
						...
						echo "<tr>";
							...
							echo "<td><select name=\"id_entrada\" style=\"width:150px\">";
								echo "<option value=\"0\">-</option>";
								$sqle = "select id,entrada from sim_incidencias_entrada order by entrada";
								$resulte = mysqli_query($dbhandle,convertSQL($sqle));
								while ($rowe = mysqli_fetch_array($resulte)) {
						...
						if ($id_inc != ""){
							$sqlu = "select usuario from sim_users where validado=1 and activo=1 and id=".$row["id_usuario_registro"];
						} else {
							$sqlu = "select usuario from sim_users where validado=1 and activo=1 and id=".$userid;
						}
							$resultu = mysqli_query($dbhandle,convertSQL($sqlu));
							$rowu = mysqli_fetch_array($resultu);
						...
						echo "<tr>";
							...
							echo "<td><select name=\"id_usuario_origen\" style=\"width:150px\">";
								$sqlu = "select id,usuario from sim_users where validado=1 and activo=1 order by usuario";
								$resultu = mysqli_query($dbhandle,convertSQL($sqlu));
								while ($rowu = mysqli_fetch_array($resultu)) {
									if ($id_inc != ""){
						...
						echo "<tr>";
							...
							echo "</select></td>";
						echo "</tr>";
						$sqld = "select sum(duracion) as total from sim_incidencias where id_incidencia=".$id_inc." and visible=1";
						$resultd = mysqli_query($dbhandle,convertSQL($sqld));
						$rowd = mysqli_fetch_array($resultd);
						$total = $rowd["total"];
						$hora = $total/60;
						...
						$sqlpa = "select pausada from sim_incidencias where id_incidencia=".$id_inc." and visible=1 order by fecha_inicio desc";
				...
				echo "</td><td style=\"text-align:right;vertical-align:top;width:60%;\">";
					echo "<table cellspacing=\"0\" style=\"width:100%;\">";
						$sqlcl = "select nif from sim_dades_origen_factura";
						$resultcl = mysqli_query($dbhandle,convertSQL($sqlcl));
						$rowcl = mysqli_fetch_array($resultcl);
						$sqlcli = "select id from sim_clientes where visible=1 and nif='".$rowcl["nif"]."'";
						$resultcli = mysqli_query($dbhandle,convertSQL($sqlcli));
						...
						echo "<tr>";
							...
							echo "<td colspan=\"9\"><select name=\"id_servicio\" id=\"id_servicio\" style=\"width:100%\" onchange=\"desplegableCombinado()\">";
								echo "<option value=\"0\">-</option>";
								$sqlc = "select id from sim_contratos where visible=1 and id_cliente=".$rowcli["id"]." and activo=1 order by num_contrato";
								$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
								while ($rowc = mysqli_fetch_array($resultc)) {
						...
						echo "<tr>";
							...
							echo "<td colspan=\"9\"><select name=\"id_cliente\" id=\"id_cliente\" style=\"width:100%\" onchange=\"desplegableCombinado2()\">";
								echo "<option value=\"0\">-</option>";
								$sqlcl1 = "select id,nombre from sim_clientes where visible=1 and id<>".$rowcli["id"]." and id in (select id_cliente from sim_contratos where visible=1 and activo=1) order by nombre";
								$resultcl1 = mysqli_query($dbhandle,convertSQL($sqlcl1));
								while ($rowcl1 = mysqli_fetch_array($resultcl1)){
									if ($_POST["id_cliente"] == $rowcl1["id"]){
										echo "<option value=\"".$rowcl1["id"]."\" selected>- ".$rowcl1["nombre"]."</option>";
						...
						echo "</tr>";
							...
							echo "<td colspan=\"9\"><select name=\"id_servicio2\" id=\"id_servicio2\" style=\"width:100%;\" onchange=\"desplegableCombinado3(this.value)\">";
								echo "<option value=\"0\">-</option>";
									if ($_POST["id_cliente"] != ""){
										$sqlc = "select id,id_cliente_final,descripcion from sim_contratos where visible=1 and activo=1 and id_cliente=".$_POST["id_cliente"]."";
									} elseif ($row["id_cliente"] != ""){
										$sqlc = "select id,id_cliente_final,descripcion from sim_contratos where visible=1 and activo=1 and id_cliente=".$row["id_cliente"]."";
									} elseif ($_GET["id_cli"] != ""){
										$sqlc = "select id,id_cliente_final,descripcion from sim_contratos where visible=1 and activo=1 and id_cliente=".$_GET["id_cli"]."";
									}
									$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
									while ($rowc = mysqli_fetch_array($resultc)){
										$sqlclie = "select * from sim_clientes where visible=1 and id=".$rowc["id_cliente_final"]." order by nombre";
										$resultclie = mysqli_query($dbhandle,convertSQL($sqlclie));
										$rowclie = mysqli_fetch_array($resultclie);
										$sqls = "select * from sim_contratos_servicio where visible=1 and id_contrato=".$rowc["id"]." order by servicio";
										$results = mysqli_query($dbhandle,convertSQL($sqls));
										while ($rows = mysqli_fetch_array($results)){
				...
				if ($row["correo"] == 1){
					...
					$sqlfiles = "select name from sim_files where visible=1 and id_elemento=".$row["id"];
		...
		if ($id_inc != ""){
			...
			echo "</form>";
			echo "<tr><td>&nbsp;</td></tr>";
			$sqlc = "select * from sim_incidencias where id_incidencia=".$id_inc." and visible=1 order by fecha_inicio asc";
			$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
			while ($rowc = mysqli_fetch_array($resultc)) {
				echo "<form action=\"index.php?op=1018&sop=100&ssop=2&id=".$id_inc."&id_not=".$rowc["id"]."\" method=\"post\" name=\"form1\">";
					...
					echo "<table cellspacing=\"0\" cellspacing=\"0\" style=\"width:100%;\">";
						...
						if ($rowc["id_usuario_registro"] == $userid){
							...
							$sqlu = "select id,usuario from sim_users where validado=1 and activo=1 and id=".$rowc["id_usuario_registro"];
							$resultu = mysqli_query($dbhandle,convertSQL($sqlu));
							$rowu = mysqli_fetch_array($resultu);
						...
						if ($rowc["id_usuario_registro"] == $userid){
							...
							if ($row["id_estado"] != -2){
								echo "<tr>";
									...
									echo "</form>";
									$sqlud = "select mail from sim_users where id=".$row["id_usuario_origen"];
									$resultud = mysqli_query($dbhandle,convertSQL($sqlud));
									$rowud = mysqli_fetch_array($resultud);
									$sqlcs = "select codigo_catalogo from sim_contratos_servicio where id=".$row["id_servicio"];
...
#									echo "</td>";
			...
			echo "<tr><td style=\"text-align:left;vertical-align:top;width:23%;\">";
					echo "<table cellspacing=\"0\" cellspacing=\"0\" style=\"width:100%;\">";
						...
						$sqlu = "select usuario from sim_users where id=".$row["id_usuario_finalizacion"];
						$resultu = mysqli_query($dbhandle,convertSQL($sqlu));
						$rowu = mysqli_fetch_array($resultu);
						if ($row["fecha_cierre"]){
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "<tr>";
				echo "<td><select name=\"id_usuario\" style=\"width:200px;\">";
					...
					$sqlu = "select id,usuario from sim_users where validado=1 and activo=1 and sgm=1";
					$resultu = mysqli_query($dbhandle,convertSQL($sqlu));
					while ($rowu = mysqli_fetch_array($resultu)) {
						...
						for ($i=1;$i<=31;$i++){
							if ($i <= cal_days_in_month(CAL_GREGORIAN, $me, $_POST["any"])){
								...
								echo "<tr style=\"background-color:".$color."\">";
									...
									$sqlis = "select sum(duracion) as duracion_total from sim_incidencias where visible=1 and id_incidencia<>0 and id_usuario_registro=".$_POST["id_usuario"]." and fecha_inicio between ".$data1." and ".$data2."";
...
#				if ($count == 6){ echo "</tr><td>&nbsp;</td><tr></tr><td>&nbsp;</td><tr>"; $count=0;}
		...
		$sqlr = "select num_horas_any from sim_rrhh_jornada_anual";
...
#estados incidencias#
	if (($soption == 510) and ($admin == true)) {
		if (($ssoption == 1) and ($admin == true)) {	
			...
			insertFunction ("sim_incidencias_estados",$camposInsert,$datosInsert);
		}
		if (($ssoption == 2) and ($admin == true)) {
			$camposUpdate = array("estado","editable");
			$datosUpdate = array($_POST["estado"],$_POST["editable"]);
			updateFunction ("sim_incidencias_estados",$_GET["id"],$camposUpdate,$datosUpdate);
		}
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "<tr>";
				...
				echo "</form>";
			echo "</tr>";
				$sql = "select * from sim_incidencias_estados where visible=1";
				$result = mysqli_query($dbhandle,convertSQL($sql));
...
#	if ($soption = 600){
#		$sqlinc = "select id_servicio,id from sim_incidencias where visible=1 and id_incidencia=0 and id_cliente=0";
#		$resultinc = mysqli_query($dbhandle,convertSQL($sqlinc));
#		while ($rowinc = mysqli_fetch_array($resultinc)){
#			$sqlc = "select id_cliente from sim_contratos where visible=1 and id=(select id_contrato from sim_contratos_servicio where id=".$rowinc["id_servicio"].")";
#			$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
#			$rowc = mysqli_fetch_array($resultc);

#			$camposUpdate = array("id_cliente");
#			$datosUpdate = array($rowc["id_cliente"]);
#			updateFunction ("sim_incidencias",$rowinc["id"],$camposUpdate,$datosUpdate);
#		}
#	}
...
function comprobar_festivo_empleado($fecha,$id_usuario)
{
	global $db,$dbhandle;
	$festivo=0;
	$sqle = "select * from sim_rrhh_empleado_calendario where ((data_ini=".$fecha.") or ((".$fecha." >= data_ini) and (".$fecha." <= data_fi))) and id_empleado=(select id from sim_rrhh_empleado where id_usuario=".$id_usuario.")";
	$resulte = mysqli_query($dbhandle,convertSQL($sqle));
