...
if (($option == 1005) AND ($autorizado == true)) {
	...
	if (($soption == 0) or ($soption == 200)) {
		if ($ssoption == 1) {
			$camposInsert = "nombre,id_tipo,id_cliente,codigo,id_empleado,num_serie";
			$datosInsert = array($_POST["nombre"],$_POST["id_tipo"],$_POST["id_cliente"],$_POST["codigo"],$_POST["id_empleado"],$_POST["num_serie"]);
			insertFunction ("sim_inventario",$camposInsert,$datosInsert);
		}
		if ($ssoption == 2) {
			$camposUpdate = array("nombre","id_tipo","id_cliente","codigo","id_empleado","num_serie");
			$datosUpdate = array($_POST["nombre"],$_POST["id_tipo"],$_POST["id_cliente"],$_POST["codigo"],$_POST["id_empleado"],$_POST["num_serie"]);
			updateFunction ("sim_inventario",$_GET["id"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 3) {
...
#					if ($soption == 200) {echo "<th style=\"width:100px\">".$Datos."</th>";}
			...
			if ($soption == 200) {
				...
				echo "<tr>";
				$sqlx = "select * from sim_inventario where visible=1 and id=".$_GET["id_disp"]."";
...
#					echo "<td><input type=\"Text\" name=\"codigo\" value=\"".$_POST["codigo"]."\" style=\"width:100%\"></td>";
					echo "<td><select style=\"width:100%;\" name=\"id_tipo\">";
						...
						$sql = "select * from sim_inventario_tipo where visible=1 order by tipo";
					...
					echo "</td>";
					echo "<td style=\"color:black;text-align:left;width:250px\"><input type=\"Text\" name=\"dato\" value=\"".$_POST["dato"]."\" style=\"width:250px\"></td>";
...
#					echo "<td></td>";
					echo "<td>";
						...
						echo "<option value=\"0\">-</option>";
						$sqlo = "select * from sim_inventario_tipo where visible=1 order by tipo";
			...
			echo "<tr><td>&nbsp;</td></tr>";
		if (($soption == 0) or (($soption == 200) and (($_POST["nombre"] != "") or ($_POST["id_tipo"] > 0) or ($_POST["id_cliente"] > 0) or ($_POST["dato"] != "")))) {
			$sql = "select * from sim_inventario where visible=1";
			if (($soption == 200) and ($ssoption == 0)){
				if ($_POST["dato"]) {
					$sqldato = "select * from sim_inventario_tipo_atributo_dato where visible=1 and dato like '%".$_POST["dato"]."%'";
					$resultdato = mysqli_query($dbhandle,convertSQL($sqldato));
					$rowdato = mysqli_fetch_array($resultdato);
					$sql = $sql." and id=".$rowdato["id_inventario"]."";
				}
				if ($_POST["nombre"]) {$sql = $sql." and nombre like '%".$_POST["nombre"]."%'";}
				if ($_POST["id_tipo"]) {$sql = $sql." and id_tipo=".$_POST["id_tipo"]."";}
				if ($_POST["id_cliente"]) {$sql = $sql." and id_cliente=".$_POST["id_cliente"]."";}
			}
			if (($soption == 200) and ($ssoption == 2)) {
				...
				if ($_POST["id_cliente2"]) {$sql = $sql." and id_cliente2=".$_POST["id_cliente2"]."";}
			}
			$sql = $sql." order by id_cliente,id_empleado,id_tipo,nombre";
#			echo $sql;
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)){
...
#					echo "<td>".$row["codigo"]."</td>";
					...
					echo "<input type=\"Hidden\" name=\"dato2\" value=\"".$_POST["dato"]."\">";
...
#					echo "<td><a href=\"index.php?op=1005&sop=100&id=".$row["id"]."\"><img src=\"mgestion/pics/icons-mini/page_white_edit.png\" border=\"0\"></a></td>";
	...
	if (($soption >= 100) and ($soption < 200) and ($_GET["id"] > 0)) {
		if (($soption == 100) and ($ssoption == 1)) {
			...
			updateFunction ("sim_inventario",$_GET["id"],$camposUpdate,$datosUpdate);
			$sqla = "select * from sim_inventario_tipo_atributo where id_tipo=".$_POST["id_tipo"];
			$resulta = mysqli_query($dbhandle,convertSQL($sqla));
			while ($rowa = mysqli_fetch_array($resulta)){
				if ($_POST["dato_".$rowa["id"]]){
					$sqld = "select count(*) as total from sim_inventario_tipo_atributo_dato where id_inventario=".$_GET["id"]." and id_atributo=".$rowa["id"];
					$resultd = mysqli_query($dbhandle,convertSQL($sqld));
					$rowd = mysqli_fetch_array($resultd);
					if ($rowd["total"] != 0){
						$sqlb = "update sim_inventario_tipo_atributo_dato set ";
						$sqlb = $sqlb."dato='".comillas($_POST["dato_".$rowa["id"]])."'";
						$sqlb = $sqlb." WHERE id_inventario=".$_GET["id"]." and id_atributo=".$rowa["id"];
						mysqli_query($dbhandle,convertSQL($sqlb));
					}
					if ($rowd["total"] == 0){
						$sqlc = "insert into sim_inventario_tipo_atributo_dato (dato,id_atributo,id_inventario) ";
						$sqlc = $sqlc."values (";
						$sqlc = $sqlc."'".comillas($_POST["dato_".$rowa["id"]])."'";
						$sqlc = $sqlc.",".$rowa["id"]."";
						$sqlc = $sqlc.",".$_GET["id"]."";
						$sqlc = $sqlc.")";
		...
		$sql = "select * from sim_inventario where id=".$_GET["id"];
		$result = mysqli_query($dbhandle,convertSQL($sql));
...
#							echo "<td class=\"ficha\"><a href=\"index.php?op=1005&sop=110&id=".$row["id"]."\" style=\"color:white;\">".$Relaciones."</a></td>";
	...
	if ($soption == 100) {
		$sql = "select * from sim_inventario where id=".$_GET["id"].$row1["id"];
		$result = mysqli_query($dbhandle,convertSQL($sql));
		$row = mysqli_fetch_array($result);
			...
			echo "<tr>";
				...
				echo "<td style=\"vertical-align:top\">";
					echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
						...
						$sqla = "select * from sim_inventario_tipo_atributo where visible=1 and id_tipo=".$tipo."";
						$resulta = mysqli_query($dbhandle,convertSQL($sqla));
						while ($rowa = mysqli_fetch_array($resulta)) {
							$sqld = "select * from sim_inventario_tipo_atributo_dato where visible=1 and id_atributo=".$rowa["id"]." and id_inventario=".$row["id"]."";
							$resultd = mysqli_query($dbhandle,convertSQL($sqld));
							$rowd = mysqli_fetch_array($resultd);
							echo "<tr><td style=\"text-align:right;width:150px\">".$rowa["atributo"]." : </td><td><input type=\"Text\" name=\"dato_".$rowa["id"]."\" value=\"".$rowd["dato"]."\"  style=\"width:385px\"></td></tr>";
						}
					echo "</table>";
				echo "</td>";
			echo "</tr>";
...
/*
	if ($soption == 110){
		if ($ssoption == 3) {
			$sql = "delete from sim_inventario_relacion WHERE (id_disp1=".$_GET["id_disp"]." and id_disp2=".$_GET["id"].") or (id_disp2=".$_GET["id_disp"]." and id_disp1=".$_GET["id"].")";
		...
		echo "<table cellpadding=\"0\" cellspacing=\"0\">";
			...
			echo "</tr>";
		$sqli = "select * from sim_inventario_relacion where visible=1 and (id_disp1=".$_GET["id"]." or id_disp2=".$_GET["id"].")";
		...
		while ($rowi = mysqli_fetch_array($resulti)) {
			...
			$sql = "select * from sim_inventario where visible=1 and id=".$id_disp."";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			$row = mysqli_fetch_array($result);
			echo "<tr>";
				...
				$sqlt = "select * from sim_inventario_tipo where visible=1 and id=".$row["id_tipo"]." order by tipo";
				...
				echo "<td><table>";
					$sqla = "select * from sim_inventario_tipo_atributo where visible=1 and id_tipo=".$row["id_tipo"]." order by atributo";
					$resulta = mysqli_query($dbhandle,convertSQL($sqla));
					while ($rowa = mysqli_fetch_array($resulta)){
						echo "<tr><td style=\"width:150px;vertical-align:top\">".$rowa["atributo"]."</td>";
						$sqlda = "select * from sim_inventario_tipo_atributo_dato where visible=1 and id_atributo=".$rowa["id"]." and id_inventario=".$row["id"]."";
						$resultda = mysqli_query($dbhandle,convertSQL($sqlda));
						while ($rowda = mysqli_fetch_array($resultda)){
							if (strlen($rowda["dato"]) < 2){
								echo "<td style=\"width:150px;\">".$rowda["dato"]."</td></tr>";
							} else {
								echo "<td><textarea cols=\"20\" rows=\"2\">".$rowda["dato"]."</textarea></td>";
							}
						}
					}
				echo "</table></td>";
	...
	if ($soption == 115){
		...
		$sql0 = "select * from sim_inventario where visible=1 and id=".$_GET["id"]."";
		...
		echo "<table>";
			...
			while ($rowi = mysqli_fetch_array($resulti)) {
				...
				if ($row["id"]){
					echo "<tr>";
						$id = $id.",".$row["id"];
						$sqlt = "select * from sim_inventario_tipo where visible=1 and id=".$row["id_tipo"]."";
						$resultt = mysqli_query($dbhandle,convertSQL($sqlt));
						$rowt = mysqli_fetch_array($resultt);
					if (($ssoption == 1) and ($row["id"] == $_GET["id_disp"])){
						echo "<td>";
							echo "<table>";
							$sqlii = "select * from sim_inventario_relacion where visible=1 and (id_disp2=".$_GET["id_disp"]." or id_disp1=".$_GET["id_disp"].")";
							...
							while ($rowii = mysqli_fetch_array($resultii)) {
								...
								$sqltx = "select * from sim_inventario_tipo where visible=1 and orden>".$rowt["orden"]."";
								$resulttx = mysqli_query($dbhandle,convertSQL($sqltx));
								while ($rowtx = mysqli_fetch_array($resulttx)) {
									$sqlx = "select * from sim_inventario where visible=1 and id=".$id_disp2." and id NOT IN (".$id.") and id_tipo=".$rowtx["id"];
									$resultx = mysqli_query($dbhandle,convertSQL($sqlx));
									$rowx = mysqli_fetch_array($resultx);
									if ($rowx["id"]){
										echo "<tr>";
...
*/
	if ($soption == 120){
		...
		if ($ssoption == 1) {
			...
			$datosUpdate=array(1);
			updateFunction("sim_inventario_actuacion_disp",$_GET["id_act"],$camposUpdate,$datosUpdate);
		}
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			echo "<tr style=\"background-color:silver;\">";
				...
				echo "<th></th>";
			echo "</tr>";
		$sqli = "select * from sim_inventario_actuacion_disp where visible=1 and id_dispositivo=".$_GET["id"]."";
		$resulti = mysqli_query($dbhandle,convertSQL($sqli));
		while ($rowi = mysqli_fetch_array($resulti)) {
			...
			echo "<tr style=\"background-color:".$color."\">";
				echo "<td></td>";
				$sqlac = "select * from sim_inventario_actuacion where visible=1 and id=".$rowi["id_actuacion"];
				...
				$sqluu = "select * from sim_usuarios where id=".$rowi["id_usuario"]."";
				...
				echo "<td style=\"color:black;\">".$rowi["fecha_prevision"]."</td>";
	...
	if ($soption == 300) {
		if ($ssoption == 1) {
			$camposInsert = "id_actuacion,id_dispositivo,id_usuario,fecha_prevision";
			$datosInsert = array($_POST["id_actuacion"],$_POST["id_dispositivo"],$_POST["id_usuario"],$_POST["fecha_prevision"]);
			insertFunction ("sim_inventario_actuacion_disp",$camposInsert,$datosInsert);
		}
		if ($ssoption == 2) {
			$camposUpdate = array("id_actuacion","id_dispositivo","id_usuario","fecha_prevision");
			$datosUpdate = array($_POST["id_actuacion"],$_POST["id_dispositivo"],$_POST["id_usuario"],$_POST["fecha_prevision"]);
			updateFunction ("sim_inventario_actuacion_disp",$_GET["id"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 3) {
			$camposUpdate=array('visible');
			$datosUpdate=array(0);
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "<tr>";
				...
				echo "<td><select style=\"width:250px\" name=\"id_actuacion\">";
					echo "<option value=\"0\">-</option>";
					$sqla = "select * from sim_inventario_actuacion where visible=1 order by nombre";
					$resulta = mysqli_query($dbhandle,convertSQL($sqla));
					while ($rowa = mysqli_fetch_array($resulta)) {
				...
				echo "<td><select style=\"width:250px\" name=\"id_dispositivo\">";
					echo "<option value=\"0\">-</option>";
					$sqli = "select * from sim_inventario where visible=1 order by nombre";
					$resulti = mysqli_query($dbhandle,convertSQL($sqli));
					while ($rowi = mysqli_fetch_array($resulti)) {
						echo "<option value=\"".$rowi["id"]."\">".$rowi["nombre"]."</option>";
					}
				echo "</select></td>";
				echo "<td><select style=\"width:250px\" name=\"id_usuario\">";
					echo "<option value=\"0\">-</option>";
					$sqlu = "select * from sim_usuarios where sgm=1 and validado=1 and activo=1 order by usuario";
				...
				echo "</select></td>";
				$fecha = date("Y-m-d H:i:s");
				echo "<td><input type=\"text\" name=\"fecha_prevision\" value=\"".$fecha."\"></td>";
				echo "<td></td>";
				echo "<td><input type=\"Submit\" value=\"".$Anadir."\" style=\"width:150px\"></td>";
				echo "</form>";
			echo "</tr>";
			echo "<tr><td>&nbsp;</td></tr>";
			$sql = "select * from sim_inventario_actuacion_disp where visible=1 order by fecha_prevision";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)) {
				echo "<tr>";
					...
					echo "<td><select style=\"width:250px\" name=\"id_actuacion\">";
						echo "<option value=\"0\">-</option>";
						$sqlia = "select * from sim_inventario_actuacion where visible=1 order by nombre";
					...
					echo "</select></td>";
					echo "<td><input type=\"text\" name=\"fecha_prevision\" value=\"".$row["fecha_prevision"]."\"></td>";
					$sqlac = "select * from sim_inventario_actuacion where visible=1 and id=".$row["id_actuacion"];
					$resultac = mysqli_query($dbhandle,convertSQL($sqlac));
					$rowac = mysqli_fetch_array($resultac);
	...
	if ($soption == 310) {
		if ($ssoption == 1) {
			mysqli_query($dbhandle,convertSQL($sql));
			$camposInsert = "id_act,id_disp,id_usuario,data";
			$datosInsert = array($_GET["id_act"],$_GET["id_disp"],$userid,$xfecha);
			insertFunction ("sim_inventario_actuacion_disp",$camposInsert,$datosInsert);
		}
		$sqla = "select * from sim_inventario_actuacion where visible=1 and id=".$_GET["id"]."";
		$resulta = mysqli_query($dbhandle,convertSQL($sqla));
		...
		echo "<table cellspacing=\"0\ style=\"width:300px;\" class=\"lista\">";
			echo "<tr style=\"background-color:silver\">";
				...
				echo "<th>".$Estado."</th>";
			echo "</tr>";
			$sql = "select * from sim_inventario where visible=1 and id=".$rowa["id_disp"]."";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			$row = mysqli_fetch_array($result);
			$sqltip = "select * from sim_inventario_tipo where visible=1 and id=".$row["id_tipo"]."";
			$resulttip = mysqli_query($dbhandle,convertSQL($sqltip));
			$rowtip = mysqli_fetch_array($resulttip);
			$sqlr = "select * from sim_inventario_relacion where visible=1 and ((id_disp1=".$rowa["id_disp"].") or (id_disp2=".$rowa["id_disp"]."))";
			...
			while ($rowr = mysqli_fetch_array($resultr)) {
				...
				$sqlti = "select * from sim_inventario_tipo where visible=1 and orden < ".$rowtip["orden"]."";
				$resultti = mysqli_query($dbhandle,convertSQL($sqlti));
				$rowti = mysqli_fetch_array($resultti);
				$sqlt = "select * from sim_inventario where visible=1 and id=".$id_disp." and id_tipo=".$rowti["id"]."";
				$resultt = mysqli_query($dbhandle,convertSQL($sqlt));
				$rowt = mysqli_fetch_array($resultt);
				if ($rowt["id"]){
	 				$sqld = "select count(*) as total from sim_inventario_actuacion_disp where visible=1 and id_disp=".$rowt["id"]." and id_act=".$rowa["id"]."";
					$resultd = mysqli_query($dbhandle,convertSQL($sqld));
					$rowd = mysqli_fetch_array($resultd);
	...
	if (($soption == 510) and ($admin == true)) {
		if ($ssoption == 1) {
			...
			insertFunction ("sim_inventario_tipo",$camposInsert,$datosInsert);
		}
		if ($ssoption == 2) {
			$camposUpdate = array("tipo");
			$datosUpdate = array($_POST["tipo"]);
			updateFunction ("sim_inventario_tipo",$_GET["id"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 3) {
			$sql = "update sim_inventario set ";
			$sql = $sql."id_tipo=0";
			$sql = $sql." WHERE id_tipo=".$_GET["id"]."";
			mysqli_query($dbhandle,convertSQL($sql));
			$sql = "delete from sim_inventario_tipo WHERE id=".$_GET["id"];
			mysqli_query($dbhandle,convertSQL($sql));
		}
		echo "<h4>".$Tipos." :</h4>";
...
#				echo "<td><input type=\"text\" style=\"width:100%\" name=\"codigo\"></td>";
			...
			echo "</tr>";
			echo "<tr><td>&nbsp;</td></tr>";
			$sql = "select * from sim_inventario_tipo order by orden";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)) {
				echo "<tr>";
...
#					echo "</td>";
	...
	if (($soption == 515) and ($admin == true)) {
		if ($ssoption == 1) {
			...
			insertFunction ("sim_inventario_tipo_atributo",$camposInsert,$datosInsert);
		}
		if ($ssoption == 2) {
			$camposUpdate = array("atributo");
			$datosUpdate = array($_POST["atributo"]);
			updateFunction ("sim_inventario_tipo_atributo",$_GET["id_atri"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 3) {
			$sql = "delete from sim_inventario_tipo_atributo WHERE id=".$_GET["id_atri"];
			mysqli_query($dbhandle,convertSQL($sql));
		}
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "<tr><td>&nbsp;</td></tr>";
			echo "</form>";
			$sql = "select * from sim_inventario_tipo_atributo where id_tipo=".$_GET["id"]." order by atributo";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)) {
				echo "<tr>";
	...
	if (($soption == 520) AND ($admin == true)) {
		if ($ssoption == 1) {
			...
			insertFunction ("sim_inventario_actuacion",$camposInsert,$datosInsert);
		}
		if ($ssoption == 2) {
			$camposUpdate = array("nombre","parada","duracion");
			$datosUpdate = array($_POST["nombre"],$_POST["parada"],$_POST["duracion"]);
			updateFunction ("sim_inventario_actuacion",$_GET["id"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 3) {
			$camposUpdate=array('visible');
			$datosUpdate=array(0);
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "<tr><td>&nbsp;</td></tr>";
			echo "</form>";
			$sql = "select * from sim_inventario_actuacion where visible=1 order by nombre";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)) {
				echo "<tr>";
	...
	if ($soption == 999){
		...
		while ($row = mysqli_fetch_array($result)){
			...
			if ($row["id_cliente"] > 0) {$tipo = 2; $id = $row["id_cliente"];}
