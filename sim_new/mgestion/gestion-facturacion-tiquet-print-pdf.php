<?php 
	...
	$sqlcabezera = "select * from sim_factura where id=".$_GET["id"];
	...
	$sql = "select * from sim_factura_cuerpo where id_factura=".$_GET["id"]." order by id";
	...
	$sqlxtp = "select * from sim_factura_tipos_pago where id=".$rowcabezera["id_tipo_pago"];
	$resultxtp = mysqli_query($dbhandle,convertSQL($sqlxtp));
	$rowxtp = mysqli_fetch_array($resultxtp);
	$pdf->Cell(60,4,$forma_de_pago.": ".$rowxtp["id_tipo"],0,1);
