...
if (($option == 1003) AND ($autorizado == true)) {
	$sqldiv = "select * from sim_divisas where predefinido=1";
	...
	if (($soption >= 0) and ($soption < 200)){
			echo "<table cellpadding=\"1\" cellspacing=\"2\" class=\"lista\">";
				echo "<tr>";
				$sqltipos2 = "select id,tipo from sim_factura_tipos where visible=1 order by orden,descripcion";
				$resulttipos2 = mysqli_query($dbhandle,convertSQL($sqltipos2));
				while ($rowtipos2 = mysqli_fetch_array($resulttipos2)) {
					$sqlpermiso = "select count(*) as total from sim_factura_tipos_permisos where id_tipo=".$rowtipos2["id"]." and id_usuario=".$userid;
					...
					if ($rowpermiso["total"] > 0) { 
						...
						echo "<td class=".$class2."><a href=\"index.php?op=1003&sop=0&id=".$rowtipos2["id"]."\" class=".$class2.">".$rowtipos2["id_tipo"]."</a></td>";
					} else { echo "<td style=\"background-color:black;color:white;border:1px solid black;text-align:center;vertical-align:middle;height:20px;width:150px;\"><a href=\"index.php?op=1003&sop=0\" class=\"menu\">".$rowtipos2["id_tipo"]."</a></td>"; }
				}
				echo "</tr>";
			echo "</table>";
		echo "</td>";
	...
	if (($soption == 0) or ($soption == 200)) {
		$sqltipos = "select * from sim_factura_tipos where id=".$_GET["id"].$_POST["id_tipo"];
			...
			echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
				...
				echo "</tr><tr style=\"background-color:silver;\">";
					...
					echo "<td style=\"width:10%\">";
						...
						$sqlft = "select id,tipo from sim_factura_tipos where visible=1 order by orden,descripcion";
						$resultft = mysqli_query($dbhandle,convertSQL($sqlft));
						echo "<option value=\"0\">".$Todas."</option>";
						while ($rowft = mysqli_fetch_array($resultft)) {
							if ($_POST["id_tipo"] == $rowft["id"]){
								echo "<option value=\"".$rowft["id"]."\" selected>".$rowft["id_tipo"]."</option>";
							} else {
								echo "<option value=\"".$rowft["id"]."\">".$rowft["id_tipo"]."</option>";
							}
						}
						echo "</select>";
					echo "</td>";
		...
		if (($_GET["id"] != "") or $_POST["id_tipo"] or $_POST["id_cliente"] or $_POST["ref_cli"] or $_POST["data_desde1"] or $_POST["data_fins1"] or $_POST["codigo"] or $_POST["articulo"] or $_POST["hist"]){
			if ($ssoption == 1) {
				$sqlcc = "select count(*) as total from sim_factura where visible=1 and numero=".$_POST["numero"]." and tipo=".$_POST["id_tipo"];
				...
				if ($rowcc["total"] == 0){
					...
					$datosInsert = array('numero'=>$_POST["numero"],'tipo'=>$_POST["id_tipo"],'id_subtipo'=>$_POST["id_subtipo"],'version'=>$_POST["version"],'numero_rfq'=>$_POST["numero_rfq"],'numero_cliente'=>$_POST["numero_cliente"],'fecha'=>$fecha,'fecha_prevision'=>$fecha_prev,'id_cliente'=>$_POST["id_cliente"]);
			...
			if ($ssoption == 2) {
				...
				updateFunction ("sim_factura",$_GET["id_fact"],$camposUpdate,$datosUpdate);

				$sqlfa = "select numero from sim_factura where visible=1 and tipo=".$id_tipus." order by numero desc";
				$resultfa = mysqli_query($dbhandle,convertSQL($sqlfa));
				$rowfa = mysqli_fetch_array($resultfa);
				$sqla = "select * from sim_factura where visible=1 and id=".$_GET["id_fact"];
				$resulta = mysqli_query($dbhandle,convertSQL($sqla));
				$rowa = mysqli_fetch_array($resulta);
				$sqlfav = "select version from sim_factura where visible=1 and numero=".$rowa["numero"]." order by version desc";
				$resultfav = mysqli_query($dbhandle,convertSQL($sqlfav));
				$rowfav = mysqli_fetch_array($resultfav);
				$sqlf = "select presu from sim_factura_tipos where visible=1 and id=".$id_tipus;
				$resultf = mysqli_query($dbhandle,convertSQL($sqlf));
				$rowf = mysqli_fetch_array($resultf);
				if (($rowa["id_tipo"] == $id_tipus) and ($rowf["presu"] == 1)){
					$numero = $rowa["numero"];
				...
				$datosInsert = array('id_origen'=>$rowa["id"],'numero'=>$numero,'version'=>$version,'tipo'=>$id_tipus,'id_subtipo'=>$rowa["id_subtipo"],'numero_cliente'=>$rowa["numero_cliente"],'fecha'=>$_POST["data"],'id_cliente'=>$rowa["id_cliente"],'notas'=>$rowa["notas"],'total_forzado'=>$rowa["total_forzado"],'recibos'=>$rowa["recibos"],'confirmada'=>$rowa["confirmada"],'confirmada_cliente'=>$rowa["confirmada_cliente"],'fecha_prevision'=>$_POST["data"],'fecha_entrega'=>$_POST["data"],'fecha_vencimiento'=>$_POST["data"],'nombre'=>$rowa["nombre"],'nif'=>$rowa["nif"],'direccion'=>$rowa["direccion"],'poblacion'=>$rowa["poblacion"],'cp'=>$rowa["cp"],'provincia'=>$rowa["provincia"],'id_pais'=>$rowa["id_pais"],'mail'=>$rowa["mail"],'telefono'=>$rowa["telefono"],'subtotal'=>$rowa["subtotal"],'descuento'=>$rowa["descuento"],'descuento_absoluto'=>$rowa["descuento_absoluto"],'subtotaldescuento'=>$rowa["subtotaldescuento"],'iva'=>$rowa["iva"],'total'=>$rowa["total"],'id_usuario'=>$rowa["id_usuario"],'id_tipo_pago'=>$rowa["id_tipo_pago"],'id_divisa'=>$rowa["id_divisa"],'div_cambio'=>$rowa["div_cambio"]);
				insertCabezera($datosInsert);

				$sqlfac = "select id from sim_factura where visible=1 order by id desc";
				$resultfac = mysqli_query($dbhandle,convertSQL($sqlfac));
				$rowfac = mysqli_fetch_array($resultfac);

				$sql = "select * from sim_factura_cuerpo where id_factura=".$_GET["id_fact"];
				$result = mysqli_query($dbhandle,convertSQL($sql));
				while ($row = mysqli_fetch_array($result)){
					if ($_POST["trasp"] == 2) {
						if ($_POST["traspasar_".$row["id"]] == true) {
							...
							$camposInsert = "id_origen,linea,id_cuerpo,id_factura,fecha_prevision,fecha_entrega,codigo,nombre,pvd,pvp,unidades,descuento,descuento_absoluto,subtotaldescuento,total,id_articulo,stock,prioridad,controlcalidad";
							$datosInsert = array($row["id"],$row["linea"],$row["id_cuerpo"],$rowfac["id"],$row["fecha_prevision"],$row["fecha_entrega"],$row["codigo"],$row["nombre"],$row["pvd"],$row["pvp"],$_POST["unitats_".$row["id"]],$descuento,$descuento_absoluto,$row["subtotaldescuento"],$total,$row["id_articulo"],$row["stock"],$row["prioridad"],$row["controlcalidad"]);
							insertFunction ("sim_factura_cuerpo",$camposInsert,$datosInsert);
						}
					}
					if ($_POST["trasp"] == 1) {
						$camposInsert = "id_origen,linea,id_cuerpo,id_factura,fecha_prevision,fecha_entrega,codigo,nombre,pvd,pvp,unidades,descuento,descuento_absoluto,subtotaldescuento,total,id_articulo,stock,prioridad,controlcalidad";
						$datosInsert = array($row["id"],$row["linea"],$row["id_cuerpo"],$rowfac["id"],$row["fecha_prevision"],$row["fecha_entrega"],$row["codigo"],$row["nombre"],$row["pvd"],$row["pvp"],$row["unidades"],$row["descuento"],$row["descuento_absoluto"],$row["subtotaldescuento"],$row["total"],$row["id_articulo"],$row["stock"],$row["prioridad"],$row["controlcalidad"]);
						insertFunction ("sim_factura_cuerpo",$camposInsert,$datosInsert);
					}
					if ($id_tipus != $rowa["id_tipo"]){
						$sqlc = "select id from sim_factura_cuerpo order by id desc";
						$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
						$rowc = mysqli_fetch_array($resultc);
						$sqlf = "select * from sim_files where id_elemento=".$row["id"];
						$resultf = mysqli_query($dbhandle,convertSQL($sqlf));
						while ($rowf = mysqli_fetch_array($resultf)){
							...
							insertFunction ("sim_files",$camposInsert,$datosInsert);
						}
					}
				}
				refactura($rowfac["id"]);
			...
			if ($ssoption == 3) {
				...
				$datosUpdate = array("0");
				updateFunction ("sim_factura",$_GET["id_fact"],$camposUpdate,$datosUpdate);
				$sqlc = "select id from sim_factura_cuerpo where id_factura=".$_GET["id_fact"];
				$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
				while ($rowc = mysqli_fetch_array($resultc)){
					$sql = "update sim_files set ";
					$sql = $sql."visible = 0";
			...
			if ($soption != 200){
				echo "<tr>";
					...
					$sql = "select numero from sim_factura where visible=1 AND tipo=".$rowtipos["id"]." order by numero desc";
					$result = mysqli_query($dbhandle,convertSQL($sql));
				...
				### CALCULO DE LA FECHA DE PREVISION DE ENTREGA
					...
					if ($rowtipos["tpv"] == 0) { 
						echo "<td>";
							...
							echo "<option value=\"0\">-</option>";
							$sql = "select id,nombre,cognom1,cognom2 from sim_clients where visible=1 ";
					...
					if ($rowtipos["v_subtipos"] == 1) {
						echo "<td>";
							echo "<select style=\"width:100px\" name=\"id_subtipo\">";
							echo "<option value=\"0\">-</option>";
							$sql = "select id,id_subtipo from sim_factura_subtipos where visible=1 and id_tipo=".$_GET["id"]." order by id_subtipo";
							$result = mysqli_query($dbhandle,convertSQL($sql));
							while ($row = mysqli_fetch_array($result)) {
								echo "<option value=\"".$row["id"]."\">".$row["id_subtipo"]."</option>";
							}
							echo "</select>";
						echo "</td>";
					} else {
						echo "<input type=\"hidden\" name=\"id_subtipo\" value=\"0\">";
						echo "<td></td>";
					}
				...
				$totalPagat = 0;
				$totalArticles = 0;
				$sql = "select * from sim_factura where visible=1";
...
#echo $sql;
	...
	if ($soption == 10) {
		$sqlf = "select id,tipo,aprovado from sim_factura_tipos where id=".$_GET["id"];
		$resultf = mysqli_query($dbhandle,convertSQL($sqlf));
		$rowf = mysqli_fetch_array($resultf);
			...
			echo "<tr>";
				echo "<td style=\"vertical-align:top;width:50%;text-align:center;background-color:silver;\">";
					echo "<center><table style=\"text-align:left;vertical-align:top;\">";
						echo "<tr>";
							echo "<th style=\"width:400px\">".$Eliminar." ".$rowf["id_tipo"]."</th>";
						echo "</tr><tr>";
					...
					echo "</table></center>";
				echo "</td>";
			$sql = "select aprovado from sim_factura where id=".$_GET["id_fact"];
			$result = mysqli_query($dbhandle,convertSQL($sql));
			$row = mysqli_fetch_array($result);
			$ver = 1;
						...
						echo "<table style=\"width:400px;background-color:grey;\">";
							echo "<tr>";
								...
								echo "<td><select name=\"id_tipus\" style=\"width:200px\">";
									echo "<option value=\"".$rowf["id"]."\">".$rowf["id_tipo"]."</option>";
									$sqlr = "select id_tipo_d,cerrar from sim_factura_tipos_relaciones where id_tipo_o=".$_GET["id"];
									$resultr = mysqli_query($dbhandle,convertSQL($sqlr));
									while ($rowr = mysqli_fetch_array($resultr)) {
										$sqld = "select * from sim_factura_tipos where id=".$rowr["id_tipo_d"];
										$resultd = mysqli_query($dbhandle,convertSQL($sqld));
										$rowd = mysqli_fetch_array($resultd);
										echo "<option value=\"".$rowd["id"].",".$rowr["cerrar"]."\">".$rowd["id_tipo"]."</option>";
									}
								echo "</select></td>";
	...
	if ($soption == 12) {
		...
		$sql = "select id_cliente,numero,fecha,tipo from sim_factura where visible=1 and id=".$_GET["id_fact"];
		$result = mysqli_query($dbhandle,convertSQL($sql));
		$row = mysqli_fetch_array($result);
		$sqlc = "select nombre from sim_clients where visible=1 and id=".$row["id_cliente"];
		$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
		$rowc = mysqli_fetch_array($resultc);
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "</tr>";
		echo "<form action=\"index.php?op=1003&sop=0&ssop=2&id=".$_GET["id"]."&id_fact=".$_GET["id_fact"]."\" method=\"post\">";
		$sqlcu = "select codigo,nombre,unidades,id from sim_factura_cuerpo where id_factura=".$_GET["id_fact"];
		...
		while ($rowcu = mysqli_fetch_array($resultcu)){
			...
			echo "<tr>";
				...
				echo "<td><input type=\"text\" name=\"data\" value=\"".$date."\" style=\"width:100px\"></td>";
				echo "<td><select name=\"id_tipus\" style=\"width:200px\">";
					$sqlf = "select id,tipo from sim_factura_tipos where id=".$row["id_tipo"];
					$resultf = mysqli_query($dbhandle,convertSQL($sqlf));
					$rowf = mysqli_fetch_array($resultf);
					echo "<option value=\"".$rowf["id"]."\">".$rowf["id_tipo"]."</option>";
					$sqlr = "select id_tipo_d,cerrar from sim_factura_tipos_relaciones where id_tipo_o=".$_GET["id"];
					$resultr = mysqli_query($dbhandle,convertSQL($sqlr));
					while ($rowr = mysqli_fetch_array($resultr)) {
						$sqld = "select id,tipo from sim_factura_tipos where id=".$rowr["id_tipo_d"];
						$resultd = mysqli_query($dbhandle,convertSQL($sqld));
						$rowd = mysqli_fetch_array($resultd);
					echo "<option value=\"".$rowd["id"].",".$rowr["cerrar"]."\">".$rowd["id_tipo"]."</option>";
					}
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "</tr><tr>";
				...
				echo "<td><select name=\"idioma\" style=\"width:90px\">";
					$sqli = "select idioma,descripcion,predefinido from sim_idiomas where visible=1";
	...
	if ($soption == 30) {
		if ($ssoption == 1) {
			$sql = "select * from sim_factura where id=".$_GET["id"];
			$result = mysqli_query($dbhandle,convertSQL($sql));
			$row = mysqli_fetch_array($result);
			### COPIA CABEZERA PLANTILLA
			$sqlxx = "select numero,numero_serie from sim_factura_recibos where visible=1 and id_factura=".$_GET["id"]." order by numero desc,numero_serie desc";
			...
			$camposInsert = "numero,numero_serie,id_factura,fecha,fecha_vencimiento,nombre,nif,direccion,poblacion,cp,provincia,total,id_cliente,id_usuario,id_tipo_pago,cobrada";
			$datosInsert = array($numero,$numero_serie,$_GET["id"],$fecha_emi,$fecha_ven,$row["nombre"],$row["nif"],$row["direccion"],$row["poblacion"],$row["cp"],$row["provincia"],$_POST["total"],$row["id_cliente"],$row["id_usuario"],$_POST["id_tipo_pago"],$_POST["cobrada"]);
			insertFunction ("sim_factura_recibos",$camposInsert,$datosInsert);
		}
		if ($ssoption == 2) {
			$fecha_emi = cambiarFormatoFechaYMD($_POST["fecha_emision"]);
			$fecha_ven = cambiarFormatoFechaYMD($_POST["fecha_vencimiento"]);
			...
			updateFunction ("sim_factura_recibos",$_GET["id_recibo"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 3) {
			$camposUpdate = array("visible");
			$datosUpdate = array("0");
		...
		if ($ssoption > 0){
			$sql = "select id,total from sim_factura where id=".$_GET["id"];
			$result = mysqli_query($dbhandle,convertSQL($sql));
			$row = mysqli_fetch_array($result);
			## COMPRUEBA SI ESTAN LOS RECIBOS REALIZADOS
			$sqlcalc = "select SUM(total) as total from sim_factura_recibos where visible=1 AND id_factura =".$row["id"];
			...
			if ($rowcalc["total"] >= $row["total"]) {
				...
				$datosUpdate = array("1");
				updateFunction ("sim_factura",$row["id"],$camposUpdate,$datosUpdate);
			} else {
				$camposUpdate = array("recibos");
				$datosUpdate = array("0");
				updateFunction ("sim_factura",$row["id"],$camposUpdate,$datosUpdate);
			...
			$sqlcalc = "select SUM(total) as total from sim_factura_recibos where visible=1 AND id_factura =".$row["id"]." and cobrada=1";
			$resultcalc = mysqli_query($dbhandle,convertSQL($sqlcalc));
			$rowcalc = mysqli_fetch_array($resultcalc);
			if ($rowcalc["total"] >= $row["total"]) {
		...
		$sqlf = "select fecha_vencimiento,id_cliente,tipo,id_divisa,fecha,numero,total from sim_factura where id=".$_GET["id"];
		$resultf = mysqli_query($dbhandle,convertSQL($sqlf));
		$rowf = mysqli_fetch_array($resultf);
		$sqlc = "select id,nombre,entidadbancaria,domiciliobancario,cuentabancaria,dias_vencimiento,dias from sim_clients where id=".$rowf["id_cliente"];
		$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
		$rowc = mysqli_fetch_array($resultc);
		$sqltipos = "select tipo from sim_factura_tipos where id=".$rowf["id_tipo"];
		$resulttipos = mysqli_query($dbhandle,convertSQL($sqltipos));
		$rowtipos = mysqli_fetch_array($resulttipos);
		$sqldiv2 = "select abrev from sim_divisas where id=".$rowf["id_divisa"];
		$resultdiv2 = mysqli_query($dbhandle,convertSQL($sqldiv2));
		$rowdiv2 = mysqli_fetch_array($resultdiv2);
		$sqlre = "select sum(total) as total_recibos from sim_factura_recibos where visible=1 and id_factura=".$_GET["id"];
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			echo "<tr style=\"background-color:silver;\">";
				echo "<td style=\"text-align:left;vertical-align:top;width:25%;\">";
					echo "<table>";
						echo "<tr><th>".$rowtipos["id_tipo"]."</th><td>".$rowf["numero"]."</td></tr>";
				...
				echo "<td style=\"text-align:left;vertical-align:top;width:100%;\" colspan=\"3\">";
					echo "<table cellpadding=\"1\" cellspacing=\"0\">";
						...
						echo "<tr>";
							...
							$sqlt = "select count(*) as total from sim_clients_dias_recibos where id_cliente=".$rowc["id"]." order by dia";
							...
							if ($rowt["total"] == 0) {
								...
								$sqlz = "select dia from sim_clients_dias_recibos where id_cliente=".$rowc["id"]." order by dia";
							...
							$sqlfp = "select id,tipo from sim_factura_tipos_pago order by tipo";
							$resultfp = mysqli_query($dbhandle,convertSQL($sqlfp));
							while ($rowfp = mysqli_fetch_array($resultfp)) {
								if ($rowfp["id"] == $row["id_tipo_pago"]) {
									echo "<option value=\"".$rowfp["id"]."\" selected>".$rowfp["id_tipo"]."</option>";
								} else {
									echo "<option value=\"".$rowfp["id"]."\">".$rowfp["id_tipo"]."</option>";
								}
							}
							echo "</select></td>";
							...
							echo "<td></td>";
						echo "</tr>";
						$sqlr = "select * from sim_factura_recibos where visible=1 and id_factura=".$_GET["id"]." order by numero desc, numero_serie desc";
						$resultr = mysqli_query($dbhandle,convertSQL($sqlr));
						while ($rowr = mysqli_fetch_array($resultr)) {
							echo "<tr>";
								...
								echo "<td class=\"submit\">";
									...
									echo "</form>";
								echo "</td>";
#								$sqlx = "select * from sim_clients where id=".$rowf["id_cliente"];
...
#								}
	...
	if ($soption == 100) {
		if ($ssoption == 1) {
			$sql = "select * from sim_clients where id=".$_POST["id_cliente"];
			$result = mysqli_query($dbhandle,convertSQL($sql));
			$row = mysqli_fetch_array($result);
			$sqlc = "select nombre,apellido1,apellido2,mail,telefono from sim_clients_contactos where pred=1 and id_client=".$_POST['id_cliente'];
			$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
			$rowc = mysqli_fetch_array($resultc);
			...
			if ($row["id_direccion_envio"] <> 0) {
				$sql3 = "select * from sim_clients_envios where id=".$row["id_direccion_envio"];
			...
			updateFunction ("sim_factura",$_GET["id"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 2) {
			$sqlc = "select nombre,apellido1,apellido2,mail,telefono from sim_clients_contactos where id=".$_POST["id_contacto"];
			$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
			$rowc = mysqli_fetch_array($resultc);
		...
		if ($ssoption == 3) {
			$sql = "select * from sim_clients where id=".$_GET["id_cliente"];
			$result = mysqli_query($dbhandle,convertSQL($sql));
			$row = mysqli_fetch_array($result);
			if ($row["id_direccion_envio"] == 0) {
				$edireccion= $row["direccion"];
		...
		if ($ssoption == 4) {
			$fecha = date("Y-m-d");
			$sqlf = "select fecha_prevision,fecha_entrega from sim_factura where id=".$_GET["id"];
			$resultf = mysqli_query($dbhandle,convertSQL($sqlf));
			$rowf = mysqli_fetch_array($resultf);
			if ($rowf["fecha_prevision"] != cambiarFormatoFechaDMY($_POST["fecha_prevision"])){
				$camposInsert = "id_factura, id_usuario, fecha_ant, fecha";
				$datosInsert = array($_GET["id"],$userid,$rowf["fecha_prevision"],$fecha);
				insertFunction ("sim_factura_cambio_fecha_prevision",$camposInsert,$datosInsert);
			}
			if ($rowf["fecha_entrega"] != cambiarFormatoFechaDMY($_POST["fecha_entrega"])){
				...
				insertFunction ("sim_factura_cambio_fecha_entrega",$camposInsert,$datosInsert);
			}
			...
			$camposUpdate = array("numero","version","numero_rfq","numero_cliente","fecha","fecha_prevision","fecha_entrega","fecha_vencimiento","id_cliente","nombre","nif","direccion","poblacion","cp","provincia","id_pais","mail","telefono","edireccion","epoblacion","ecp","eprovincia","notas","imp_exp","id_divisa","div_cambio","id_pagador","id_usuario","id_factura_dades_origen_iban","cnombre","cmail","ctelefono","id_contrato");
			$datosUpdate = array($_POST["numero"],$_POST["version"],$_POST["numero_rfq"],$_POST["numero_cliente"],$fechaf,$fecha_prevf,$fecha_entf,$fecha_venf,$_POST["id_cliente"],$_POST["nombre"],$_POST["nif"],$_POST["direccion"],$_POST["poblacion"],$_POST["cp"],$_POST["provincia"],$_POST["id_pais"],$_POST["mail"],$_POST["telefono"],$_POST["edireccion"],$_POST["epoblacion"],$_POST["ecp"],$_POST["eprovincia"],$_POST["notas"],$_POST["imp_exp"],$_POST["id_divisa"],$_POST["div_cambio"],$_POST["id_pagador"],$_POST["id_usuario"],$_POST["id_factura_dades_origen_iban"],$_POST["cnombre"],$_POST["cmail"],$_POST["ctelf"],$_POST["id_contrato"]);
			updateFunction ("sim_factura",$_GET["id"],$camposUpdate,$datosUpdate);
		}
#### INSERTA DATOS EN EL CUERPO
		if ($ssoption == 5) {
			...
			if ($_POST["codigo"] != ''){
				$sql = "select * from sim_articulos where codigo='".$_POST["codigo"]."'";
			} elseif ($_POST["nombre"] != ''){
				$sql = "select * from sim_articulos where nombre='".$_POST["nombre"]."'";
			}
			$result = mysqli_query($dbhandle,convertSQL($sql));
			$row = mysqli_fetch_array($result);
			$sqls = "select pvd,pvp from sim_articulos_precios  where id_articulo=".$row["id"];
			$results = mysqli_query($dbhandle,convertSQL($sqls));
			$rows = mysqli_fetch_array($results);
			if ($row) {
				$total = $_POST["unidades"] * $rows["pvp"];
				$camposInsert = "id_factura,linea,codigo,nombre,pvd,pvp,unidades,descuento,descuento_absoluto,total,fecha_prevision,id_articulo";
				$datosInsert = array($_GET["id"],$_POST["linea"],$row["codigo"],$row["nombre"],$rows["pvd"],$rows["pvp"],$_POST["unidades"],$_POST["descuento"],$_POST["descuento_absoluto"],$total,$fecha_prev,$row["id"]);
				insertFunction ("sim_factura_cuerpo",$camposInsert,$datosInsert);
			} else {
				$total = $_POST["unidades"] * $_POST["pvp"];
				$camposInsert = "id_factura,linea,codigo,nombre,pvd,pvp,unidades,descuento,descuento_absoluto,total,fecha_prevision,id_articulo";
				$datosInsert = array($_GET["id"],$_POST["linea"],$_POST["codigo"],$_POST["nombre"],$_POST["pvd"],$_POST["pvp"],$_POST["unidades"],$_POST["descuento"],$_POST["descuento_absoluto"],$total,$fecha_prev,0,);
				insertFunction ("sim_factura_cuerpo",$camposInsert,$datosInsert);
			}
			refactura($_GET["id"]);
		}
#### ELIMINAR CUERPO
		if ($ssoption == 6) {
			deleteFunction ("sim_factura_cuerpo",$_GET["id_cuerpo"]);
			$sql = "update sim_files set ";
			$sql = $sql."visible = 0";
			$sql = $sql." WHERE id_elemento=".$_GET["id_cuerpo"]."";
			mysqli_query($dbhandle,convertSQL($sql));
			refactura($_GET["id_factura"]);
			$num = 1;
			$sqll = "select linea,id from sim_factura_cuerpo where id_factura=".$_GET["id"]." order by linea";
			...
			while ($rowl = mysqli_fetch_array($resultl)) {
				if ($num == $rowl["linea"]) {
					...
					updateFunction ("sim_factura_cuerpo",$rowl["id"],$camposUpdate,$datosUpdate);
					$num++;
				}
			}
		}
		if ($ssoption == 9) {
			$sql = "select id,fecha_prevision from sim_factura_cuerpo where id_factura=".$_GET["id"]." order by id";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)) {
				if ($row["fecha_prevision"] != cambiarFormatoFechaDMY($_POST["fecha_prevision".$row["id"].""])){
					$fecha = date("Y-m-d");
					$camposInsert = "id_factura,id_usuario,fecha_ant,fecha,id_cuerpo";
					$datosInsert = array($_GET["id"],$userid,$row["fecha_prevision"],$fecha,$row["id"]);
					insertFunction ("sim_factura_cambio_fecha_prevision_cuerpo",$camposInsert,$datosInsert);
				}
				...
				updateFunction ("sim_factura_cuerpo",$row["id"],$camposUpdate,$datosUpdate);
			}
			if ($_POST["descuento"] == "") {
				$descuento=0;
		...
		if ($ssoption == 11) {
			...
			$datosUpdate = array("1");
			updateFunction ("sim_factura_cuerpo",$_GET["id_linea"],$camposUpdate,$datosUpdate);
			refactura($_GET["id"]);
		}
		if ($ssoption == 12) {
			$camposUpdate = array("suma");
		...
		if ($ssoption == 13) {
			$camposUpdate = array("aprobado");
			$datosUpdate = array("1");
			updateFunction ("sim_factura_cuerpo",$_GET["id_linea"],$camposUpdate,$datosUpdate);
			contaAprovats($_GET["id_linea"],$_GET["id"]);
		}
		if ($ssoption == 18) {
			$sql = "select id,total_forzado,id_divisa,total,subtotal,descuento_absoluto,subtotaldescuento from sim_factura where id=".$_GET["id"];
			$result = mysqli_query($dbhandle,convertSQL($sql));
			$row = mysqli_fetch_array($result);
			$sqldivi = "select cambio from sim_divisas_cambios where visible=1 and id_divisa_origen=".$row["id_divisa"]." and id_divisa_destino=".$_POST["id_div_cambio"];
			$resultdivi = mysqli_query($dbhandle,convertSQL($sqldivi));
			$rowdivi = mysqli_fetch_array($resultdivi);
			$camposUpdate = array("subtotal","descuento_absoluto","subtotaldescuento","total","total_forzado","div_cambio","id_divisa");
			$datosUpdate = array($row["subtotal"]*$rowdivi["cambio"],$row["descuento_absoluto"]*$rowdivi["cambio"],$row["subtotaldescuento"]*$rowdivi["cambio"],$row["total"]*$rowdivi["cambio"],$row["total_forzado"]*$rowdivi["cambio"],$rowdivi["cambio"],$_POST["id_div_cambio"]);
			updateFunction ("sim_factura",$_GET["id"],$camposUpdate,$datosUpdate);
			$sqll = "select id,pvd,pvp,total,descuento_absoluto,subtotaldescuento from sim_factura_cuerpo where id_factura=".$row["id"];
			$resultl = mysqli_query($dbhandle,convertSQL($sqll));
			while ($rowl = mysqli_fetch_array($resultl)) {
				$camposUpdate = array("pvd","descuento_absoluto","subtotaldescuento","total","pvp");
				$datosUpdate = array($rowl["pvd"]*$rowdivi["cambio"],$rowl["descuento_absoluto"]*$rowdivi["cambio"],$rowl["subtotaldescuento"]*$rowdivi["cambio"],$rowl["total"]*$rowdivi["cambio"],$rowl["pvp"]*$rowdivi["cambio"]);
				updateFunction ("sim_factura_cuerpo",$rowl["id"],$camposUpdate,$datosUpdate);
			}
			refactura($_GET["id"]);
		}
		if (($ssoption >= 1) and ($ssoption <= 18)) {
			...
			insertFunction ("sim_factura_modificacion",$camposInsert,$datosInsert);
		}
		echo boton(array("op=1003&sop=104&id_tipo=".$_GET["id_tipo"]."&id=".$_GET["id"]),array($Modificadores));
		$sql = "select * from sim_factura where id=".$_GET["id"];
		$result = mysqli_query($dbhandle,convertSQL($sql));
		$row = mysqli_fetch_array($result);
		$sqlf = "select tpv,id,tipo from sim_factura_tipos where id=".$row["id_tipo"];
		$resultf = mysqli_query($dbhandle,convertSQL($sqlf));
		$rowf = mysqli_fetch_array($resultf);
		echo "<table cellpadding=\"1\" cellspacing=\"10\" class=\"lista\">";
			echo "<tr>";
						...
						if ($row["id_tipo_pago"] == 0) { $color = "#FF6347"; } elseif ($row["id_tipo_pago"] <> 0) {$color = "#32CD32";}
								echo "<td style=\"background-color:".$color.";width:450px;vertical-align:middle;height:50px;text-align:center\">";
									...
									echo "<select name=\"id_tipo_pago\" style=\"width:150px;\">";
										echo "<option value=\"0\">-</option>";
										$sql1 = "select id,tipo from sim_factura_tipos_pago order by id";
										$result1 = mysqli_query($dbhandle,convertSQL($sql1));
										while ($row1 = mysqli_fetch_array($result1)) {
											if ($row["id_tipo_pago"] == $row1["id"]) {
												echo "<option value=\"".$row1["id"]."\" selected>".$row1["id_tipo"]."</option>";
											} else {
												echo "<option value=\"".$row1["id"]."\">".$row1["id_tipo"]."</option>";
											}
										}
										echo "</select>";
										if ($cambio_pago == true) { 
						...
						echo "<table cellpadding=\"1\" cellspacing=\"0\" style=\"width:100%\">";
							echo "<tr>";
								...
								echo "<td><select name=\"id_cliente\">";
									echo "<option value=\"0\">-</option>";
									$sqltipos = "select presu,v_fecha_prevision,v_subtipos from sim_factura_tipos where id=".$row["id_tipo"]." order by id";
									$resulttipos = mysqli_query($dbhandle,convertSQL($sqltipos));
									$rowtipos = mysqli_fetch_array($resulttipos);
									$sqlx = "select id,nombre,cognom1,cognom2 from sim_clients where visible=1 order by nombre";
									$resultx = mysqli_query($dbhandle,convertSQL($sqlx));
								...
								echo "<td><select name=\"id_contacto\">";
									echo "<option value=\"0\">-</option>";
									$sqlcc = "select id,nombre,apellido1,apellido2 from sim_clients_contactos where id_client=".$row["id_cliente"]." order by nombre";
									$resultcc = mysqli_query($dbhandle,convertSQL($sqlcc));
								...
								echo "<td><select name=\"id_div_cambio\">";
									echo "<option value=\"0\">-</option>";
									$sqldi = "select abrev from sim_divisas where visible=1 and id=".$row["id_divisa"];
									$resultdi = mysqli_query($dbhandle,convertSQL($sqldi));
									$rowdi = mysqli_fetch_array($resultdi);
									$sqldivc = "select id_divisa_destino,cambio from sim_divisas_cambios where id_divisa_origen=".$row["id_divisa"];
									...
									while ($rowdivc = mysqli_fetch_array($resultdivc)) {
										if ($rowdivc["id_divisa_destino"] != $row["id_divisa"]) {
											$sqldivs = "select id,divisa,abrev from sim_divisas where visible=1 and id=".$rowdivc["id_divisa_destino"];
											$resultdivs = mysqli_query($dbhandle,convertSQL($sqldivs));
											$rowdivs = mysqli_fetch_array($resultdivs);
											echo "<option value=\"".$rowdivs["id"]."\">".$rowdivs["divisa"]." (1 ".$rowdi["abrev"]." = ".$rowdivc["cambio"]." ".$rowdivs["abrev"].")</option>";
										}
									}
									echo "</select>";
								echo "</td>";
						...
						echo "<table cellpadding=\"1\" cellspacing=\"0\" bgcolor=\"#c0c0c0\" style=\"width:100%\">";
							echo "<tr>";
										...
										echo "<tr>";
											echo "<td style=\"vertical-align:top;width:45%\">";
												echo "<table style=\"width:100%;\">";
													echo "<tr><th>".$rowf["id_tipo"]."</th><td><input style=\"width:80%\" type=\"Text\" name=\"numero\" value=\"".$row["numero"]."\"><strong>&nbsp;/&nbsp;</strong><input type=\"Text\" name=\"version\" style=\"width:10%\" value=\"".$row["version"]."\"></td></tr>";
													if ($rowtipos["presu"] == 1){
								...
								echo "<td style=\"vertical-align:top;width:30%\">";
									echo "<table style=\"width:100%\">";
									$sqlpermiso2 = "select admin from sim_factura_tipos_permisos where id_tipo=".$rowf["id"]." and id_usuario=".$userid;
									...
									$rowpermiso2 = mysqli_fetch_array($resultpermiso2);
										echo "<tr>";
											...
											echo "<td style=\"width:80%;\">";
												...
												echo "<option value=\"0\">-</option>";
												$sql1a = "select id,descripcion from sim_contratos where activo=1 and id_cliente=".$row["id_cliente"];
												$result1a = mysqli_query($dbhandle,convertSQL($sql1a));
												while ($row1a = mysqli_fetch_array($result1a)) {
													if ($row["id_contrato"] == $row1a["id"]) { 
														echo "<option value=\"".$row1a["id"]."\" selected>".$row1a["descripcion"]."</option>";
													} else {
														echo "<option value=\"".$row1a["id"]."\">".$row1a["descripcion"]."</option>";
													}
												}
												echo "</select>";
											echo "</td>";
										echo "</tr><tr>";
											...
											echo "<td style=\"width:80%;\">";
												if ($rowpermiso2["admin"] == 0){
													echo "<select name=\"id_factura_dades_origen_iban\" disabled>";
												} else {
													echo "<select name=\"id_factura_dades_origen_iban\">";
												}
												$seleccionar = 0;
												$sql1 = "select id,entidad_bancaria,descripcion from sim_factura_dades_origen_iban";
												$result1 = mysqli_query($dbhandle,convertSQL($sql1));
												while ($row1 = mysqli_fetch_array($result1)) {
													if ($row["id_factura_dades_origen_iban"] == $row1["id"]) { 
										...
										echo "</tr><tr>";
											...
											echo "<td>";
												if ($rowpermiso2["admin"] == 0){
													echo "<select name=\"id_usuario\" disabled>";
												} else {
													echo "<select name=\"id_usuario\">";
												}
												echo "<option value=\"0\">-</option>";
												$sql1 = "select id,usuario from sim_users where sgm=1 order by usuario";
												$result1 = mysqli_query($dbhandle,convertSQL($sql1));
												while ($row1 = mysqli_fetch_array($result1)) {
													if ($row["id_usuario"] == $row1["id"]) {
										...
										echo "</tr><tr>";
											...
											echo "</td>";
										echo "</tr><tr>";
												echo "<th>".$Divisa." ".$rowf["id_tipo"]."</th>";
												echo "<td><select name=\"id_divisa\">";
													echo "<option value=\"0\">-</option>";
													$sql1 = "select id,divisa,predefinido from sim_divisas where visible=1";
													$result1 = mysqli_query($dbhandle,convertSQL($sql1));
													while ($row1 = mysqli_fetch_array($result1)) {
								...
								echo "</td>";
							echo "</tr>";
							echo "<tr><td class=\"submit\" style=\"text-align:center;\" colspan=\"2\"><input type=\"Submit\" value=\"".$Modificar."\"></td></tr>";
						echo "</table>";
						echo "</form>";
					}
					echo "</td>";
				echo "</tr><tr>";
					echo "<td style=\"width:100%;vertical-align:top;text-align:left;\">";
						$sql2 = "select abrev from sim_divisas where visible=1 and id=".$row["id_divisa"];
						...
						echo "<table cellpadding=\"1\" cellspacing=\"0\">";
							...
							echo "</tr><tr>";
								...
								echo "<td></td>";
								$sqlxx = "select linea from sim_factura_cuerpo where id_factura=".$_GET["id"]." order by linea desc";
								$resultxx = mysqli_query($dbhandle,convertSQL($sqlxx));
								$rowxx = mysqli_fetch_array($resultxx);
								$linea = $rowxx["linea"] +1;
						...
						$sql = "select * from sim_factura_cuerpo where id_factura=".$_GET["id"]." order by linea";
						$result = mysqli_query($dbhandle,convertSQL($sql));
						while ($row = mysqli_fetch_array($result)) {
							$color = "white";
							if ($id_tipo == 5) {
							...
							echo boton_form("op=1003&sop=101&id_tipo=".$_GET["id_tipo"]."&id_factura=".$_GET["id"]."&id=".$row["id"],$Opciones);
				...
				#				}
								if ($row["pvd"] == 0){
									$sqla = "select preu_cost from sim_articulos_costos where visible=1 and id_cuerpo=".$row["id"]." and aprobado=1";
									$resulta = mysqli_query($dbhandle,convertSQL($sqla));
									$rowa = mysqli_fetch_array($resulta);
							...
							echo "<tr><td colspan=\"12\"></td><td class=\"submit\"><input type=\"Submit\" value=\"".$Modificar."\"></td></tr>";
							$sql = "select id,subtotal,descuento,iva,total_forzado,descuento_absoluto,total,retenciones from sim_factura where id=".$_GET["id"];
							$result = mysqli_query($dbhandle,convertSQL($sql));
							$row = mysqli_fetch_array($result);
							echo "<tr><th style=\"text-align:right;\" colspan=\"12\">".$Subtotal."</th><td style=\"text-align:right;\">".number_format ($row["subtotal"],3)."</td><td>".$row2["abrev"]."</td></tr>";
							echo "<tr><th style=\"text-align:right;\" colspan=\"12\">Desc.%</th><td><input type=\"Text\" name=\"descuento\" style=\"text-align:right;\" value=\"".number_format ($row["descuento"],3)."\"></td><td>%</td></tr>";
							echo "<tr><th style=\"text-align:right;\" colspan=\"12\">Desc. ".$row2["abrev"]."</th><td><input type=\"Text\" name=\"descuento_absoluto\" style=\"text-align:right;\" value=\"".number_format ($row["descuento_absoluto"],3)."\"></td><td>".$row2["abrev"]."</td></tr>";
							echo "<tr><th style=\"text-align:right;\" colspan=\"12\">IVA</th><td><input type=\"Text\" name=\"iva\" style=\"text-align:right;\" value=\"".$row["iva"]."\"></td><td>%</td></tr>";
							echo "<tr><th style=\"text-align:right;\" colspan=\"12\">".$Retenciones."</th><td><input type=\"Text\" name=\"retenciones\" style=\"text-align:right;\" value=\"".$row["retenciones"]."\"></td><td>%</td></tr>";
							$color_fondo_total = "white";
							if ($row["total_forzado"] == 1) { $color_fondo_total = "#FF6347"; }
							echo "<tr><th style=\"text-align:right;\" colspan=\"12\">".$Total."</th><td><input type=\"Text\" name=\"total\" style=\"text-align:right;background-color:".$color_fondo_total.";\" value=\"".number_format ($row["total"],3)."\"></td><td>".$row2["abrev"]."</td></tr>";
							echo "<tr><td colspan=\"12\"></td><td class=\"submit\"><input type=\"Submit\" value=\"".$Modificar."\"></td></tr>";
							echo "</form>";
						echo "</table>";
					echo "</td>";
		...
		echo "<table cellpadding=\"1\" cellspacing=\"1\" class=\"lista\">";
			echo "<tr>";
				echo "<td style=\"vertical-align:top;width:50%;text-align:center;background-color:silver;\">";
					...
					echo "</tr></table>";
					echo "<form action=\"index.php?op=1003&sop=102&id=".$_GET["id"]."&id_factura=".$_GET["id_factura"]."&id_tipo=".$_GET["id_tipo"]."\" method=\"post\">";
					echo "<table><tr>";
						echo boton_form("op=1003&sop=102&id=".$_GET["id"]."&id_factura=".$_GET["id_factura"]."&id_tipo=".$_GET["id_tipo"],$Convertir);
				...
				echo "<td style=\"vertical-align:top;width:50%;text-align:center;background-color:silver;\">";
					...
					echo "</tr></table><table><tr>";
						echo boton_form("op=1003&sop=103&id=".$_GET["id"]."&id_factura=".$_GET["id_factura"]."&id_tipo=".$_GET["id_tipo"],$Eliminar);
					echo "</tr><tr><td>&nbsp;</td>";
	...
	if ($soption == 102) {
		$sqlc = "select codigo,nombre,unidades,pvd,pvp from sim_factura_cuerpo where id=".$_GET["id"];
		$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
		$rowc = mysqli_fetch_array($resultc);
		$sqlcf = "select id_divisa from sim_factura where id=".$_GET["id_factura"];
		$resultcf = mysqli_query($dbhandle,convertSQL($sqlcf));
		$rowcf = mysqli_fetch_array($resultcf);

		$sqlar = "select id from sim_articulos where codigo='".$rowc["codigo"]."' or nombre='".$rowc["nombre"]."'";
		...
		if (!$rowar){
			...
			insertFunction ("sim_articulos",$camposInsert,$datosInsert);

			$sql = "select id from sim_articulos where codigo='".$rowc["codigo"]."' and nombre='".$rowc["nombre"]."'";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			$row = mysqli_fetch_array($result);

			$camposInsert = "id_articulo,unidades,pvd,id_divisa_pvd,pvp,id_divisa_pvp,vigente,fecha,id_usuario";
			$datosInsert = array($row["id"],$rowc["unidades"],$rowc["pvd"],$rowcf["id_divisa"],$rowc["pvp"],$rowcf["id_divisa"],1,date("Y-m-d"),$userid);
			insertFunction ("sim_articulos_precios",$camposInsert,$datosInsert);

			$camposUpdate = array("id_articulo","id_cuerpo");
			$datosUpdate = array($row["id"],0);
			updateFunction ("sim_articulos_costos",$_GET["id"],$camposUpdate,$datosUpdate);

			echo "<table cellpadding=\"1\" cellspacing=\"\" class=\"lista\">";
				echo "<tr>";
					echo boton_form("op=1003&sop=100&id=".$_GET["id_factura"]."&id_tipo=".$_GET["id_tipo"],$Volver);
			...
			echo "<table cellpadding=\"1\" cellspacing=\"\" class=\"lista\">";
				echo "<tr>";
					echo boton_form("op=1003&sop=100&id=".$_GET["id_factura"]."&id_tipo=".$_GET["id_tipo"]."&id_tipo=".$_GET["id_tipo"],$Volver);
				echo "</tr>";
			echo "</table>";
		}
	}
	...
	if ($soption == 103) {
		echo "<center>";
		echo "<br><br>".$pregunta_eliminar;
		echo boton(array("op=1003&sop=100&ssop=6&id=".$_GET["id_factura"]."&id_cuerpo=".$_GET["id"]."&id_tipo=".$_GET["id_tipo"],"op=1003&sop=100&id=".$_GET["id_factura"]."&id_tipo=".$_GET["id_tipo"]),array($Si,$No));
		echo "</center>";
	}
	...
	if ($soption == 104) {
		...
		echo "<table cellpadding=\"3\" cellspacing=\"0\" class=\"lista\">";
			echo "<tr style=\"background-color:silver;\">";
				...
				echo "<th>".$Fecha."</th>";
			echo "</tr>";
			$sql = "select id_usuario,fecha from sim_factura_modificacion where id_factura=".$_GET["id"]." order by fecha";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)) {
				$sqlu = "select usuario from sim_users where id=".$row["id_usuario"];
	...
	if ($soption == 105) {
		...
		echo "<table cellpadding=\"2\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "</tr>";
			if ($_GET["classe"] == 1) {
				$sql = "select id_usuario,fecha,fecha_ant from sim_factura_cambio_fecha_prevision where id_factura=".$_GET["id"];
			}
			if ($_GET["classe"] == 2) {
				$sql = "select id_usuario,fecha,fecha_ant from sim_factura_cambio_fecha_entrega where id_factura=".$_GET["id"];
			}
			if ($_GET["classe"] == 3) {
				$sql = "select id_usuario,fecha,fecha_ant from sim_factura_cambio_fecha_prevision_cuerpo where id_factura=".$_GET["id"]." and id_cuerpo=".$_GET["id_cuerpo"];
			}
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)){
				$sqlu = "select usuario from sim_users where id=".$row["id_usuario"];
	...
	if ($soption == 107) {
		...
		if ($ssoption == 2) {
			$sqlf = "select name from sim_files where id=".$_GET["id_archivo"];
			$resultf = mysqli_query($dbhandle,convertSQL($sqlf));
			$rowf = mysqli_fetch_array($resultf);
			deleteFunction ("sim_files",$_GET["id_archivo"]);
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			echo "<tr>";
				echo "<td style=\"width:70%;vertical-align:top;\">";
					echo "<h4>".$Archivos." :</h4>";
						echo "<table>";
						$sql = "select id,nombre from sim_files_tipos order by nombre";
						$result = mysqli_query($dbhandle,convertSQL($sql));
						while ($row = mysqli_fetch_array($result)) {
							$sqlele = "select id,name,size from sim_files where id_tipo=".$row["id"]." and tipo_id_elemento=0 and id_elemento=".$_GET["id"];
				...
				echo "</td><td style=\"width:200px;vertical-align:top;\">";
					...
					echo "<select name=\"id_tipo\" style=\"width:200px\">";
						$sql = "select * from sim_files_tipos order by nombre";
						$result = mysqli_query($dbhandle,convertSQL($sql));
						while ($row = mysqli_fetch_array($result)) {
	...
	if ($soption == 300) {
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "<form method=\"post\" action=\"".$urlmgestion."/mgestion/gestion-facturacion-print-pdf.php?\" target=\"_blank\">";
				echo "<td>";
					echo "<select name=\"id_tipo\" style=\"width:200px\">";
					$sqlx = "select id,tipo from sim_factura_tipos where visible=1 order by tipo";
					$resultx = mysqli_query($dbhandle,convertSQL($sqlx));
					while ($rowx = mysqli_fetch_array($resultx)) {
						echo "<option value=\"".$rowx["id"]."\">".$rowx["id_tipo"]."</option>";
					}
					echo "</select>";
				echo "</td>";
				$data = cambiarFormatoFechaDMY(date("Y-m-d"));
				...
				echo "<td><select name=\"idioma\" style=\"width:90px\">";
				echo "<option value=\"0\">-</option>";
				$sqli = "select predefinido,idioma,descripcion from sim_idiomas where visible=1";
				$resulti = mysqli_query($dbhandle,convertSQL($sqli));
				while ($rowi = mysqli_fetch_array($resulti)) {
					if ($rowi["predefinido"] == 1) {
						echo "<option value=\"".$rowi["idioma"]."\" selected>".$rowi["descripcion"]."</option>";
		...
		while ($dia_actual<=$mes_ultimo){
			...
			echo "<td style=\"background-color:".$color_fondo."\">";
				echo "<table cellspacing=\"0\" cellpadding=\"0\" style=\"width:100%\">";
					echo "<tr><th>".$d."</th></tr>";
					$sql = "select * from sim_factura_calendario where fecha='".$dia_actual."'";
					$result = mysqli_query($dbhandle,convertSQL($sql));
					$row = mysqli_fetch_array($result);
	...
	if ($soption == 420) {
		...
		echo "<table cellpadding=\"1\" cellspacing=\"10\" class=\"lista\">";
			...
			echo "</tr>";
			$sqltipos = "select id,tipo from sim_factura_tipos where visible=1 order by orden";
			$resulttipos = mysqli_query($dbhandle,convertSQL($sqltipos));
			while ($rowtipos = mysqli_fetch_array($resulttipos)){
				...
				for ($x = 1; $x <= 12; $x++) {
					...
					$sql = "select sum(subtotaldescuento) as total1, sum(total) as total3,count(*) as total4 from sim_factura where visible=1 and tipo=".$rowtipos["id"]." and ((fecha>= '".$y."-".$m."-01') and (fecha<'".$yf."-".$mf."-01'))";
#					if ($rowtipos["v_recibos"] == 1) { $sql = $sql." and cobrada=1"; }
	...
	if ($soption == 421) {
		...
		echo "<table cellpadding=\"1\" cellspacing=\"10\" class=\"lista\">";
			...
			echo "</tr><tr>";
			$sqltipos = "select id,tipo,v_recibos from sim_factura_tipos where visible=1 order by orden";
			$resulttipos = mysqli_query($dbhandle,convertSQL($sqltipos));
			while ($rowtipos = mysqli_fetch_array($resulttipos)){
				$totala1 = 0;
				$totala3 = 0;
				...
				for ($x = 1; $x <= $diasmes; $x++) {
					...
					$sql = "select sum(subtotaldescuento) as total1, sum(total) as total3,count(*) as total4 from sim_factura where visible=1 and tipo=".$rowtipos["id"]." and fecha='".$yact."-".$mact."-".$d."'";
					if ($rowtipos["v_recibos"] == 1) { $sql = $sql." and cobrada=1"; }
					$result = mysqli_query($dbhandle,convertSQL($sql));
					$row = mysqli_fetch_array($result);
	...
	if ($soption == 430) {
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			echo "<tr style=\"background-color:silver\">";
				...
				echo "<th>".$Total."</th>";
			echo "</tr>";
			$sqls= "select id,usuario from sim_users where activo=1 and validado=1 and sgm=1 order by usuario";
			$results = mysqli_query($dbhandle,convertSQL($sqls));
			while ($rows = mysqli_fetch_array($results)) {
				$sqlf= "select sum(total) as totales from sim_factura where visible=1 and tipo=5 and id_pagador=".$rows["id"];
				$resultf = mysqli_query($dbhandle,convertSQL($sqlf));
				$rowf = mysqli_fetch_array($resultf);
				echo "<tr>";
			...
			echo "<tr><th style=\"text-align:center;\" colspan=\"17\">".$Tipo." : <select name=\"id_tipo\">";
				...
				while ($rowtipos = mysqli_fetch_array($resulttipos)) {
					if ($_POST["id_tipo"] == $rowtipos["id"]) {
						echo "<option value=\"".$rowtipos["id"]."\" selected>".$rowtipos["id_tipo"]."</option>";
					} else {
						echo "<option value=\"".$rowtipos["id"]."\">".$rowtipos["id_tipo"]."</option>";
					}
				}
				echo "</select>";
		...
		for ($i = 2012; $i <= (date("Y")+1); $i++) {
			...
			echo "</tr></tr>";
				...
				for ($x = 1; $x <= 12; $x++) {
					...
					$sql = "select sum(subtotaldescuento) as total1, sum(total) as total3,count(*) as total4 from sim_factura where visible=1 and tipo=".$_POST["id_tipo"]." and ((fecha>= '".$y."-".$m."-01') and (fecha<'".$yf."-".$mf."-01'))";
					$result = mysqli_query($dbhandle,convertSQL($sql));
					$row = mysqli_fetch_array($result);
	...
	if ($soption == 490) {
		...
		$v_docs = 0;
		$v_recibo = 0;
		if (($_GET["id_tipo"] == 1) or ($_GET["id_tipo"] == 4) or ($_GET["iduser"] > 0)) {$sqltipo = "select * from sim_factura_tipos where contabilidad=-1";}
		if (($_GET["id_tipo"] == 3) or ($_GET["id_tipo"] == 5) or ($_GET["id_tipo"] == 9)) {$sqltipo = "select * from sim_factura_tipos where contabilidad=1";}
		if (($_GET["id_tipo"] == 6) or ($_GET["id_tipo"] == 7) or ($_GET["id_tipo"] == 8)) {$sqltipo = "select * from sim_factura_tipos where id =".$_GET["id_tipo"];}
#		echo $sqltipo;
		...
		echo "<table cellpadding=\"2\" cellspacing=\"0\" class=\"lista\">";
			...
			$hoy = date("Y-m-d");
			$sqlca = "select * from sim_factura where visible=1 ";
			if ($_GET["id_tipo"] == 1) {$sqlca .= " AND tipo IN (select id from sim_factura_tipos where contabilidad=-1) and id_pagador=1";}
			if ($_GET["id_tipo"] == 3) {$sqlca .= " AND tipo IN (select id from sim_factura_tipos where contabilidad=1)";}
			if ($_GET["id_tipo"] == 4) {$sqlca .= " AND tipo IN (select id from sim_factura_tipos where contabilidad=-1) and id_pagador<>1";}
			if ($_GET["id_tipo"] == 5) {$sqlca .= " and tipo IN (select id from sim_factura_tipos where contabilidad=1) and id in (select id_factura from sim_factura_recibos where fecha='".date("Y-m-d", $_GET["fecha"])."' and visible=1)";}
			if ($_GET["iduser"] != "") {$sqlca .= " and id_pagador=".$_GET["iduser"];}
			if (($_GET["id_tipo"] != 4) and ($_GET["id_tipo"] != 5) and ($_GET["iduser"] <= 0)){$sqlca = $sqlca." AND fecha_vencimiento='".date("Y-m-d", $_GET["fecha"])."'";}
			if ($_GET["id_tipo"] == 6) {$sqlca = "select * from sim_factura where visible=1 and tipo=".$_GET["id_tipo"]." and fecha between '".$_GET["y"]."-".$_GET["m"]."-01' and '".$_GET["y"]."-".$_GET["m"]."-31'";}
			if ($_GET["id_tipo"] == 7) {$sqlca = "select * from sim_factura where visible=1 and tipo=".$_GET["id_tipo"]." and fecha between '".$_GET["y"]."-".($_GET["m"]-2)."-01' and '".$_GET["y"]."-".$_GET["m"]."-31'";}
			if ($_GET["id_tipo"] == 8) {$sqlca = "select * from sim_factura where visible=1 and tipo=".$_GET["id_tipo"]." and fecha='".$_GET["y"]."-".$_GET["m"]."-".$_GET["d"]."'";}
			if ($_GET["id_tipo"] == 9) {$sqlca = "select * from sim_factura where visible=1 and ((tipo IN (select id from sim_factura_tipos where contabilidad=1) AND fecha_vencimiento='".date("Y-m-d", $_GET["fecha"])."') or (id in (select id_factura from sim_factura_recibos where fecha='".date("Y-m-d", $_GET["fecha"])."' and visible=1)))";}
...
#			echo $sqlca;
	...
	if (($soption == 510) and ($admin == true)) {
		if ($ssoption == 1) {
			$sql = "select count(*) as total from sim_factura_dades_origen";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			$row = mysqli_fetch_array($result);
			if ($row["total"] == 0){
				...
				insertFunction ("sim_factura_dades_origen",$camposinsert,$datosInsert);
			}
			if ($row["total"] == 1) {
				...
				updateFunction ("sim_factura_dades_origen","1",$camposUpdate,$datosUpdate);
			}
		}
		if ($ssoption == 2) {
			$camposinsert = "id_factura_dades_origen,entidad_bancaria,iban,descripcion";
			$datosInsert = array($_GET["id"],$_POST["entidad_bancaria"],$_POST["iban"],$_POST["descripcion"]);
			insertFunction ("sim_factura_dades_origen_iban",$camposinsert,$datosInsert);
		}
		if ($ssoption == 3) {
			...
			updateFunction ("sim_factura_dades_origen_iban",$_GET["id"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 4) {
			$sql = "delete from sim_factura_dades_origen_iban WHERE id=".$_GET["id"];
			mysqli_query($dbhandle,convertSQL($sql));
		}
		if ($ssoption == 5) {
			$sql = "update sim_factura_dades_origen_iban set ";
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			echo "<tr>";
				echo "<td style=\"vertical-align:top;width:35%\">";
					$sql = "select * from sim_factura_dades_origen";
					$result = mysqli_query($dbhandle,convertSQL($sql));
					$row = mysqli_fetch_array($result);
					echo "<table style=\"width:100%\">";
				...
				echo "<td style=\"vertical-align:top;width:65%\">";
					echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
						...
						echo "</tr>";
						echo "<tr><td>&nbsp;</td></tr>";
						$sql2 = "select id,predefinido,entidad_bancaria,iban,descripcion from sim_factura_dades_origen_iban where id_factura_dades_origen=".$row["id"];
						$result2 = mysqli_query($dbhandle,convertSQL($sql2));
	...
	if (($soption == 520) and ($admin == true)) {
		if ($ssoption == 1) {
			...
			$sql = "select limite_kb from sim_files_tipos where id=".$tipo;
			$result = mysqli_query($dbhandle,convertSQL($sql));
			$row = mysqli_fetch_array($result);
			...
			if (($archivo != "none") AND ($archivo_size != 0) AND ($archivo_size<=$lim_tamano)){
				...
				if (copy ($archivo, "../archivos_comunes/images/".$archivo_name)) {
					if ($_GET["logo"] == 1){
						$sql = "update sim_factura_dades_origen set ";
		...
		echo "<h4>".$Logos."</h4>";
		echo boton(array("op=1003&sop=500"),array("&laquo; ".$Volver));
		$sqlele = "select logo1,logo_ticket from sim_factura_dades_origen";
		$resultele = mysqli_query($dbhandle,convertSQL($sqlele));
	...
	if (($soption == 530) AND ($admin == true)) {
		if ($ssoption == 1) {
			$camposInsert = "tipo,orden,descripcion,dias,facturable,tpv,caja,v_fecha_prevision,v_fecha_prevision_dias,v_fecha_vencimiento,v_numero_cliente,v_subtipos,v_recibos,tipo_ot,presu,presu_dias,stock,aprovado,v_rfq,contabilidad";
			$datosInsert = array($_POST["id_tipo"],$_POST["orden"],$_POST["descripcion"],$_POST["dias"],$_POST["facturable"],$_POST["tpv"],$_POST["caja"],$_POST["v_fecha_prevision"],$_POST["v_fecha_prevision_dias"],$_POST["v_fecha_vencimiento"],$_POST["v_numero_cliente"],$_POST["v_subtipos"],$_POST["v_recibos"],$_POST["tipo_ot"],$_POST["presu"],$_POST["presu_dias"],$_POST["stock"],$_POST["aprovado"],$_POST["v_rfq"],$_POST["contabilidad"]);
			insertFunction ("sim_factura_tipos",$camposInsert,$datosInsert);
		}
		if ($ssoption == 2) {
			$camposUpdate = array("tipo","orden","descripcion","dias","facturable","tpv","caja","v_fecha_prevision","v_fecha_prevision_dias","v_fecha_vencimiento","v_numero_cliente","v_subtipos","v_recibos","tipo_ot","presu","presu_dias","stock","aprovado","v_rfq","contabilidad");
			$datosUpdate = array($_POST["id_tipo"],$_POST["orden"],$_POST["descripcion"],$_POST["dias"],$_POST["facturable"],$_POST["tpv"],$_POST["caja"],$_POST["v_fecha_prevision"],$_POST["v_fecha_prevision_dias"],$_POST["v_fecha_vencimiento"],$_POST["v_numero_cliente"],$_POST["v_subtipos"],$_POST["v_recibos"],$_POST["tipo_ot"],$_POST["presu"],$_POST["presu_dias"],$_POST["stock"],$_POST["aprovado"],$_POST["v_rfq"],$_POST["contabilidad"]);
			updateFunction ("sim_factura_tipos",$_GET["id"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 3) {
			$camposUpdate = array("visible");
			$datosUpdate = array("0");
...
#				echo "<td><input type=\"Text\" name=\"descripcion\" style=\"width:100%\"></td>";
			...
			echo "</form>";
			echo "<tr><td>&nbsp;</td></tr>";
			$sqltipos = "select * from sim_factura_tipos where visible=1 order by orden,descripcion";
			$resulttipos = mysqli_query($dbhandle,convertSQL($sqltipos));
			while ($rowtipos = mysqli_fetch_array($resulttipos)) {
				...
				echo "<tr>";
					...
					echo "<td><input type=\"Text\" name=\"tipo\" value=\"".$rowtipos["id_tipo"]."\" style=\"width:100%\"></td>";
#					echo "<td><input type=\"Text\" name=\"descripcion\" value=\"".$rowtipos["descripcion"]."\" style=\"width:100%\"></td>";
	...
	if (($soption == 535) and ($admin == true)) {
		if ($ssoption == 1){
			$camposInsert = "id_tipo,id_subtipo";
			$datosInsert = array($_GET["id"],$_POST["id_subtipo"]);
			insertFunction ("sim_factura_subtipos",$camposInsert,$datosInsert);
		}
		if ($ssoption == 2){
			$camposUpdate = array("id_subtipo");
			$datosUpdate = array($_POST["id_subtipo"]);
			updateFunction ("sim_factura_subtipos",$_GET["id_sub"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 3){
			$camposUpdate = array("visible");
			$datosUpdate = array("0");
		...
		if ($ssoption == 4) {
			$sql = "select id from sim_idiomas where visible=1";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)) {
				$sqli = "select id from sim_factura_tipos_idiomas where id_tipo=".$_GET["id"]." and id_idioma=".$row["id"];
				$resulti = mysqli_query($dbhandle,convertSQL($sqli));
				$rowi = mysqli_fetch_array($resulti);
				if ($rowi){
					$camposUpdate = array("tipo");
					$datosUpdate = array($_POST["tipo".$row["id"]]);
					updateFunction ("sim_factura_tipos_idiomas",$rowi["id"],$camposUpdate,$datosUpdate);
				} else {
					$camposInsert = "id_tipo,id_idioma,tipo";
					$datosInsert = array($_GET["id"],$row["id"],$_POST["tipo".$row["id"]]);
					insertFunction ("sim_factura_tipos_idiomas",$camposInsert,$datosInsert);
				}
			}
		}
		$sqltipos = "select tipo from sim_factura_tipos where visible=1 and id=".$_GET["id"];
		$resulttipos = mysqli_query($dbhandle,convertSQL($sqltipos));
		$rowtipos = mysqli_fetch_array($resulttipos);
		echo "<h4>".$rowtipos["id_tipo"]."</h4>";
			...
			echo "<tr>";
				echo "<td style=\"width:50%;vertical-align:top;\">";
					echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
						...
						$sql = "select id,descripcion from sim_idiomas where visible=1";
						$result = mysqli_query($dbhandle,convertSQL($sql));
						while ($row = mysqli_fetch_array($result)) {
							$sqli = "select tipo from sim_factura_tipos_idiomas where id_tipo=".$_GET["id"]." and id_idioma=".$row["id"];
							$resulti = mysqli_query($dbhandle,convertSQL($sqli));
							$rowi = mysqli_fetch_array($resulti);
							echo "<tr>";
								echo "<td>".$row["descripcion"]."</td>";
								echo "<td><input type=\"Text\" name=\"tipo".$row["id"]."\" value=\"".$rowi["id_tipo"]."\"></td>";
							echo "</tr>";
						}
				...
				echo "</td><td style=\"width:50%;vertical-align:top;\">";
					echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
						...
						echo "<tr>";
							echo "<td></td>";
							echo "<td><input type=\"text\" name=\"id_subtipo\"></td>";
							echo "<td class=\"submit\"><input type=\"submit\" value=\"".$Anadir."\"></td>";
						echo "</tr>";
						echo "</form>";
						$sqlstipos = "select id,id_subtipo from sim_factura_subtipos where visible=1 and id_tipo=".$_GET["id"]." order by id_subtipo";
						...
						while ($rowstipos = mysqli_fetch_array($resultstipos)) {
							...
							echo "<tr>";
								...
								echo "<td><input type=\"text\" value=\"".$rowstipos["id_subtipo"]."\" name=\"id_subtipo\"></td>";
								echo "<td class=\"submit\"><input type=\"submit\" value=\"".$Modificar."\"></td>";
							echo "</tr>";
							echo "</form>";
						}
	...
	if (($soption == 540) AND ($admin == true)) {
		if ($ssoption == 1) {
			$camposInsert = "id_usuario,id_tipo,admin";
			$datosInsert = array($_POST["id_usuario"].$_GET["id_usuario"],$_GET["id_tipo"].$_POST["id_tipo"],$_POST["admin"]);
			insertFunction ("sim_factura_tipos_permisos",$camposInsert,$datosInsert);
		}
		if ($ssoption == 2) {
			$camposUpdate = array("admin");
			$datosUpdate = array($_POST["admin"]);
			updateFunction ("sim_factura_tipos_permisos",$_GET["id"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 3) {
			deleteFunction ("sim_factura_tipos_permisos",$_GET["id"]);
		}
		...
		if (($soption == 540) and ($_GET["tip"] == 1)) {
			echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
				echo "<tr style=\"background-color : Silver;\">";
					...
					echo "<th>".$Admin."</th>";
				echo "</tr>";
				$sql = "select id,tipo from sim_factura_tipos where visible=1 order by tipo";
				$result = mysqli_query($dbhandle,convertSQL($sql));
				while ($row = mysqli_fetch_array($result)) {
					echo "<tr>";
					echo "<form action=\"index.php?op=1003&sop=540&tip=1&ssop=1&id_tipo=".$row["id"]."\" method=\"post\">";
						echo "<td style=\"width:150px;text-align:right;\">".$row["id_tipo"]."</td>";
						echo "<td></td>";
						echo "<td><select name=\"id_usuario\" style=\"width:150px\">";
							echo "<option value=\"0\">-</option>";
							$sqlx = "select id,usuario from sim_users where sgm=1 and activo=1 and validado=1 order by usuario";
							$resultx = mysqli_query($dbhandle,convertSQL($sqlx));
							while ($rowx = mysqli_fetch_array($resultx)) {
					...
					echo "</form>";
					echo "</tr>";
					$sql2 = "select id,id_usuario,admin from sim_factura_tipos_permisos where id_tipo=".$row["id"];
					$result2 = mysqli_query($dbhandle,convertSQL($sql2));
					while ($row2 = mysqli_fetch_array($result2)) {
						$sqlv = "select usuario from sim_users where id=".$row2["id_usuario"];
		...
		if (($soption == 540) and ($_GET["tip"] == 2)) {
			echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
				...
				while ($rowx = mysqli_fetch_array($resultx)) {
					echo "<tr>";
					echo "<form action=\"index.php?op=1003&sop=540&tip=2&ssop=1&id_usuario=".$rowx["id"]."\" method=\"post\">";
						...
						echo "<td><select name=\"id_tipo\" style=\"width:150px\">";
							...
							$result = mysqli_query($dbhandle,convertSQL($sql));
							while ($row = mysqli_fetch_array($result)) {
								echo "<option value=\"".$row["id"]."\">".$row["id_tipo"]."</option>";
							}
						echo "</select></td>";
						echo "<td><select name=\"admin\" style=\"width:50px\">";
						echo "<option value=\"0\" selected>".$No."</option>";
					...
					echo "</form>";
					echo "</tr>";
					$sql2 = "select id,id_tipo,admin from sim_factura_tipos_permisos where id_usuario=".$rowx["id"];
					$result2 = mysqli_query($dbhandle,convertSQL($sql2));
					while ($row2 = mysqli_fetch_array($result2)) {
						$sqlv = "select tipo from sim_factura_tipos where visible=1 and id=".$row2["id_tipo"];
						$resultv = mysqli_query($dbhandle,convertSQL($sqlv));
						$rowv = mysqli_fetch_array($resultv);
						echo "<tr>";
	...
	if (($soption == 550) AND ($admin == true)) {
		if ($ssoption == 1) {
			if ($_POST["id_tipo_d"] != 0) {
				...
				insertFunction ("sim_factura_tipos_relaciones",$camposInsert,$datosInsert);
			}
		}
		if ($ssoption == 2) {
			$camposUpdate = array("cerrar");
			$datosUpdate = array($_POST["cerrar"]);
			updateFunction ("sim_factura_tipos_relaciones",$_GET["id"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 3) {
			deleteFunction ("sim_factura_tipos_relaciones",$_GET["id"]);
		}
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			echo "<tr style=\"background-color:silver;\">";
				...
				echo "<th></th>";
			echo "</tr>";
			$sqltipos = "select id,tipo from sim_factura_tipos where visible=1 order by tipo";
			$resulttipos = mysqli_query($dbhandle,convertSQL($sqltipos));
			while ($rowtipos = mysqli_fetch_array($resulttipos)) {
			echo "<form method=\"post\" action=\"index.php?op=1003&sop=550&ssop=1&id_tipo_o=".$rowtipos["id"]."\">";
				echo "<tr>";
					...
					echo "<td><select name=\"id_tipo_d\">";
						echo "<option value=\"0\">-</option>";
						$sqltipos1 = "select id,tipo from sim_factura_tipos where visible=1 order by tipo";
						$resulttipos1 = mysqli_query($dbhandle,convertSQL($sqltipos1));
						while ($rowtipos1 = mysqli_fetch_array($resulttipos1)) {
							echo "<option value=\"".$rowtipos1["id"]."\">".$rowtipos1["id_tipo"]."</option>";
						}
					echo "</select></td>";
				...
				echo "<tr>";
				echo "</form>";
				$sqltiposrel = "select id,id_tipo_d,cerrar from sim_factura_tipos_relaciones where id_tipo_o=".$rowtipos["id"];
				...
				while ($rowtiposrel = mysqli_fetch_array($resulttiposrel)) {
					...
					$sqltipos2 = "select tipo from sim_factura_tipos where id=".$rowtiposrel["id_tipo_d"];
					$resulttipos2 = mysqli_query($dbhandle,convertSQL($sqltipos2));
	...
	if (($soption == 560) AND ($admin == true)) {
		if ($ssoption == 1) {
			...
			insertFunction ("sim_divisas",$camposInsert,$datosInsert);
		}
		if ($ssoption == 2) {
			$camposUpdate = array("abrev","divisa","simbolo");
			$datosUpdate = array($_POST["abrev"],$_POST["divisa"],$_POST["simbolo"]);
			updateFunction ("sim_divisas",$_GET["id"],$camposUpdate,$datosUpdate);
		}
		if ($ssoption == 3) {
			$camposUpdate = array("visible");
			$datosUpdate = array("0");
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			...
			echo "</form>";
			echo "</tr>";
			$sql = "select * from sim_divisas where visible=1";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)) {
				$color = "white";
	...
	if (($soption == 565) AND ($admin == true)) {
		if ($ssoption == 1) {
			$sql = "select id from sim_divisas where visible=1 and id<>".$_GET["id"];
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)) {
				$sqli = "select id,cambio from sim_divisas_cambios where id_divisa_origen=".$_GET["id"]." and id_divisa_destino=".$row["id"];
				$resulti = mysqli_query($dbhandle,convertSQL($sqli));
				$rowi = mysqli_fetch_array($resulti);
				if ($rowi){
					if ($rowi["cambio"] != $_POST["cambio"]){
						$camposUpdate = array("cambio");
						$datosUpdate = array($_POST["cambio".$row["id"]]);
						updateFunction ("sim_divisas_cambios",$rowi["id"],$camposUpdate,$datosUpdate);
						$fecha=date("Y-m-d");
						$camposInsert = "id_divisa_origen,id_divisa_destino,id_usuario,fecha,cambio";
						$datosInsert = array($_GET["id"],$row["id"],$userid,$fecha,$_POST["cambio".$row["id"]]);
						insertFunction ("sim_divisas_mod_cambio",$camposInsert,$datosInsert);
					}
				} else {
					$camposInsert = "id_divisa_origen,id_divisa_destino,cambio";
					$datosInsert = array($_GET["id"],$row["id"],$_POST["cambio".$row["id"]]);
					insertFunction ("sim_divisas_cambios",$camposInsert,$datosInsert);
				}
			}
		}
		echo "<h4>".$Cambios." ".$Divisas."</h4>";
		echo boton(array("op=1003&sop=560"),array("&laquo; ".$Volver));
		$sql = "select id,divisa from sim_divisas where id=".$_GET["id"];
		$result = mysqli_query($dbhandle,convertSQL($sql));
		$row = mysqli_fetch_array($result);
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			echo "<tr style=\"background-color: Silver;\">";
			...
			echo "<tr>";
				...
				$sqld = "select id,divisa,abrev from sim_divisas where visible=1 and id<>".$row["id"];
				$resultd = mysqli_query($dbhandle,convertSQL($sqld));
				while ($rowd = mysqli_fetch_array($resultd)) {
					$sqldc = "select cambio from sim_divisas_cambios where id_divisa_origen=".$_GET["id"]." and id_divisa_destino=".$rowd["id"];
					...
					$rowdc = mysqli_fetch_array($resultdc);
						...
						echo "<td style=\"text-align:left;width:150px\"><input type=\"Text\" style=\"width:80px\" name=\"cambio".$rowd["id"]."\" value=\"".$rowdc["cambio"]."\"> ".$rowd["abrev"]."</td>";
					echo "</tr><tr><td></td>";
				}
			echo "</tr>";
			echo "<tr>";
	...
	if (($soption == 566) AND ($admin == true)) {
		...
		echo "<table cellpadding=\"1\" cellspacing=\"0\" class=\"lista\">";
			echo "<tr style=\"background-color: Silver;\">";
				...
				echo "<th>".$Usuario."</th>";
			echo "</tr>";
			$sql = "select * from sim_divisas_mod_cambio where id_divisa_origen=".$_GET["id"];
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)) {
				echo "<tr>";
					$sqlu = "select usuario from sim_users where id=".$row["id_usuario"];
					$resultu = mysqli_query($dbhandle,convertSQL($sqlu));
					$rowu = mysqli_fetch_array($resultu);
					$sqld = "select divisa from sim_divisas where id=".$row["id_divisa_destino"];
					$resultd = mysqli_query($dbhandle,convertSQL($sqld));
					$rowd = mysqli_fetch_array($resultd);
					echo "<td style=\"text-align:left;width:100px\">".$rowd["divisa"]."</td>";
					echo "<td style=\"text-align:left;width:100px\">".$row["fecha"]."</td>";
					echo "<td style=\"text-align:left;width:100px\">".$row["cambio"]."</td>";
...
if (($soption == 100000) AND ($admin == true)) {
	$sql = "select * from sim_files";
	$result = mysqli_query($dbhandle,convertSQL($sql));
	while ($row = mysqli_fetch_array($result)) {
		if ($row["id_cuerpo"] != 0){
				$camposUpdate = array("id_elemento");
				$datosUpdate = array($row["id_cuerpo"]);
				updateFunction ("sim_files",$row["id"],$camposUpdate,$datosUpdate);
		}
		if ($row["id_articulo"] != 0){
				$camposUpdate = array("id_elemento");
				$datosUpdate = array($row["id_articulo"]);
				updateFunction ("sim_files",$row["id"],$camposUpdate,$datosUpdate);
		}
		if ($row["id_client"] != 0){
				$camposUpdate = array("id_elemento");
