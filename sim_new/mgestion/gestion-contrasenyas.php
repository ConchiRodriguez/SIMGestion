...
if (($option == 1024) AND ($autorizado == true)) {
		...
		echo "<form action=\"index.php?op=1024&sop=0\" method=\"post\">";
			echo "<tr>";
				echo "<td><select style=\"width:700px\" name=\"id_contrato\">";
					...
					$sqlc = "select * from sim_clients where visible=1 order by nombre";
					$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
					while ($rowc = mysqli_fetch_array($resultc)) {
						$sql = "select * from sim_contratos where visible=1 and activo=1 and id_cliente_final=".$rowc["id"]."";
						$result = mysqli_query($dbhandle,convertSQL($sql));
						while ($row = mysqli_fetch_array($result)){
							if ($row){
								if($row["id"] == $id_contrato){
									echo "<option value=\"".$row["id"]."\" selected>".$rowc["nombre"]." ".$rowc["cognom1"]." ".$rowc["apellido2"]." - ".$row["descripcion"]."</option>";
								} else {
									echo "<option value=\"".$row["id"]."\">".$rowc["nombre"]." ".$rowc["cognom1"]." ".$rowc["apellido2"]." - ".$row["descripcion"]."</option>";
								}
							}
						}
					}
				...
				echo "<td><select style=\"width:200px\" name=\"id_aplicacion\">";
					echo "<option value=\"0\">-</option>";
					$sql = "select * from sim_contrasenas_apliciones where visible=1 order by aplicacion";
					$result = mysqli_query($dbhandle,convertSQL($sql));
					while ($row = mysqli_fetch_array($result)) {
	...
	if ($soption == 1) {
		$sqlc = "select * from sim_contrasenas where id=".$_GET["id_con"];
		$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
	...
	if (($soption == 510) AND ($admin == true)) {
		if (($ssoption == 1) AND ($admin == true)){
			...
			insertFunction ("sim_contrasenas_apliciones","aplicacion,descripcion",$datosInsert);
		}
		if (($ssoption == 2) AND ($admin == true)){
			...
			updateFunction ("sim_contrasenas_apliciones",$_GET["id"],$camposUpdate,$datosUpdate);
		}
		if (($ssoption == 3) AND ($admin == true)){
			$sqlc = "select count(*) as total from sim_contrasenas where visible=1 and id_aplicacion=".$_GET["id"];
			$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
			$rowc = mysqli_fetch_array($resultc);
			if ($rowc["total"] > 0){
...
#	if ($soption == 1234) {
#		$sql = "select * from sim_contrasenas";
#		$result = mysqli_query($dbhandle,convertSQL($sql));
#		while ($row = mysqli_fetch_array($result)) {
#			$cadena = encrypt($row["contrasena"], $simclau);
#			$sql = "update sim_contrasenas set ";
#			$sql = $sql."contrasena='".$cadena."'";
