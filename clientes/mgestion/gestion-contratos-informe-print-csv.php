...
else {list($mes_fins,$any_fins) = explode ("-",$_POST["fecha2"]); if ($mes_fins < 12) {$mes_fins++;} else { $mes_fins = 1; $any_fins ++; }}
...
if ($_POST["id_cliente2"] > 0){ $sql .= " and id_cliente=".$_POST["id_cliente2"]."";}
...
while ($row = mysqli_fetch_array($result)){
	...
	$rowc = mysqli_fetch_array($resultc);
 	...
 	$sqli = "select * from sgm_incidencias where fecha_inicio between ".$ini_con." and ".$fin_con." and id_servicio in (select id from sgm_contratos_servicio where id_contrato=".$row["id"].") and visible=1 order by fecha_inicio";
