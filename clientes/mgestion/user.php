...
$simclau = "Solucions00";
	...
	$sqluserclient = "select admin,id_client from sgm_users_clients WHERE id_user=".$userid;
	$resultuserclient = mysqli_query($dbhandle,convertSQL($sqluserclient));
	while ($rowuserclient = mysqli_fetch_array($resultuserclient)){
		$id_cliente .= $rowuserclient["id_client"].",";
		$admin = $rowuserclient["admin"];
	}
	$id_cliente = rtrim($id_cliente, ",");

	$sqlt = "select id from sgm_users_permisos where id_user=".$userid." and id_modulo='1002'";
	$resultt = mysqli_query($dbhandle,convertSQL($sqlt));
	$rowt = mysqli_fetch_array($resultt);
	if ($rowt) { $gusuarios = 1;}
	$sqlf = "select id from sgm_users_permisos where id_user=".$userid." and id_modulo='1003'";
	$resultf = mysqli_query($dbhandle,convertSQL($sqlf));
	$rowf = mysqli_fetch_array($resultf);
	if ($rowf) { $gfacturacion = 1;}

	$sqlc = "select id from sgm_users_permisos where id_user=".$userid." and id_modulo='1011'";
	$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
	$rowc = mysqli_fetch_array($resultc);
	if ($rowc) { $gcontratos = 1;}

	$sqli = "select id from sgm_users_permisos where id_user=".$userid." and id_modulo='1018'";
	$resulti = mysqli_query($dbhandle,convertSQL($sqli));
	$rowi = mysqli_fetch_array($resulti);
	if ($rowi) { $gincidencias = 1;}

	$sqla = "select id from sgm_users_permisos where id_user=".$userid." and id_modulo='1024'";
	$resulta = mysqli_query($dbhandle,convertSQL($sqla));
	$rowa = mysqli_fetch_array($resulta);
	if ($rowa) { $gaplicaciones = 1;}

	$sqles = "select id from sgm_users_permisos where id_user=".$userid." and id_modulo='2001'";
	$resultes = mysqli_query($dbhandle,convertSQL($sqles));
	$rowes = mysqli_fetch_array($resultes);
	if ($rowes) { $gservidor = 1;}

	$sqlr = "select id from sgm_users_permisos where id_user=".$userid." and id_modulo='2002'";
	$resultr = mysqli_query($dbhandle,convertSQL($sqlr));
	$rowr = mysqli_fetch_array($resultr);
	if ($rowr) { $greports = 1;}

	$sqld = "select id from sgm_users_permisos where id_user=".$userid." and id_modulo='2003'";
	$resultd = mysqli_query($dbhandle,convertSQL($sqld));
	$rowd = mysqli_fetch_array($resultd);
	if ($rowd) { $gdocumentos = 1;}

	$sqlct = "select id from sgm_users_permisos where id_user=".$userid." and id_modulo='1024'";
...
#			echo "</tr>";
	...
	if ((($soption == 11) and ($gcontratos == 1)) or (($soption == 81) and ($gcontratos == 1))) {
		...
		echo "<tr><td></td><td></td><td>";
			...
			echo "</table>";
		echo "</td></tr></table>";
		$sqlccc = "select id_clasificacio_tipus from sgm_clients_classificacio where visible=1 and predeterminado=1 and id_client=".$rowc["id_cliente"]."";
	...
	if ((($soption == 12) and ($gcontratos == 1)) or (($soption == 82) and ($gcontratos == 1))) {
		...
		$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
		$rowc = mysqli_fetch_array($resultc);
		$sqla = "select id from sgm_clients where visible=1 and id=".$rowc["id_cliente"];
		$resulta = mysqli_query($dbhandle,convertSQL($sqla));
		$rowa = mysqli_fetch_array($resulta);
	...
	if ((($soption == 13) and ($gcontratos == 1)) or (($soption == 83) and ($gcontratos == 1))) {
		...
		echo "<center><table cellpadding=\"0\" cellspacing=\"0\">";
			...
			$sqlind = "select id,id_servicio,asunto from sgm_incidencias where fecha_inicio between ".$inicio_mes." and ".$final_mes." and visible=1";
			...
			while ($rowind = mysqli_fetch_array($resultind)){
				...
				$sqls = "select precio_hora from sgm_contratos_servicio where visible=1";
	...
	if (($soption == 20) and ($gusuarios == 1)) {
		if ($ssoption == 1) {
			...
			if ($registro == 1 ) {
				...
				$sqlt = "select id from sgm_users order by id desc";
				$resultt = mysqli_query($dbhandle,convertSQL($sqlt));
				$rowt = mysqli_fetch_array($resultt);
				$sqlcl = "select id from sgm_clients where id in (".$id_cliente.") and id_agrupacio=0";
	...
	if (($soption == 23) and ($gusuarios == 1)) {
		...
		while ($rowc = mysqli_fetch_array($resultc)) {
			...
			$result = mysqli_query($dbhandle,convertSQL($sql));
			while ($row = mysqli_fetch_array($result)) {
				$sqlt = "select id from sgm_users_permisos where id_user=".$_GET["id_user"]." and id_modulo=".$row["id_modulo"];
				$resultt = mysqli_query($dbhandle,convertSQL($sqlt));
				$rowt = mysqli_fetch_array($resultt);
				echo "<tr>";
	...
	if (($soption == 30) and ($gfacturacion == 1)) {
		...
		echo "<table cellpadding=\"0\" cellspacing=\"0\" style=\"width:600px\">";
			$sqltipos = "select id,dias from sgm_factura_tipos where id=1 order by id";
	...
	if (($soption == 40) and ($gincidencias == 1)) {
		if ($ssoption == 1) {
			$sql = "select id from sgm_incidencias where visible=1 and id_usuario=".$userid." and id_servicio=".$_POST["id_servicio"]." and data_registro2=".$_POST["data_registro"]."";
			$result = mysqli_query($dbhandle,convertSQL($sql));
			$row = mysqli_fetch_array($result);
			if (!($row)){
				$sqls = "select id_contrato from sgm_contratos_servicio where visible=1 and incidencias=1 and id=".$_POST["id_servicio"];
				$results = mysqli_query($dbhandle,convertSQL($sqls));
				$rows = mysqli_fetch_array($results);
				$sqlc = "select id_cliente from sgm_contratos where visible=1 and id=".$rows["id_contrato"]." and activo=1";
				$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
				$rowc = mysqli_fetch_array($resultc);
...
#			echo $sql;
			...
			while ($row = mysqli_fetch_array($result)) {
				...
				$sqlu = "select usuario,usuario_origen from sgm_users where id=".$row["id_usuario_origen"];
				$resultu = mysqli_query($dbhandle,convertSQL($sqlu));
				$rowu = mysqli_fetch_array($resultu);
				$sqlu2 = "select usuario from sgm_users where id=".$row["id_usuario_destino"];
				...
				echo "<tr style=\"background-color:".$estado_color.";\">";
					...
					if ($row["id_servicio"] == "-1") {
						...
						echo $rows["descripcion"]." - ".$rowc["servicio"];
					}
					echo "</td>";
			...
			echo "<tr>";
				echo "<td style=\"text-align:right;vertical-align:top;width:400px\">";
					echo "<table cellspacing=\"0\" cellspacing=\"0\">";
						...
						$sqle = "select entrada,id from sgm_incidencias_entrada where id=4";
						$resulte = mysqli_query($dbhandle,convertSQL($sqle));
						...
						$sqlu = "select usuario,id from sgm_users where validado=1 and activo=1 and id=".$userid;
						$resultu = mysqli_query($dbhandle,convertSQL($sqlu));
						$rowu = mysqli_fetch_array($resultu);
				...
				echo "</td><td style=\"text-align:right;vertical-align:top;\">";
					echo "<table cellspacing=\"0\" cellspacing=\"0\">";
						...
						echo "<tr><td><select name=\"id_servicio\" style=\"width:600px\">";
							$sqlc = "select id,id_cliente_final,descripcion from sgm_contratos where visible=1 and id_cliente in (".$id_cliente.") and activo=1 order by num_contrato";
							$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
							while ($rowc = mysqli_fetch_array($resultc)) {
								$sqlcli = "select nombre,cognom1,cognom2 from sgm_clients where visible=1 and id=".$rowc["id_cliente_final"]." order by nombre";
								$resultcli = mysqli_query($dbhandle,convertSQL($sqlcli));
								$rowcli = mysqli_fetch_array($resultcli);
								$sqls = "select id,servicio from sgm_contratos_servicio where visible=1 and incidencias=1 and id_contrato=".$rowc["id"];
								$results = mysqli_query($dbhandle,convertSQL($sqls));
								while ($rows = mysqli_fetch_array($results)){
									if ($row["id_servicio"] == $rows["id"]){
										echo "<option value=\"".$rows["id"]."\" selected>".$rowcli["nombre"]." ".$row["cognom1"]." ".$row["cognom2"]." - ".$rowc["descripcion"]." - ".$rows["servicio"]."</option>";
									} else {
										echo "<option value=\"".$rows["id"]."\">".$rowcli["nombre"]." ".$row["cognom1"]." ".$row["cognom2"]." - ".$rowc["descripcion"]." - ".$rows["servicio"]."</option>";
									}
								}
							}
						echo "</select></td></tr>";
...
#		if ($row["id_estado"] == -2){$dis = "disabled";} else {$dis = "";}
		...
		echo "<tr>";
			echo "<td style=\"text-align:right;vertical-align:top;width:300px\">";
				echo "<table cellspacing=\"0\" cellspacing=\"0\">";
					...
					echo "<tr>";
						...
						$sqle = "select entrada from sgm_incidencias_entrada where id=4";
						$resulte = mysqli_query($dbhandle,convertSQL($sqle));
						$rowe = mysqli_fetch_array($resulte);
					...
					echo "<tr>";
						...
						$sqlu = "select usuario from sgm_users where validado=1 and activo=1 and id=".$row["id_usuario_origen"];
						$resultu = mysqli_query($dbhandle,convertSQL($sqlu));
						$rowu = mysqli_fetch_array($resultu);
			...
			echo "</td><td style=\"text-align:right;vertical-align:top;\">";
				echo "<table cellspacing=\"0\" cellspacing=\"0\" style=\"width:850px;\">";
					$sqls = "select servicio,id_contrato from sgm_contratos_servicio where id=".$row["id_servicio"];
					$results = mysqli_query($dbhandle,convertSQL($sqls));
					$rows = mysqli_fetch_array($results);
					$sqlcli = "select nombre,cognom1,cognom2 from sgm_clients where visible=1 and id in (".$id_cliente.")";
					$resultcli = mysqli_query($dbhandle,convertSQL($sqlcli));
					$rowcli = mysqli_fetch_array($resultcli);
					$sqlc = "select descripcion from sgm_contratos where id=".$rows["id_contrato"];
					$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
					$rowc = mysqli_fetch_array($resultc);
					echo "<tr><td style=\"text-align:right;vertical-align:top;width:150px;\">".$Cliente." ".$Final." :</td><td>".$rowcli["nombre"]." ".$row["cognom1"]." ".$row["cognom2"]."</td></tr>";
					...
					echo "<tr><td>&nbsp;</td></tr>";
					$sqlc = "select notas_desarrollo,id_usuario_registro,fecha_inicio from sgm_incidencias where id_incidencia=".$_GET["id"]." and visible_cliente=1 order by fecha_inicio";
					$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
					while ($rowc = mysqli_fetch_array($resultc)) {
						$fecha = date("Y-m-d H:i:s", $rowc["fecha_inicio"]);
						$sqlu = "select usuario,sgm from sgm_users where id=".$rowc["id_usuario_registro"];
						$resultu = mysqli_query($dbhandle,convertSQL($sqlu));
						$rowu = mysqli_fetch_array($resultu);
						if ($rowu["sgm"] == 0) {
							$colortr = "white";
			...
			#			if ($row["id_estado"] != -2){
							echo "<tr><td></td><td class=\"submit\"><input type=\"Submit\" value=\"".$Guardar."\"></td></tr>";
			#			}
						echo "</form>";
					} else {
	...
	if ($soption == 61) {
		...
		$sqlc = "select id_client,id_aplicacion from sgm_contrasenyes where id=".$_GET["id"];
		$resultc = mysqli_query($dbhandle,convertSQL($sqlc));
		$rowc = mysqli_fetch_array($resultc);
	...
	if (($soption == 92) and ($gservidor == 1)) {
		...
		echo "<table><tr>";
			echo "<td style=\"width:60px;height:20px;text-align:center;vertical-align:middle;background-color:grey;border:1px solid black\">";
				echo "<a href=\"index.php?op=200&sop=90&id=".$_GET["id"]."\" style=\"color:white;\">".$Volver."</a>";
			echo "</td>";
		echo "</tr></table>";
		echo "<br>";
		echo "<center>";
		...
		while ($rowu = mysqli_fetch_array($resultu)) {
			echo "<tr>";
				...
				if ($client != $rowu["id_client"]) {
					echo "<td>".$rowc["nombre"]." ".$row["cognom1"]." ".$row["cognom2"]."</td>";
