		...
		while ($row = mysql_fetch_array($result)) {
			...
			while ($row2 = mysql_fetch_array($result2)) {
				...
				echo "<td style=\"vertical-align : middle;border-top : 1px solid #AAAAAA;\">&nbsp;&nbsp;<a href=\"index.php?op=10&sop=1&id_foro=".$row2["id"]."\">";
					...
					$resultnuevo = mysql_query(convertSQL($sqlnuevo));
...
#				echo $sqlnuevo."xx<br>";
	...
	if ($soption == 1) {
		...
		if ( $autorizado == 1 ) {
			...
			$resultadmins = mysql_query(convertSQL($sqladmins));
			$coma = 0;
	...
	if ($soption == 2) {
		...
		echo "</td><td>";
				...
				echo "<table>";
					...
					$resultfirma = mysql_query(convertSQL($sqlfirma));
	...
	if ($soption == 4) {
		...
		if (($page == 1) OR ($page == 0)){ 
			...
			echo "<tr>";
				...
				$resultxxx = mysql_query(convertSQL($sqlxxx));
