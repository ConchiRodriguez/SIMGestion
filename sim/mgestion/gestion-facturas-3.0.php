...
if (($option == 1003) AND ($autorizado == true)) {
	...
	if ($soption == 5) {
		if ($ssoption == 1000) {
			...
			if ($rowt["total"] != 0) {
				echo mensageError("No se puede añadir archivo por que ya existe uno con el mismo nombre.");
			} else {
				if (($archivo != "none") AND ($archivo_size != 0) AND ($archivo_size<=$lim_tamano)){
					if (copy ($archivo, "files/facturacion/".$archivo_name)) {
						...
						echo mensageError("<h2>No ha podido transferirse el archivo.</h2><br><h3>Su tamaño no puede exceder de ".$lim_tamano." bytes.</h2>");
				}
			}
		}
		...
		if ($ssoption == 1) {
			...
			if ($rowcc["total"] == 0){
				...
				if ($rowtipos["v_fecha_vencimiento"] == 1) {
					$fecha_vencimiento = calculDataVencimientFact($_POST["cliente"],cambiarFormatoFechaYMD($_POST["fecha"]));
...
#				echo $sql;
		...
		if ($_GET["filtra"]  == 1){
			echo "<tr>";
				echo "<form action=\"index.php?op=1003&sop=10&filtra=1&id=".$_GET["id"]."\" method=\"post\">";
				...
				echo "<td><input type=\"Submit\" value=\"".$Desplegar." ".$Todo."\" style=\"width:110px\"></td>";
				echo "</form>";
			echo "</tr>";
			echo "<tr><td>&nbsp;</td></tr>";
		}
			...
			echo "<tr>";
				...
				echo "<td><input type=\"Text\" name=\"fecha\" style=\"width:100px\" value=\"".date("d-m-Y")."\"></td>";
			### CALCULO DE LA FECHA DE PREVISION DE ENTREGA
				$suma = $rowtipos["v_fecha_prevision_dias"];
				$fecha_prevision1 = date("d-m-Y")+$suma;
				$fecha_prevision2 = date("d-m-Y");
				if ($rowtipos["v_fecha_prevision"] == 1) { 
					echo "<td><input type=\"Text\" name=\"fecha_prevision\" style=\"width:70px\" value=\"".$fecha_prevision1."\"></td>";
				} else {
					echo "<input type=\"hidden\" name=\"fecha_prevision\" style=\"width:70px\" value=\"".$fecha_prevision2."\">";
				}
				if ($rowtipos["v_fecha_vencimiento"] == 1) { echo "<td></td>"; }
				if ($rowtipos["v_numero_cliente"] == 1) { 
					echo "<td><input type=\"Text\" name=\"numero_cliente\" style=\"width:70px\"></td>";
			...
			if (($_GET["filtra"] == 1) AND ($_GET["fecha"] != 0)) { $sql = $sql." AND fecha_vencimiento2='".$_GET["fecha"]."'"; }
			$sql = $sql." order by numero desc,version desc,fecha desc";
			$result = mysql_query(convertSQL($sql));
#			echo $sql;
			$fechahoy = getdate();
			$data1 = date("U", mktime(0,0,0,$fechahoy["mon"] ,$fechahoy["mday"]-100, $fechahoy["year"]));
			$hoy = time();
			while ($row = mysql_fetch_array($result)) {
		#### OPCIONES DE VISUALIZACION SI SE MUESTRA O NO
				$ver = 0;
				if ($data1 <= $row["fecha2"]) { $ver = 1; }
...
#				if (($data1 >= $row["fecha"]) AND ($_GET["id"] == 4) AND ($row["cerrada"] == 1)) { $ver = 0; }
				...
				if ($_GET["filtra"]  == 1){
					...
					if ($_POST["data_desde1"] != 0) { $sqlcc = $sqlcc." AND fecha_prevision2>='".strtotime($_POST["data_desde1"])."'"; }
					if ($_POST["data_fins1"] != 0) { $sqlcc = $sqlcc." AND fecha_prevision2<='".strtotime($_POST["data_fins1"])."'"; }
					if ($_POST["data_desde2"] != 0) { $sqlcc = $sqlcc." AND fecha_prevision2>='".strtotime($_POST["data_desde2"])."'"; }
					if ($_POST["data_fins2"] != 0) { $sqlcc = $sqlcc." AND fecha_prevision2<='".strtotime($_POST["data_fins2"])."'"; }
...
#						echo "</tr></table></td>";
					...
					############ CALCULOS SOBRE LAS FECHAS OFICIALES
							...
							echo "<td style=\"background-color : ".$color.";text-align : right;\">".number_format($row["total"],2)." €</td>";
...
#							echo "<td style=\"background-color : ".$color.";\"><table cellpadding=\"0\" cellspacing=\"0\"><tr>";
							if ($rowtipos["v_recibos"] == 1)  {
								...
								echo "</td>";
									...
									echo "<td style=\"background-color:".$color.";width:350px;text-align:right;\">".number_format(($row["total"]-$rowr["total"]),2)." €</td>";
							}
							echo "<td style=\"background-color:".$color.";width:50px;text-align:right;\">";
...
#						echo "<tr><td>&nbsp;</td></tr>";
						echo "<tr style=\"background-color:silver;\">";
							...
							echo "<td>".$Unidades."</td>";
							echo "<td><table><tr><td style=\"width:75px\">".$Codigo."</td>";
							echo "<td>".$Nombre."</td></td></tr></table>";
							echo "<td>".$Total."</td>";
							echo "<td>PVD</td>";
							echo "<td>PVP</td>";
							echo "<td>Desc.%</td>";
						...
						while ($rowxx = mysql_fetch_array($resultxx)) {
							...
							echo "<tr style=\"background-color : ".$color.";\">";
								echo "<td><input type=\"Text\" style=\"width:65px\" name=\"linea\" value=\"".$rowxx["linea"]."\"></td>";
								echo "<td><input type=\"Text\" style=\"width:75px\" name=\"fecha_prevision\" value=\"".$rowxx["fecha_prevision"]."\"></td>";
								echo "<td><input type=\"Text\" style=\"width:75px\" name=\"unidades\" style=\"text-align:right;\" value=\"".$rowxx["unidades"]."\"></td>";
								echo "<td><table><tr><td><input type=\"Text\" style=\"width:75px\" name=\"codigo\" value=\"".$rowxx["codigo"]."\"></td>";
								echo "<td><input type=\"Text\" style=\"width:180px\" name=\"nombre\" value=\"".$rowxx["nombre"]."\"></td></tr></table></td>";
								...
								if ($row["pvd"] == 0){
									...
									echo "<td><input type=\"Text\" style=\"width:50px\" name=\"pvd\" style=\"text-align:right;background-color:white;color:silver\" value=\"".$rowa["preu_cost"]."\"></td>";
								} else {
									echo "<td><input type=\"Text\" style=\"width:50px\" name=\"pvd\" style=\"text-align:right;background-color:white;color:silver\" value=\"".$row["pvd"]."\"></td>";
								}
								echo "<td><input type=\"Text\" style=\"width:50px\" name=\"pvp\" style=\"text-align:right;\" value=\"".$rowxx["pvp"]."\"></td>";
								echo "<td><input type=\"Text\" style=\"width:50px\" name=\"descuento\" style=\"text-align:right;\" value=\"".$rowxx["descuento"]."\"></td>";
								echo "<td><input type=\"Text\" style=\"width:50px\" name=\"descuento_absoluto\" style=\"text-align:right;\" value=\"".$rowxx["descuento_absoluto"]."\"></td>";
							echo "</tr>";
						}
						echo "<tr><td>&nbsp;</td></tr>";
					}
	...
	if ($soption == 22) {
		...
		if ($ssoption == 4) {
			...
			$sql = $sql.",version=".$_POST["version"];
			$sql = $sql.",numero_rfq='".$_POST["numero_rfq"]."'";
			$sql = $sql.",numero_cliente='".$_POST["numero_cliente"]."'";
			$sql = $sql.",fecha='".cambiarFormatoFechaDMY($_POST["fecha"])."'";
			$sql = $sql.",fecha_prevision='".cambiarFormatoFechaDMY($_POST["fecha_prevision"])."'";
			$sql = $sql.",fecha_entrega='".cambiarFormatoFechaDMY($_POST["fecha_entrega"])."'";
			$sql = $sql.",fecha_vencimiento='".cambiarFormatoFechaDMY($_POST["fecha_vencimiento"])."'";
			$sql = $sql.",nombre='".$_POST["nombre"]."'";
			$sql = $sql.",cnombre='".$_POST["cnombre"]."'";
			$sql = $sql.",cmail='".$_POST["cmail"]."'";
			$sql = $sql.",ctelefono='".$_POST["ctelf"]."'";
			...
			$sql = $sql.",edireccion='".$_POST["edireccion"]."'";
			$sql = $sql.",epoblacion='".$_POST["epoblacion"]."'";
			$sql = $sql.",ecp='".$_POST["ecp"]."'";
			$sql = $sql.",eprovincia='".$_POST["eprovincia"]."'";
			$sql = $sql.",notas='".$_POST["notas"]."'";
			$sqlpermiso2 = "select * from sgm_factura_tipos_permisos where id_tipo=".$_GET["id_tipo"]." and id_user=".$userid;
			$resultpermiso2 = mysql_query(convertSQL($sqlpermiso2));
			$rowpermiso2 = mysql_fetch_array($resultpermiso2);
			if ($rowpermiso2["admin"] == 1) {
				$sql = $sql.",id_cliente=".$_POST["id_client"];
				$sql = $sql.",id_user=".$_POST["id_user"];
				$sql = $sql.",id_pagador=".$_POST["id_pagador"];
			} else {
				$sql = $sql.",id_cliente=".$rowf["id_cliente"];
				$sql = $sql.",id_user=".$rowf["id_user"];
				$sql = $sql.",id_pagador=".$_POST["id_pagador"];
			}
			$sql = $sql.",imp_exp=".$_POST["imp_exp"];
			$sql = $sql.",id_divisa=".$_POST["id_divisa"];
			if ($_POST["id_divisa"] != $rowf["id_divisa"]){
				if ($_POST["id_divisa"] > 0) {
					$sqldi = "select * from sgm_divisas where id=".$_POST["id_divisa"];
				...
				$sql = $sql.",div_canvi=".$_POST["div_canvi"];
			}
			$sql = $sql." WHERE id=".$_GET["id"]."";
			mysql_query(convertSQL($sql));
			$data = getdate();
		...
		if ($fu == ""){
									...
									echo "<td>";
										echo "<table><tr>";
											...
											echo "<table>";
												...
												$sqlp = "select * from sys_naciones where visible=1 order by Nacion";
												$resultp = mysql_query(convertSQL($sqlp));
												while ($rowp = mysql_fetch_array($resultp)){
													if ($row["id_pais"] == $rowp["CodigoNacion"]){
														echo "<option value=\"".$rowp["CodigoNacion"]."\" selected>".$rowp["Nacion"]."</option>";
													} else {
														if ($rowp["predeterminado"] == 1){
															echo "<option value=\"".$rowp["CodigoNacion"]."\" selected>".$rowp["Nacion"]."</option>";
														} else {
															echo "<option value=\"".$rowp["CodigoNacion"]."\">".$rowp["Nacion"]."</option>";
														}
													}
												}
												echo "</select></td></tr>";
							...
							echo "<td style=\"vertical-align:top;\">";
								...
								$rowpermiso2 = mysql_fetch_array($resultpermiso2);
									...
									echo "</tr><tr>";
										...
										echo "<td>";
											echo "<table>";
												echo "<tr><td>".$Divisa."</td><td>".$Cambio."</td></tr>";
												echo "<tr><td><select name=\"id_divisa\" style=\"width:150px;\">";
													...
													while ($row1 = mysql_fetch_array($result1)) {
														if ($row["id_divisa"] == $row1["id"]) {
															echo "<option value=\"".$row1["id"]."\" selected>".$row1["divisa"]."-".$row1["canvi"]."</option>";
														} else {
															if ($row1["predefinido"] == 1){
																echo "<option value=\"".$row1["id"]."\" selected>".$row1["divisa"]."-".$row1["canvi"]."</option>";
															} else {
																echo "<option value=\"".$row1["id"]."\">".$row1["divisa"]."-".$row1["canvi"]."</option>";
															}
														}
													}
													echo "</select>";
			...
			while ($row = mysql_fetch_array($result)) {
				...
				echo "<tr style=\"background-color : ".$color.";\">";
					...
					echo "<td style=\"text-align:right;background-color : silver;\"><strong>".$row["total"]."</strong> ".$row2["abrev"]."</td>";
					if ($rowtipos["presu"] == 1){
			...
			$result = mysql_query(convertSQL($sql));
			$row = mysql_fetch_array($result);
			echo "<tr><td style=\"text-align:right;\" colspan=\"11\"><em>".$Subtotal."</em></td><td style=\"text-align:right;\">".number_format ($row["subtotal"],3)." ".$row2["abrev"]."</td><td></td></tr>";
			...
			echo "</form>";
			echo "<form action=\"index.php?op=1003&sop=27&id=".$row["id"]."\" method=\"post\">";
			echo "<tr><td style=\"text-align:right;\" colspan=\"11\"><em>Desc.".$row2["abrev"]."</em></td><td><input type=\"Text\" name=\"descuento_absoluto\" style=\"text-align:right;width:60px\" value=\"".number_format ($row["descuento_absoluto"],3)."\"> ".$row2["abrev"]."</td><td><input type=\"Submit\" value=\"".$Modificar."\" style=\"width:70px\"></td></tr>";
			echo "</form>";
			...
			echo "<tr><td style=\"text-align:right;\" colspan=\"11\"><em>".$Total."</em></td><td><input type=\"Text\" name=\"total\" style=\"text-align:right;width:60px;background-color:".$color_fondo_total."\" value=\"".number_format ($row["total"],3)."\"> ".$row2["abrev"]."</td><td><input type=\"Submit\" value=\"".$Modificar."\" style=\"width:70px\"></td></tr>";
			echo "</form>";
			echo "</table>";
			echo "<br><br>";
			echo "</center>";
	...
	if ($soption == 52) {
		...
		$sql = $sql.",'".calculDataVencimientFact($row["id_cliente"],$_POST["fecha_nueva"])."'";
		$sql = $sql.",1";
		$sql = $sql.",'".$row["nombre"]."'";
		$sql = $sql.",'".$row["nif"]."'";
		$sql = $sql.",'".$row["direccion"]."'";
...
#						echo "<td><table cellpadding=\"0\" cellspacing=\"0\"><tr>";
						...
						$resultca2 = mysql_query(convertSQL($sqlca2));
...
#					if ($rowtipos["v_recibos"] == 1) { $sql = $sql." and cobrada=1"; }
	...
	if ($soption == 105) {
		...
		while ($dia_actual<=$mes_ultimo){
			...
			if ($m == 7) {$meso = "Julio";}
			if ($m == 8) {$meso = "Agosto";}
			if ($m == 9) {$meso = "Septiembre";}
			if ($m == 10) {$meso = "Octubre";}
			if ($m == 11) {$meso = "Noviembre";}
			if ($m == 12) {$meso = "Diciembre";}
			if ($contador != $m){
...
#							if (($_GET["id"] == 5) AND ($row["cerrada"] == 1)) { $ver = 0; }
	...
	if ($soption == 200) {
		...
		echo "<center><table border=\"0px\" cellpadding=\"0\">";
			echo "<tr>";
				...
				$resulrecs = mysql_query(convertSQL($sqlrecs));
	...
	if ($soption == 310){
		if ($ssoption == 1) {
			...
			$row = mysql_fetch_array($result);
			if ($row["total"] == 0){
				$sql = "insert into sgm_dades_origen_factura (nombre,nif,direccion,poblacion,cp,provincia,mail,telefono,notas,cuenta_bancaria) ";
				$sql = $sql."values (";
			...
			if ($row["total"] == 1) {
				...
				$sql = $sql.",telefono='".$_POST["telefono"]."'";
				$sql = $sql.",notas='".$_POST["notas"]."'";
				$sql = $sql.",cuenta_bancaria='".$_POST["cuenta_bancaria"]."'";
				mysql_query(convertSQL($sql));
			}
		}
		...
		echo "<table><tr>";
			echo "<td style=\"vertical-align:top\">";
				...
				echo "<form action=\"index.php?op=1003&sop=310&ssop=1\" method=\"post\">";
					echo "<tr><td>".$Nombre."</td><td><input type=\"Text\" name=\"nombre\" class=\"px400\" value=\"".$row["nombre"]."\"></td></tr>";
					echo "<tr><td>NIF</td><td><input type=\"Text\" name=\"nif\" class=\"px400\" value=\"".$row["nif"]."\"></td></tr>";
					echo "<tr><td>".$Direccion."</td><td><input type=\"Text\" name=\"direccion\" class=\"px400\" value=\"".$row["direccion"]."\"></td></tr>";
					echo "<tr><td>".$Poblacion."</td><td><input type=\"Text\" name=\"poblacion\" class=\"px400\" value=\"".$row["poblacion"]."\"></td></tr>";
					echo "<tr><td>".$Codigo." ".$Postal."</td><td><input type=\"Text\" name=\"cp\" class=\"px400\" value=\"".$row["cp"]."\"></td></tr>";
					echo "<tr><td>".$Provincia."</td><td><input type=\"Text\" name=\"provincia\" class=\"px400\" value=\"".$row["provincia"]."\"></td></tr>";
					echo "<tr><td>".$Email."</td><td><input type=\"Text\" name=\"mail\" class=\"px400\" value=\"".$row["mail"]."\"></td></tr>";
					echo "<tr><td>".$Telefono."</td><td><input type=\"Text\" name=\"telefono\" class=\"px400\" value=\"".$row["telefono"]."\"></td></tr>";
					echo "<tr><td>IBAN</td><td><input type=\"Text\" name=\"cuenta_bancaria\" class=\"px400\" value=\"".$row["cuenta_bancaria"]."\"></td></tr>";
					echo "<tr><td>".$Notas."</td><td><textarea name=\"notas\" class=\"px400\" rows=\"6\">".$row["notas"]."</textarea></td></tr>";
	...
	if ($soption == 700) {
		if ($ssoption == 1) {
			$sql = "insert into sgm_divisas (abrev,divisa,canvi) ";
			$sql = $sql."values (";
			$sql = $sql."'".$_POST["abrev"]."'";
		...
		echo "<table cellspacing=\"0\">";
			...
			while ($row = mysql_fetch_array($result)) {
				...
				if ($row["predefinido"] == 0) {
					...
					echo "</form>";
					echo "<td style=\"text-align:center;\"><a href=\"index.php?op=1003&sop=700&ssop=3&id=".$row["id"]."\"><img src=\"mgestion/pics/icons-mini/page_white_delete.png\" style=\"border:0px\"></a></td>";
				}
