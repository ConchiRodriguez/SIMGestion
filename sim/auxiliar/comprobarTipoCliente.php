...
{ 
	...
	while ($rowc = mysqli_fetch_array($resultc)){
		...
		while ($rowct = mysqli_fetch_array($resultct)){
			$valido = 0;
			$sqlct1 = "select count(*) as total from sgm_cabezera where visible=1 and id_cliente=".$rowc["id"]." and tipo=".$rowct["id_tipo_factura"];
			$resultct1 = mysqli_query($dbhandle,$sqlct1);
			$rowct1 = mysqli_fetch_array($resultct1);
			if (($rowct1["total"] > 0) and ($rowct["factura"] == 1)) { $valido = 1; }
			...
			if (($rowct2["total"] > 0) and ($rowct["contrato"] == 1)) { $valido = 1; }
			...
			if (($rowct3["total"] > 0) and ($rowct["incidencia"] == 1)) { $valido = 1; }
			if (($rowct3["total"] > 0) and ($rowct["incidencia"] == 0)) { $valido = 0; }

			if (($rowc["tipo_identificador"] == 2) or ($rowc["tipo_identificador"] == 3)) { $check_nif = 1;} else { $check_nif = valida_nif_cif_nie($rowc["nif"]);}
			if (($rowct["datos_fiscales"] == 0) and (($check_nif <= 0) OR ($rowc["nombre"] == "") OR ($rowc["direccion"] == "") OR ($rowc["cp"] == "") OR ($rowc["poblacion"] == "") OR ($rowc["provincia"] == ""))) { $valido = 1; }
